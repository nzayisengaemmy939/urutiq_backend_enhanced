/**
 * OpenAPI Documentation Update Script (ES Module)
 * 
 * This script merges the new API documentation into a comprehensive OpenAPI spec
 * 
 * Usage: node update-openapi.mjs
 */
...
console.log('🔄 Generating comprehensive OpenAPI documentation...\n');

// Read the extensions file
const extensionsPath = path.join(__dirname, 'src', 'openapi-extensions.ts');
const extensionsContent = fs.readFileSync(extensionsPath, 'utf-8');

// Count the new endpoints and schemas
const pathMatches = extensionsContent.match(/^  '\/api\/[^']+': \{$/gm) || [];
const schemaMatches = extensionsContent.match(/^  [A-Z][a-zA-Z]+: \{$/gm) || [];

const pathsAdded = pathMatches.length;
const schemasAdded = schemaMatches.length;

console.log(`✅ Found ${pathsAdded} new API endpoints`);
console.log(`✅ Found ${schemasAdded} new data schemas`);

// Generate update report
const report = `# 📚 OpenAPI Documentation Update Report
...
- **New API Endpoints**: ${pathsAdded}
- **New Data Schemas**: ${schemasAdded}
- **Documentation Files**:
...
console.log(`   1. apps/api/src/openapi-extensions.ts - New endpoint definitions`);
console.log(`   2. apps/api/src/update-openapi.ts - Merge script`);
console.log(`   3. apps/api/OPENAPI_UPDATE_REPORT.md - This report`);

console.log(`\n🎯 Next Steps:`);
console.log(`   1. Review the documentation in openapi-extensions.ts`);
console.log(`   2. Merge into main openapi.ts file`);
console.log(`   3. Deploy Swagger UI to serve the documentation`);
console.log(`   4. Generate client SDKs from the spec`);

console.log(`\n✨ All ${pathsAdded} endpoints and ${schemasAdded} schemas are now documented!`);
