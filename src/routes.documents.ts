...
export function mountDocumentRoutes(router: Router) {
  // Document CRUD Operations
  
  // List documents with filtering, search, and pagination
  router.get('/documents', async (req: TenantRequest, res) => {
    try {
  ...
  router.get('/documents/stats', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
  ...
  // Get document by ID
  router.get('/documents/:id', async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
  ...
  // Upload new document
  router.post('/documents/upload', async (req: TenantRequest, res) => {
    ...
    multer.single('file')(req as any, res as any, async (err: any) => {
      try {
        if (err) {
  ...
  // Download document
  router.get('/documents/:id/download', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/documents/:id/preview', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/documents/:id/preview-image', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.put('/documents/:id', validateBody(schemas.documentUpdate), async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.delete('/documents/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.post('/documents/:id/restore', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  // List categories
  router.get('/documents/categories', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
  ...
  // Create category
  router.post('/documents/categories', validateBody(schemas.documentCategoryCreate), async (req: TenantRequest, res) => {
  ...
  // Update category
  router.put('/documents/categories/:id', validateBody(schemas.documentCategoryUpdate), async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  // Delete category
  router.delete('/documents/categories/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/documents/analytics', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
    ...
    res.json({
      ...
      uploadTrends: uploadTrends.map(trend => ({
        date: (trend.uploadedAt as any)?.toISOString?.().split('T')[0] ?? null,
        count: (trend._count as any)?.id ?? 0,
        size: (trend._sum as any)?.sizeBytes ?? 0
      })),
      categoryDistribution: categoryDistribution.map(cat => ({
        categoryId: cat.categoryId,
        count: (cat._count as any)?.id ?? 0,
        size: (cat._sum as any)?.sizeBytes ?? 0
      })),
      fileTypeDistribution: fileTypeDistribution.map(type => ({
        mimeType: type.mimeType,
        count: (type._count as any)?.id ?? 0,
        size: (type._sum as any)?.sizeBytes ?? 0
      })),
      userActivity: userActivity.map(user => ({
        userId: user.uploaderId,
        count: (user._count as any)?.id ?? 0,
        size: (user._sum as any)?.sizeBytes ?? 0
      })),
      workflowStats: workflowStats.map(workflow => ({
        status: workflow.status,
        count: (workflow._count as any)?.id ?? 0
      }))
    });
  });

  // Get storage usage report
  router.get('/documents/storage-report', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
  ...
  const totalStorage = storageByCategory.reduce((sum, cat) => sum + ((cat._sum as any)?.sizeBytes ?? 0), 0);
  const totalFiles = storageByCategory.reduce((sum, cat) => sum + ((cat._count as any)?.id ?? 0), 0);
  ...
  count: (cat._count as any)?.id ?? 0,
  size: (cat._sum as any)?.sizeBytes ?? 0,
  percentage: totalStorage > 0 ? (((cat._sum as any)?.sizeBytes ?? 0) / totalStorage * 100).toFixed(2) : '0'
      })),
      storageByWorkspace: storageByWorkspace.map(ws => ({
        workspaceId: ws.workspaceId,
  count: (ws._count as any)?.id ?? 0,
  size: (ws._sum as any)?.sizeBytes ?? 0,
  percentage: totalStorage > 0 ? (((ws._sum as any)?.sizeBytes ?? 0) / totalStorage * 100).toFixed(2) : '0'
      })),
      storageByFileType: storageByFileType.map(type => ({
        mimeType: type.mimeType,
  count: (type._count as any)?.id ?? 0,
  size: (type._sum as any)?.sizeBytes ?? 0,
  percentage: totalStorage > 0 ? (((type._sum as any)?.sizeBytes ?? 0) / totalStorage * 100).toFixed(2) : '0'
      })),
  ...
  // Recent activity
  router.get('/documents/activity', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
  ...
  router.post('/documents/:id/share', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/documents/:id/shares', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.put('/documents/shares/:shareId', async (req: TenantRequest, res) => {
  ...
  router.delete('/documents/shares/:shareId', async (req: TenantRequest, res) => {
    const { shareId } = req.params;
  ...
  router.post('/documents/:id/workflows', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.put('/documents/workflows/:workflowId', async (req: TenantRequest, res) => {
    ...
    if (workflow.assignedTo !== (req as any).user.id && 
  ...
  // Create new version
  router.post('/documents/:id/versions', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/documents/:id/versions', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/documents/search', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
    ...
    if (sizeMin > 0 || sizeMax < 999999999) {
      where.sizeBytes = {};
      if (sizeMin > 0) where.sizeBytes.gte = sizeMin;
      if (sizeMax < 999999999) where.sizeBytes.lte = sizeMax;
    }
  ...
  router.put('/documents/bulk', async (req: TenantRequest, res) => {
  ...
  router.delete('/documents/bulk', async (req: TenantRequest, res) => {
  ...
  router.post('/documents/:id/analyze', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/documents/:id/analysis', async (req: TenantRequest, res) => {
    const { id } = req.params;
    ...
    if (!latestAnalysis) {
      ...
      const inProgress = analysisActivities.find(a => 
        a.action === 'ai_analysis_started' && 
  ...
  router.post('/documents/:id/classify', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.post('/documents/templates', async (req: TenantRequest, res) => {
  ...
  router.get('/documents/templates', async (req: TenantRequest, res) => {
    const categoryId = String(req.query.categoryId || '');
  ...
  router.get('/documents/ai-search', async (req: TenantRequest, res) => {
  ...
  router.get('/documents/:id/insights', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.post('/documents/automate', async (req: TenantRequest, res) => {
  ...
  router.get('/documents/automations', async (req: TenantRequest, res) => {
    ...
    if (automationType) where.details = { contains: automationType };
  ...
  router.post('/documents/:id/access-controls', validateBody(schemas.accessControlCreate), async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/documents/:id/audit-log', async (req: TenantRequest, res) => {
    const { id } = req.params;
    ...
    if (users) {
      where.userId = { in: (users as string).split(',') };
    }
  ...
  router.post('/documents/:id/webhooks', validateBody(schemas.webhookCreate), async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/documents/:id/webhooks/:webhookId/deliveries', async (req: TenantRequest, res) => {
  ...
  router.get('/documents/analytics/business-intelligence', async (req: TenantRequest, res) => {
  ...
  uploadedAt: { gte: start, lte: end }
    ...
    res.json({
      ...
      storageMetrics: {
        totalSize: (storageMetrics._sum as any)?.sizeBytes ?? 0,
        averageSize: (storageMetrics._avg as any)?.sizeBytes ?? 0,
        totalDocuments: (storageMetrics._count as any)?.id ?? 0
      }
    });
  });
  
  // Performance Metrics & Predictive Insights
  router.get('/documents/analytics/performance', async (req: TenantRequest, res) => {
  ...
  router.post('/documents/:id/workflows/advanced', validateBody(schemas.advancedWorkflowCreate), async (req: TenantRequest, res) => {
    const { id } = req.params;
    ...
    const workflow = await prisma.documentWorkflow.create({
      data: {
        ...
        status: 'pending',
        assignedTo: steps[0]?.assignedTo || (req as any).user.id,
  ...
  router.post('/documents/workflows/:workflowId/progress', validateBody(schemas.workflowStepProgress), async (req: TenantRequest, res) => {
    const { workflowId } = req.params;
  ...
  router.post('/documents/compliance/automated-checks', validateBody(schemas.complianceCheckCreate), async (req: TenantRequest, res) => {
  ...
  router.get('/documents/compliance/checks/:checkId', async (req: TenantRequest, res) => {
  ...
  // Automated Reporting
  router.post('/documents/reports/automated', validateBody(schemas.automatedReportCreate), async (req: TenantRequest, res) => {
  ...
  router.get('/documents/reports/automated/:reportId/history', async (req: TenantRequest, res) => {
