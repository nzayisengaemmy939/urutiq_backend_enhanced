...
export function mountExpenseRoutes(router: Router) {
  ...
  async function adjustSpecificBudget(opts: {
    budgetId: string;
    amount: number; // positive to increment, negative to decrement
  }) {
    const { budgetId, amount } = opts;
    if (!budgetId || budgetId === 'auto') return;
  ...
  async function validateSpecificBudget(opts: {
    ...
    if (expenseDate) {
      ...
      if (expenseDate < budgetStart || expenseDate > budgetEnd) {
        throw new Error(
          `Expense date ${expenseDate.toISOString().split('T')[0]} is outside budget period. ` +
  ...
  router.post('/expense-categories/seed/:companyId', async (req: TenantRequest, res) => {
  ...
  router.get('/expense-categories', async (req: TenantRequest, res) => {
  ...
  router.get('/expense-categories/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.post('/expense-categories', validateBody(expenseSchemas.category), async (req: TenantRequest, res) => {
  ...
  router.put('/expense-categories/:id', validateBody(expenseSchemas.category), async (req: TenantRequest, res) => {
    const { id } = req.params;
    const data = req.body as any;
  ...
  router.delete('/expense-categories/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/budgets/available', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
    const categoryId = req.query.categoryId as string;
    const expenseDate = req.query.expenseDate as string;
  ...
  // Get budgets
  router.get('/budgets', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
  ...
  // Budget analytics
  router.get('/budgets/analytics', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
    try {
      const where: any = { tenantId: req.tenantId, companyId: companyId || undefined };
  ...
  router.post('/budgets/recalculate-spent', async (req: TenantRequest, res) => {
    ...
    try {
      ...
      const byCategory: Record<string, Array<{ amount: number; date: Date }>> = {};
      for (const e of expenses) {
        if (!e.categoryId) continue;
        const key = e.categoryId;
        const amt = Number(e.totalAmount || e.amount || 0);
        const dateVal = new Date(e.expenseDate as unknown as string);
      ...
      for (const b of budgets) {
        ...
        const spent = bucket
          .filter(x => x.date >= (b.startDate as unknown as Date) && x.date <= (b.endDate as unknown as Date))
  ...
  // Get single budget
  router.get('/budgets/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  // Create budget
  router.post('/budgets', validateBody(expenseSchemas.budget), async (req: TenantRequest, res) => {
    const data = req.body as any;
  ...
  // Update budget
  router.put('/budgets/:id', validateBody(expenseSchemas.budget), async (req: TenantRequest, res) => {
    const { id } = req.params;
    const data = req.body as any;
  ...
  // Delete budget
  router.delete('/budgets/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  // Get expense rules
  router.get('/expense-rules', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
    const categoryId = req.query.categoryId as string || undefined;
    const isActive = req.query.isActive === 'true';
  ...
  router.get('/expense-rules/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  // Create expense rule
  router.post('/expense-rules', validateBody(expenseSchemas.rule), async (req: TenantRequest, res) => {
    const data = req.body as any;
  ...
  // Update expense rule
  router.put('/expense-rules/:id', validateBody(expenseSchemas.rule), async (req: TenantRequest, res) => {
    const { id } = req.params;
    const data = req.body as any;
  ...
  // Delete expense rule
  router.delete('/expense-rules/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  // Get budget analysis
  router.get('/budget-analysis', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
  ...
  router.get('/expense-summary', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
    const startDate = req.query.startDate as string;
    const endDate = req.query.endDate as string;
  ...
  // Get expenses
  router.get('/expenses', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
    const status = req.query.status as string;
    const categoryId = req.query.categoryId as string;
    const accountId = req.query.accountId as string;
    const paymentMethod = req.query.paymentMethod as string;
    const isBillable = req.query.isBillable as string;
  ...
  // Get single expense
  router.get('/expenses/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  // Create expense
  router.post('/expenses', async (req: TenantRequest, res) => {
    const data = req.body as any;
  ...
  // Update expense
  router.put('/expenses/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
    const data = req.body as any;
    ...
    try {
      ...
      if (prev) {
        ...
        const prevDate = prev.expenseDate ? new Date(prev.expenseDate as unknown as string) : undefined;
        const newDate = expense.expenseDate ? new Date(expense.expenseDate as unknown as string) : undefined;
  ...
  // Delete expense
  router.delete('/expenses/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.post('/expenses/:id/submit', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.post('/expenses/:id/approve', async (req: TenantRequest, res) => {
    const { id } = req.params;
        ...
        if (selectedBudgetId && selectedBudgetId !== 'auto') {
          try {
            const budgetValidation = await validateSpecificBudget({
              ...
              expenseAmount,
              expenseDate: existingExpense.expenseDate as unknown as Date
            });
  ...
  router.post('/expenses/bulk-assign-gl-accounts', async (req: TenantRequest, res) => {
    try {
  ...
  router.get('/account-codes/audit', async (req: TenantRequest, res) => {
    try {
  ...
  router.post('/account-codes/create-standard', async (req: TenantRequest, res) => {
    try {
  ...
  router.post('/account-codes/validate', async (req: TenantRequest, res) => {
    try {
  ...
  router.get('/account-codes/recommend', async (req: TenantRequest, res) => {
    try {
      ...
      const recommendedAccount = await seniorAccountCodeManager.getRecommendedAccount(
        String(usageContext), 
        req.tenantId!, 
        companyId
      );
  ...
  router.get('/expenses/:id/journal-entries', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  // Reject expense
  router.post('/expenses/:id/reject', async (req: TenantRequest, res) => {
    const { id } = req.params;
