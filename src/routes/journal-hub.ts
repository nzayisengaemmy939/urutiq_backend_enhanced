...
export interface JournalEntryType {
  ...
  name: string;
  description: string;
  category: 'SALES' | 'EXPENSE' | 'ADJUSTMENT' | 'TRANSFER' | 'DEPRECIATION' | 'ACCRUAL' | 'REVERSAL' | 'CUSTOM';
...
export function mountJournalHubRoutes(router: Router) {
  
  // ==================== JOURNAL ENTRY TYPES ====================
  
  // Get all journal entry types
  router.get('/entry-types', async (req: TenantRequest, res) => {
    try {
  ...
  router.post('/entry-types', validateBody(journalEntryTypeCreate), async (req: TenantRequest, res) => {
    try {
      ...
      const entryType = await prisma.$transaction(async (tx) => {
        ...
        if (allowedAccountIds && allowedAccountIds.length > 0) {
          await tx.journalEntryTypeAccount.createMany({
            data: allowedAccountIds.map((accountId: string) => ({
              tenantId: req.tenantId!,
  ...
  router.post('/templates/from-entry/:entryId', async (req: TenantRequest, res) => {
    try {
      ...
      const template = await prisma.$transaction(async (tx) => {
        const created = await tx.journalEntryTemplate.create({
          data: {
            ...
            name,
            description,
            entryTypeId: (entry as any).entryTypeId || null,
            isRecurring,
            frequency,
            isActive: true
          }
  ...
  router.get('/entries', async (req: TenantRequest, res) => {
    try {
      ...
      // Parse filters
      const filters: JournalEntrySearchFilters = {
      ...
      if (filters.entryType) whereClause.entryTypeId = filters.entryType;
      if (filters.reference) whereClause.reference = { contains: filters.reference };
      if (filters.memo) whereClause.memo = { contains: filters.memo };
      ...
      if (filters.department || filters.project || filters.location) {
        ...
        if (filters.department) dimensionFilter.department = filters.department;
        if (filters.project) dimensionFilter.project = filters.project;
        if (filters.location) dimensionFilter.location = filters.location;
  ...
  router.post('/entries', validateBody(journalEntryCreate), async (req: TenantRequest, res) => {
    try {
      const {
        date,
        memo,
      ...
      if (entryTypeId) {
        ...
        const allowedAccountIds = entryType.allowedAccounts.map((aa: any) => aa.accountId);
        const usedAccountIds = lines.map((l: any) => l.accountId);
        const invalidAccounts = usedAccountIds.filter((id: string) => !allowedAccountIds.includes(id));
        ...
        if (entryType.maxAmount) {
          const totalAmount = lines.reduce((sum: number, line: any) => sum + (line.debit || 0) + (line.credit || 0), 0);
      ...
      const debits = lines.reduce((sum: number, line: any) => sum + (line.debit || 0), 0);
      const credits = lines.reduce((sum: number, line: any) => sum + (line.credit || 0), 0);
  ...
  router.get('/summary', async (req: TenantRequest, res) => {
    try {
      ...
      const totalDebits = entriesWithLines.reduce((sum, entry) => 
        sum + entry.lines.reduce((lineSum, line) => lineSum + Number(line.debit || 0), 0), 0
      );
      const totalCredits = entriesWithLines.reduce((sum, entry) => 
        sum + entry.lines.reduce((lineSum, line) => lineSum + Number(line.credit || 0), 0), 0
      );
      ...
      const summary: JournalEntrySummary = {
        ...
        postedToday,
        byType: {},
        byStatus: byStatus.reduce((acc: any, item: any) => ({ ...acc, [item.status]: item._count?.id || 0 }), {}),
        byMonth
      };
  ...
  router.post('/approvals/:id/approve', async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
      ...
      try {
        await emailNotificationService.sendApprovalResponse(
          approval.entryId, 
          true, 
          req.user?.id!, 
          comments
  ...
  router.post('/approvals/:id/reject', async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
      const { comments } = req.body;
  ...
  router.get('/recurring', async (req: TenantRequest, res) => {
    try {
      const companyId = String(req.query.companyId || '');
      if (!companyId) {
        return res.status(400).json({ error: 'companyId_required', message: 'Company ID is required' });
  ...
  router.post('/recurring/process', async (req: TenantRequest, res) => {
    try {
  ...
  router.post('/entries/:id/reverse', validateBody(journalEntryReversal), async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
      ...
      const result = await prisma.$transaction(async (tx) => {
        ...
        const reversalEntry = await tx.journalEntry.create({
          data: {
            ...
            status: 'POSTED',
            entryTypeId: (originalEntry as any).entryTypeId || undefined,
            createdById: req.user?.id
          } as any
        });
        ...
        const reversalLines = [];
        for (const originalLine of (originalEntry as any).lines || []) {
      ...
      try {
        await emailNotificationService.sendStatusChange(
          id, 
          'REVERSED', 
          reason
        );
  ...
  router.post('/entries/:id/adjust', validateBody(journalEntryAdjustment), async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
  ...
  router.post('/entries/:id/request-approval', validateBody(journalEntryApprovalRequest), async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
      ...
      try {
        ...
        const approverEmails = approverUsers
          .filter((approver: any) => approver.email)
          .map((approver: any) => approver.email!);
  ...
  router.get('/permissions/:userId', async (req: TenantRequest, res) => {
    try {
  ...
  router.get('/metrics', async (req: TenantRequest, res) => {
    try {
  ...
  router.get('/entries/:id/audit', async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
  ...
  router.post('/entries/batch', validateBody(journalEntryBatchCreate), async (req: TenantRequest, res) => {
    try {
      ...
      await prisma.$transaction(async (tx) => {
        for (let i = 0; i < entries.length; i++) {
          ...
          try {
            ...
            // Calculate totals
            const totalDebit = entryData.lines.reduce((sum: number, line: any) => sum + (Number(line.debit) || 0), 0);
            const totalCredit = entryData.lines.reduce((sum: number, line: any) => sum + (Number(line.credit) || 0), 0);
            ...
            const journalEntry = await tx.journalEntry.create({
              data: {
                tenantId: req.tenantId!,
                companyId: entryData.companyId || req.query.companyId as string,
  ...
  router.post('/entries/batch/approve', validateBody(journalEntryBatchApprove), async (req: TenantRequest, res) => {
    try {
  ...
  router.post('/entries/batch/post', validateBody(journalEntryBatchPost), async (req: TenantRequest, res) => {
    try {
  ...
  router.post('/entries/batch/reverse', validateBody(journalEntryBatchReverse), async (req: TenantRequest, res) => {
    try {
  ...
  router.get('/entries/batch/status/:batchId', async (req: TenantRequest, res) => {
    try {
  ...
  router.get('/entries/export/csv', async (req: TenantRequest, res) => {
    try {
      const { 
        companyId, 
        dateFrom, 
      ...
      if (entryType && entryType !== 'all') whereClause.entryTypeId = entryType;
  ...
  router.get('/entries/export/excel', async (req: TenantRequest, res) => {
    try {
      const { 
        companyId, 
        dateFrom, 
      ...
      const csvResponse = await fetch(`${req.protocol}://${req.get('host')}/api/journal-hub/entries/export/csv?${new URLSearchParams(req.query as any).toString()}`);
  ...
  router.post('/entries/import/csv', validateBody(journalEntryCsvImport), async (req: TenantRequest, res) => {
    try {
  ...
  // Get import template
  router.get('/entries/import/template', async (req: TenantRequest, res) => {
    try {
  ...
  router.get('/entries/search/advanced', async (req: TenantRequest, res) => {
    try {
      const {
        companyId,
        searchTerm,
      ...
      if (searchTerm) {
        whereClause.OR = [
          { reference: { contains: searchTerm as string } },
          { memo: { contains: searchTerm as string } }
        ];
      }

      if (reference) whereClause.reference = { contains: reference as string };
      if (memo) whereClause.memo = { contains: memo as string };
      ...
      let lineFilters: any = {};
      if (department) lineFilters.department = { contains: department as string };
      if (project) lineFilters.project = { contains: project as string };
      if (location) lineFilters.location = { contains: location as string };
      ...
      if (amountFilters.min !== undefined || amountFilters.max !== undefined) {
        filteredEntries = processedEntries.filter(entry => {
          if (amountFilters.min !== undefined && entry.totalAmount < amountFilters.min) return false;
          if (amountFilters.max !== undefined && entry.totalAmount > amountFilters.max) return false;
          return true;
        });
      }
  ...
  // Save search query
  router.post('/search/save', validateBody(journalSearchSave), async (req: TenantRequest, res) => {
    try {
  ...
  // Get saved searches
  router.get('/search/saved', async (req: TenantRequest, res) => {
    try {
  ...
  // Update saved search
  router.put('/search/saved/:id', validateBody(journalSearchUpdate), async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
  ...
  // Delete saved search
  router.delete('/search/saved/:id', async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
  ...
  router.get('/search/suggestions', async (req: TenantRequest, res) => {
    try {
      ...
      switch (field) {
        case 'reference':
          const references = await prisma.journalEntry.findMany({
            where: {
              tenantId: req.tenantId!,
              reference: { contains: query as string }
            },
        ...
        case 'department':
          const departments = await prisma.journalLine.findMany({
            where: {
              tenantId: req.tenantId!,
              department: { contains: query as string }
            },
        ...
        case 'project':
          const projects = await prisma.journalLine.findMany({
            where: {
              tenantId: req.tenantId!,
              project: { contains: query as string }
            },
            select: { project: true },
            distinct: ['project'],
            take: 10
  ...
  router.get('/entries/:id/pdf', async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
      const { 
  ...
  router.post('/entries/pdf/batch', async (req: TenantRequest, res) => {
    try {
  ...
  router.get('/entries/:id/preview', async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
      const { 
        includeAuditTrail = 'true',
