...
router.get('/revenue-recognition/:companyId/schedules', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware(), requireRoles(['accountant','admin']), asyncHandler(async (req: TenantRequest, res: any) => {
  const { companyId } = req.params
  const items = await revenueRecognitionService.listSchedules(req.tenantId!, companyId)
  res.json({ items })
}))

router.post('/revenue-recognition/:companyId/schedules', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware(), requireRoles(['accountant','admin']), asyncHandler(async (req: TenantRequest, res: any) => {
  const { companyId } = req.params
  ...
  const overlaps = existing.some((s:any) => {
    if (contractId && s.contractId && s.contractId !== contractId) return false
...
router.post('/revenue-recognition/:companyId/run', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware(), requireRoles(['accountant','admin']), asyncHandler(async (req: TenantRequest, res: any) => {
  const { companyId } = req.params
...
router.post('/revenue-recognition/:companyId/post', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware(), requireRoles(['accountant','admin']), asyncHandler(async (req: TenantRequest, res: any) => {
  const { companyId } = req.params
...
router.post('/revenue-recognition/:companyId/schedules/:id/accruals', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware(), requireRoles(['accountant','admin']), asyncHandler(async (req: TenantRequest, res: any) => {
  ...
  const postings = revenueRecognitionService.computeAccrualsForSchedule(schedule as any, periodStart, periodEnd)
  res.json({ postings, schedule })
}))

router.delete('/revenue-recognition/:companyId/schedules/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware(), requireRoles(['accountant','admin']), asyncHandler(async (req: TenantRequest, res: any) => {
  const { companyId, id } = req.params
