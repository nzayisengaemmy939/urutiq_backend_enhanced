...
router.get('/debug/documents', async (req: Request, res: Response) => {
  try {
...
router.get('/debug/expiring', async (req: Request, res: Response) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string
    const companyId = req.headers['x-company-id'] as string
  ...
  try {
    ...
    const scoredDocuments = finalDocuments.map(doc => {
      ...
      if (query) {
        ...
        if (lowerName.includes(lowerQuery)) aiScore += 30
        if (lowerDesc.includes(lowerQuery)) aiScore += 20
        if (doc.mimeType.includes('pdf')) aiScore += 10
...
router.get('/quality-analysis', async (req: Request, res: Response) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string
    const companyId = req.headers['x-company-id'] as string
...
router.get('/categorization-suggestions', async (req: Request, res: Response) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string
    const companyId = req.headers['x-company-id'] as string
...
router.get('/extractions', async (req: Request, res: Response) => {
  try {
    const tenantId = req.headers['x-tenant-id'] as string
    const companyId = req.headers['x-company-id'] as string
...
router.post('/summaries/generate', async (req: Request, res: Response) => {
  try {
    ...
    const summaryContent = await AIService.generateSummary(
      `${type} Analysis`,
      'text/plain',
      documents.map(doc => `${doc.displayName || doc.name} (${doc.mimeType})`).join(', ')
    )
