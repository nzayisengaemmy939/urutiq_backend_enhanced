...
export function mountPOSRoutes(router: Router) {
  
  /**
   * Email receipt to customer
   */
  router.post('/pos/email-receipt', upload.single('receipt'), async (req: TenantRequest, res) => {
    try {
      ...
      try {
        ...
        res.json({
          ...
          sentTo: to,
          id: (info as any).messageId || 'sent-' + Date.now()
        });
   ...
   */
  router.post('/pos/hardware/test', validateBody(hardwareTestSchema), async (req: TenantRequest, res) => {
    try {
   ...
   */
  router.get('/pos/analytics', async (req: TenantRequest, res) => {
    try {
   ...
   */
  router.get('/pos/status', async (req: TenantRequest, res) => {
    try {
      const status = {
        system: {
          status: 'online',
   ...
   */
  router.post('/pos/scan-barcode', async (req: TenantRequest, res) => {
    try {
