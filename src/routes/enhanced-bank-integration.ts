...
router.post('/process-feed', asyncHandler(async (req: TenantRequest, res: any) => {
  ...
  try {
    ...
    const result = await enhancedBankIntegrationService.processBankFeed(
      connectionId,
      transactions as BankFeedTransaction[],
      config as BankIntegrationConfig || {
...
router.post('/reconcile', asyncHandler(async (req: TenantRequest, res: any) => {
  ...
  try {
    ...
    const bankTransactions: BankFeedTransaction[] = transactions.map(t => ({
      id: t.id,
      ...
      transactionType: t.transactionType as 'debit' | 'credit' | 'transfer',
...
router.post('/rules', asyncHandler(async (req: TenantRequest, res: any) => {
  ...
  try {
    ...
    const rule = await enhancedBankIntegrationService.createReconciliationRule(
      companyId,
      name,
      description,
      conditions,
      actions,
      'demo-user-id' // TODO: Get from authenticated user
    );
...
router.post('/test-connection/:connectionId', asyncHandler(async (req: TenantRequest, res: any) => {
...
router.get('/transactions/:connectionId', asyncHandler(async (req: TenantRequest, res: any) => {
  const { connectionId } = req.params;
  ...
  try {
    ...
    const transactionsWithInsights = await Promise.all(
...
router.post('/auto-categorize/:transactionId', asyncHandler(async (req: TenantRequest, res: any) => {
...
router.post('/fraud-analysis/:transactionId', asyncHandler(async (req: TenantRequest, res: any) => {
  const { transactionId } = req.params;
  const { tenantId } = req;
...
router.get('/connections/:companyId', asyncHandler(async (req: TenantRequest, res: any) => {
  const { companyId } = req.params;
  const { tenantId } = req;
...
router.post('/sync/:connectionId', asyncHandler(async (req: TenantRequest, res: any) => {
  const { connectionId } = req.params;
  ...
  try {
    ...
    const mockTransactions: BankFeedTransaction[] = [
      {
        id: `mock-${Date.now()}-1`,
