...
router.post('/three-way-match/:poId/receive',
...
router.post('/three-way-match/:poId/match/:billId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
...
router.get('/three-way-match/:companyId/approvals', 
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'), 
  requireRoles(['admin','accountant']), 
  asyncHandler(async (req: TenantRequest, res: any) => {
...
router.post('/three-way-match/:companyId/approvals', 
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'), 
  requireRoles(['admin']), 
  asyncHandler(async (req: TenantRequest, res: any) => {
...
router.post('/three-way-match/:poId/approve', 
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'), 
  requireRoles(['admin','accountant']), 
  asyncHandler(async (req: TenantRequest, res: any) => {
    const userId = req.user?.sub || 'demo-user-1'
...
router.post('/three-way-match/:poId/reject', 
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'), 
  requireRoles(['admin','accountant']), 
  asyncHandler(async (req: TenantRequest, res: any) => {
    const userId = req.user?.sub || 'demo-user-1'
