 ...
 */
router.get('/knowledge/search', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  ...
  try {
    ...
    try {
      ...
      let whereClause: any = { tenantId, isPublished: true };
      ...
      articles = await (prisma as any).knowledgeArticle.findMany({
        where: whereClause,
        orderBy: { views: 'desc' }
      });
    } catch (dbError) {
 ...
 */
router.post('/knowledge/articles', requireRoles(['admin', 'accountant']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  ...
  try {
    ...
    // Create article
    const article = await (prisma as any).knowledgeArticle.create({
      data: {
        tenantId,
        title,
        content,
 ...
 */
router.put('/knowledge/articles/:id', requireRoles(['admin', 'accountant']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  ...
  try {
    ...
    const existingArticle = await (prisma as any).knowledgeArticle.findFirst({
    ...
    // Update article
    const updatedArticle = await (prisma as any).knowledgeArticle.update({
      where: { id: articleId },
      data: {
 ...
 */
router.delete('/knowledge/articles/:id', requireRoles(['admin']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  const articleId = req.params.id;
  ...
  try {
    ...
    // Delete article
    await (prisma as any).knowledgeArticle.delete({
      where: { id: articleId }
    });
 ...
 */
router.get('/knowledge/articles', requireRoles(['admin', 'accountant']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;

  try {
    const articles = await (prisma as any).knowledgeArticle.findMany({
      where: { tenantId },
 ...
 */
router.get('/tickets', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
 ...
 */
router.post('/tickets', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
 ...
 */
router.put('/tickets/:id', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
 ...
 */
router.get('/tutorials', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  ...
  try {
    ...
    try {
      ...
      tutorials = await (prisma as any).tutorial.findMany({
        where: {
          tenantId,
          isPublished: true
        },
    ...
    // Format tutorials
    const formattedTutorials = tutorials.map((tutorial: any) => ({
      id: tutorial.id,
 ...
 */
router.post('/tutorials', requireRoles(['admin', 'accountant']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  ...
  try {
    // Create tutorial
    const tutorial = await (prisma as any).tutorial.create({
      data: {
        tenantId,
        title,
        description,
 ...
 */
router.put('/tutorials/:id', requireRoles(['admin', 'accountant']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  ...
  try {
    ...
    const existingTutorial = await (prisma as any).tutorial.findFirst({
    ...
    // Update tutorial
    const updatedTutorial = await (prisma as any).tutorial.update({
      where: { id: tutorialId },
      data: {
 ...
 */
router.delete('/tutorials/:id', requireRoles(['admin']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  const tutorialId = req.params.id;
  ...
  try {
    ...
    // Delete tutorial
    await (prisma as any).tutorial.delete({
      where: { id: tutorialId }
    });
 ...
 */
router.get('/tutorials/all', requireRoles(['admin', 'accountant']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;

  try {
    const tutorials = await (prisma as any).tutorial.findMany({
      where: { tenantId },
      orderBy: { createdAt: 'desc' }
    });
 ...
 */
router.post('/chat/sessions', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
 ...
 */
router.post('/chat/sessions/:sessionId/messages', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  ...
  try {
    ...
    try {
      // Create message
      chatMessage = await (prisma as any).chatMessage.create({
        data: {
          sessionId,
 ...
 */
router.get('/chat/sessions/:sessionId/messages', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  const sessionId = req.params.sessionId;
 ...
 */
router.get('/chat/sessions', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  ...
  try {
    ...
    try {
      ...
      sessions = await (prisma as any).chatSession.findMany({
 ...
 */
router.put('/chat/sessions/:sessionId/close', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  const sessionId = req.params.sessionId;
 ...
 */
router.get('/community/discussions', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  ...
  try {
    ...
    try {
      ...
      discussions = await (prisma as any).communityDiscussion.findMany({
        where: {
          tenantId
        },
        orderBy: [
    ...
    try {
      ...
      knowledgeBaseArticles = await (prisma as any).knowledgeArticle.count({ where: { tenantId, isPublished: true } });
    } catch (dbError) {
    ...
    const postsThisMonth = discussions.filter(d => 
 ...
 * GET /api/help/stats
 */
router.get('/stats', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
 ...
 */
router.post('/community/discussions', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  ...
  try {
    ...
    try {
      // Create discussion
      discussion = await (prisma as any).communityDiscussion.create({
        data: {
          tenantId,
          userId: userId || 'unknown',
          userName,
 ...
 */
router.post('/knowledge/articles/:id/view', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const { id } = req.params;
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
  ...
  try {
    // Check if article exists
    const article = await (prisma as any).knowledgeArticle.findFirst({
      where: { id, tenantId }
    });
 ...
 */
router.post('/knowledge/articles/:id/helpful', requireRoles(['admin', 'accountant', 'auditor', 'employee']), asyncHandler(async (req: TenantRequest, res: any, next: any) => {
  const { id } = req.params;
  const tenantId = req.tenantId!;
  const userId = req.user?.sub;
