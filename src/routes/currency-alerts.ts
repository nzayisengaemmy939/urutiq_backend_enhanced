...
router.put('/:id', asyncHandler(async (req: any, res: any) => {
  ...
  if (targetRate !== undefined || condition !== undefined) {
    const currentRate = await CurrencyService.getExchangeRate(
      existingAlert.fromCurrency, 
      existingAlert.toCurrency
    );
...
router.post('/check', asyncHandler(async (req: any, res: any) => {
  const { companyId } = req.query;
