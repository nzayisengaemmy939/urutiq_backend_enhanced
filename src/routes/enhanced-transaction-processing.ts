...
router.post('/receipts/batch-process',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
...
router.get('/receipts/:companyId/:receiptId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
...
// Update receipt data
router.put('/receipts/:companyId/:receiptId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
    const { companyId, receiptId } = req.params;
    ...
    try {
      ...
      if (vendor !== undefined) metadata.vendor = vendor;
...
// Delete receipt
router.delete('/receipts/:companyId/:receiptId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
    const { companyId, receiptId } = req.params;
...
router.post('/invoices/ai-create',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
...
router.get('/invoices/templates/:companyId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
    const { companyId } = req.params;
...
router.get('/customers/:companyId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
    const { companyId } = req.params;
    const { search, limit = 100 } = req.query as { search?: string; limit?: string };
    ...
    try {
      ...
      const customersWithInsights = await Promise.all(
...
router.get('/intelligence/stats/:companyId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
    const { companyId } = req.params;
    const { periodDays = 30 } = req.query as { periodDays?: string };
...
router.get('/intelligence/patterns/:companyId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
    const { companyId } = req.params;
...
router.get('/recommendations/:companyId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
    const { companyId } = req.params;
    ...
    try {
      ...
      const filteredRecommendations = type 
        ? recommendations.filter(rec => rec.title.toLowerCase().includes(type as string))
        : recommendations;
...
router.get('/vendors/:companyId',
  ...
  asyncHandler(async (req: TenantRequest, res: any) => {
    ...
    try {
      ...
      const vendorsWithInsights = await Promise.all(
        vendors.map(async (vendor) => {
          ...
          if (transactionCount > 10) frequency = 'frequent';
          else if (transactionCount > 3) frequency = 'occasional';
          else if (transactionCount > 0) frequency = 'rare';
...
router.patch('/vendors/:vendorId/toggle-active',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res: any) => {
    const { vendorId } = req.params;
