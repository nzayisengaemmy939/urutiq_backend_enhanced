...
interface AnomalyDetection {
  transactionId: string;
  anomalyType: 'duplicate' | 'unusual_amount' | 'fraud_suspicious' | 'timing_anomaly';
  confidence: number;
  description: string;
...
class EnhancedAIService {
  ...
  async categorizeTransaction(
    ...
    const response = await this.callOllama(
      prompt, 
      promptConfig.transactionCategorization.systemPrompt,
      tenantId,
      companyId
    );
    ...
    if (confidence < behaviorConfig.confidenceThresholds.categorization) {
      ...
      const matchingCategory = allCategories.find(cat => 
        cat.isActive && cat.keywords.some(keyword => 
  ...
  async detectAnomaliesEnhanced(
    ...
    for (const [key, txnGroup] of amountGroups) {
      if (txnGroup.length > 1) {
        ...
        const response = await this.callOllama(
          prompt,
          promptConfig.anomalyDetection.systemPrompt,
          tenantId,
          companyId
        );
        ...
        if (duplicateMatch?.[1]?.toLowerCase() === 'yes') {
          anomalies.push({
            ...
            anomalyType: typeMatch?.[1]?.trim() as any || 'duplicate',
        ...
        if (zScore > 3) {
          ...
          if (suspiciousMatch?.[1]?.toLowerCase() === 'yes') {
            anomalies.push({
              ...
              anomalyType: typeMatch?.[1]?.trim() as any || 'unusual_amount',
  ...
  async generateNaturalLanguageReport(query: NaturalLanguageQuery): Promise<any> {
    ...
    const sqlQuery = await this.callOllama(
      prompt,
      promptConfig.naturalLanguageReports.systemPrompt,
      query.companyId,
      query.companyId
    );
  ...
  async generateSmartInsights(
    ...
    const cashFlowResponse = await this.callOllama(
      cashFlowPrompt,
      promptConfig.smartInsights.systemPrompt,
      tenantId,
      companyId
    );
    ...
    const predictionResponse = await this.callOllama(
      predictionPrompt,
      promptConfig.smartInsights.systemPrompt,
      tenantId,
      companyId
    ...
    if (predictionMatches) {
      for (const match of predictionMatches) {
        ...
        predictions.push({
          ...
          riskLevel: parts[3].replace('Risk:', '').trim() as any,
...
"Response: [your helpful response] | Actions: [comma-separated list of actions] | Confidence: [0-100] | Data: [any specific data or parameters]"`;
    ...
    const response = await this.callOllama(
      prompt, 
      promptConfig.aiAssistant.systemPrompt,
      tenantId,
      companyId
    );
