...
// Auto-Bookkeeper Interfaces
export interface AutoBookkeeperConfig {
  id: string;
  companyId: string;
  isEnabled: boolean;
...
export interface AutoBookkeeperRule {
  id: string;
  name: string;
  description: string;
  condition: string;
...
export interface AutoBookkeeperStats {
...
export interface AutoBookkeeperInsight {
  id: string;
...
export class AutoBookkeeperService {
  ...
  private transactionProcessing: EnhancedTransactionProcessingService;
  private journalManagement: EnhancedJournalManagementService;
  ...
  async initializeAutoBookkeeper(companyId: string): Promise<AutoBookkeeperConfig> {
    try {
  ...
  async getConfig(companyId: string): Promise<AutoBookkeeperConfig> {
    try {
      // TODO: Implement when Prisma client is regenerated
  ...
  async updateConfig(companyId: string, updates: Partial<AutoBookkeeperConfig>): Promise<AutoBookkeeperConfig> {
    try {
      // TODO: Implement when Prisma client is regenerated
  ...
  async reconcileTransaction(
    ...
    try {
      ...
      let bestMatch = null;
      let matchType: 'exact' | 'fuzzy' | 'ai_suggested' | 'manual' = 'manual';
      let confidence = 0;

      // Check for exact matches
      const exactMatches = matchingTransactions.filter(t => 
        Math.abs(Number(t.amount) - Number(bankTransaction.amount)) < 0.01 &&
      ...
      if (exactMatches.length > 0) {
        ...
        ${matchingTransactions.map((t, i) => 
          `${i + 1}. ${t.transactionType} | Amount: ${t.amount} | Date: ${t.transactionDate}`
        ).join('\n')}
  ...
  async getStats(companyId: string, periodDays: number = 30): Promise<AutoBookkeeperStats> {
    try {
  ...
  async getInsights(companyId: string): Promise<AutoBookkeeperInsight[]> {
    try {
      const insights: AutoBookkeeperInsight[] = [];
  ...
  async processPendingTransactions(companyId: string): Promise<{
    categorized: number;
  ...
  // Helper methods
  private mapConfigFromDB(config: any): AutoBookkeeperConfig {
    return {
      id: config.id,
  ...
  private getDefaultRules(): AutoBookkeeperRule[] {
    return [
      {
        id: 'rule-001',
  ...
  private parseCategorizationResponse(message: string): { category: string; confidence: number; reasoning: string } {
  ...
  private parseJournalEntryResponse(message: string): { debitAccount: string; creditAccount: string; amount: number; memo: string; confidence: number } {
    const parts = message.split('|').map(p => p.trim());
    const result = {
  ...
  private parseReconciliationResponse(message: string): { transactionIndex: number; confidence: number; reasoning: string } {
    const parts = message.split('|').map(p => p.trim());
    const result = {
      transactionIndex: -1,
      confidence: 0.5,
