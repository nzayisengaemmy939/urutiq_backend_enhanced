...
export type PeriodStatus = 'open' | 'locked' | 'closing' | 'closed';
...
const periodStatusStorage = new Map<string, { status: PeriodStatus; updatedBy: string; updatedAt: string }>();

// In-memory storage for period runs
const periodRunsStorage = new Map<string, Array<{ id: string; type: string; at: string; payload: any }>>();

// Helper function to get period status from database or memory
async function getPeriodStatus(companyId: string, period: string): Promise<PeriodStatus> {
...
export class PeriodCloseService {
  async getStatus(companyId: string, period: string): Promise<PeriodStatus> {
  ...
  async listPeriods(companyId: string) {
    ...
    const locks: Record<string, PeriodStatus> = {};
    ...
    return periods.map((p) => ({ 
      period: p, 
      status: locks[p] || 'open' as PeriodStatus
    }));
  }

  async lockPeriod(companyId: string, period: string, lockedBy: string) {
    await this.setPeriodStatus(companyId, period, 'locked', lockedBy);
    return { period, status: 'locked' as PeriodStatus };
  }

  async unlockPeriod(companyId: string, period: string, unlockedBy: string) {
    await this.setPeriodStatus(companyId, period, 'open', unlockedBy);
    return { period, status: 'open' as PeriodStatus };
  }

  async completeClose(companyId: string, period: string, completedBy: string) {
    await this.setPeriodStatus(companyId, period, 'closed', completedBy);
    return { period, status: 'closed' as PeriodStatus };
  }

  private async setPeriodStatus(companyId: string, period: string, status: PeriodStatus, userId: string) {
  ...
  async getChecklist(companyId: string, period: string): Promise<CloseChecklistItem[]> {
    try {
      const items = await prisma.closeChecklistItem.findMany({
        where: { companyId, period },
        orderBy: { order: 'asc' },
      });
      return items as unknown as CloseChecklistItem[];
    } catch {
      // Return default checklist items for demo purposes
      const defaultChecklist: CloseChecklistItem[] = [
  ...
  async updateChecklistItem(companyId: string, period: string, itemId: string, changes: Partial<CloseChecklistItem>) {
    try {
      const item = await prisma.closeChecklistItem.update({
        ...
        data: {
          ...
          order: changes.order as any,
        },
      });
      return item as unknown as CloseChecklistItem;
    } catch {
  ...
  async createRecurringJournalTemplate(template: RecurringJournalTemplate) {
    try {
      const created = await prisma.recurringJournalTemplate.create({ data: template as any });
      return created as unknown as RecurringJournalTemplate;
    } catch {
      return template; // fallback
    }
  }

  async runRecurringJournals(companyId: string, period: string) {
    ...
    try {
      ...
      const result = { posted: templates.length, period, at: new Date().toISOString(), type: 'recurring' } as any
      await this.recordRun(companyId, period, 'recurring', result)
      return result;
    } catch {
      const result = { posted: 0, period, at: new Date().toISOString(), type: 'recurring' } as any
      await this.recordRun(companyId, period, 'recurring', result)
      return result;
    }
  }

  async runAllocations(companyId: string, period: string) {
    // Placeholder for allocation logic (e.g., allocate overhead by driver)
    const result = { success: true, allocationsPosted: 0, period, at: new Date().toISOString(), type: 'allocations' } as any
    await this.recordRun(companyId, period, 'allocations', result)
    return result;
  }

  async runFxRevaluation(companyId: string, period: string, baseCurrency: string = 'USD') {
    ...
    const result = { success: true, entriesPosted: posted.postedCount, baseCurrency, period, at: new Date().toISOString(), type: 'fx-reval' } as any
    await this.recordRun(companyId, period, 'fx-reval', result)
    return result;
  }

  async previewFxRevaluation(companyId: string, period: string, baseCurrency: string) {
    // Try to build balances by currency using transactions; fallback to synthetic sample
    let rows: Array<{ currency: string; balance: number }>
    try {
      // Example: sum by currency for the period end (simplified)
      rows = await prisma.$queryRawUnsafe<any[]>(
        `SELECT COALESCE(currency, 'USD') as currency, SUM(amount) as balance
         FROM "Transaction"
         WHERE "companyId" = $1 AND to_char("transactionDate", 'YYYY-MM') <= $2
         GROUP BY currency
         HAVING COALESCE(currency, 'USD') <> $3`,
        companyId, period, baseCurrency
      ) as any
    } catch {
      rows = [
  ...
  async postFxRevaluation(companyId: string, period: string, baseCurrency: string, entries?: any[]) {
    ...
    try {
      ...
      if (existing) await prisma.setting.update({ where: { id: existing.id }, data: { value: record as any } })
      else await prisma.setting.create({ data: { companyId, key, value: record as any } })
    } catch {
  ...
  async postFxRevaluationJournal(tenantId: string, companyId: string, period: string, baseCurrency: string, entries: any[], accounts: { fxGainAccountId: string, fxLossAccountId: string, revaluedAccountId: string }) {
    ...
    if (lines.length === 0) return { created: null }
  ...
  async getFxRevaluationHistory(companyId: string, period: string) {
    const key = `fx_reval:${period}`
    try {
      const existing = await prisma.setting.findFirst({ where: { companyId, key } })
  ...
  private async getFxRate(companyId: string, from: string, to: string) {
    // Try settings override first
    const key = `fx_rate:${from}->${to}`
    try {
      const s = await prisma.setting.findFirst({ where: { companyId, key } })
      if (s?.value?.rate) return Number(s.value.rate)
    } catch { /* ignore */ }
    // Demo fixed rates
    const demo: Record<string, number> = { 'EUR->USD': 1.08, 'GBP->USD': 1.26 }
    return demo[`${from}->${to}`] || 1
  }
  
  async recordRun(companyId: string, period: string, type: 'recurring'|'allocations'|'fx-reval'|'adjustment', payload: any) {
    const key = `${companyId}:${period}`;
  ...
  async postPriorPeriodAdjustment(
    tenantId: string,
    companyId: string,
    closedPeriod: string,
    payload: { amount: number; description?: string; transactionType?: 'expense' | 'income'; currency?: string }
  ) {
  ...
  async listRuns(companyId: string, period: string, type?: string) {
    const key = `${companyId}:${period}`;
  ...
  async rollbackRun(companyId: string, period: string, runId: string) {
