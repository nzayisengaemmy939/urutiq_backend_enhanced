...
// Enhanced Bank Integration Interfaces
...
export interface BankReconciliationMatch {
  ...
  confidence: number;
  matchType: 'exact' | 'fuzzy' | 'ai_suggested' | 'manual';
  reasoning: string;
...
export interface BankProviderConfig {
  provider: 'plaid' | 'yodlee' | 'truelayer' | 'manual' | 'api';
  credentials: any;
  endpoints: {
    transactions: string;
    accounts: string;
...
export interface FraudDetectionRule {
  ...
  actions: ('flag' | 'block' | 'notify' | 'require_approval')[];
  isActive: boolean;
}

export interface BankIntegrationStats {
...
export class EnhancedBankIntegrationService {
  ...
  private financialReporting: EnhancedFinancialReportingEngine;
  ...
  private async matchTransaction(
    ...
    const allMatches = [
      ...exactMatches.map(m => ({ ...m, matchType: 'exact' as const })),
      ...fuzzyMatches.map(m => ({ ...m, matchType: 'fuzzy' as const })),
      ...aiSuggestions.map(m => ({ ...m, matchType: 'ai_suggested' as const }))
    ];
  ...
  private async findExactMatches(bankTransaction: any, companyId: string) {
  ...
  private async findFuzzyMatches(bankTransaction: any, companyId: string) {
    const matches = await prisma.transaction.findMany({
      where: {
        companyId,
        amount: {
  ...
  private async getAISuggestions(bankTransaction: any, companyId: string) {
    try {
      const context = {
        userId: 'demo-user-id',
        companyId,
  ...
  private parseAISuggestions(message: string) {
    const suggestions = [];
  ...
  private async autoCategorizeTransaction(bankTransaction: any): Promise<string | null> {
    try {
      const context = {
        userId: 'demo-user-id',
  ...
  // Fraud detection
  private async detectFraud(bankTransaction: any, companyId: string): Promise<number> {
  ...
  private calculateAmountAnomaly(amount: number, historicalTransactions: any[]): number {
  ...
  private calculateTimeAnomaly(transactionDate: Date, historicalTransactions: any[]): number {
    if (historicalTransactions.length === 0) return 0;
  ...
  private calculateMerchantAnomaly(merchantName: string, historicalTransactions: any[]): number {
    if (!merchantName || historicalTransactions.length === 0) return 0;
  ...
  private calculatePatternAnomaly(transaction: any, historicalTransactions: any[]): number {
    if (historicalTransactions.length === 0) return 0;

    // Check for unusual patterns
    const similarTransactions = historicalTransactions.filter(t => 
      Math.abs(Math.abs(Number(t.amount)) - Math.abs(transaction.amount)) < 0.01 &&
  ...
  private async calculateRiskScore(bankTransaction: any): Promise<number> {
  ...
  private calculateRiskLevel(averageConfidence: number, fraudAlerts: number): 'low' | 'medium' | 'high' {
    if (averageConfidence > 0.8 && fraudAlerts === 0) return 'low';
    if (averageConfidence > 0.6 && fraudAlerts <= 2) return 'medium';
    return 'high';
  }

  // Create or update bank transaction
  private async createOrUpdateBankTransaction(connectionId: string, transaction: BankFeedTransaction) {
  ...
  private async autoReconcileTransaction(bankTransactionId: string, match: BankReconciliationMatch) {
    await prisma.bankTransaction.update({
  ...
  private async generateJournalEntry(bankTransactionId: string, match: BankReconciliationMatch) {
    const bankTransaction = await prisma.bankTransaction.findUnique({
      where: { id: bankTransactionId },
      include: { connection: true }
    });

    if (!bankTransaction || !bankTransaction.connection) return;
  ...
  // Create fraud alert
  private async createFraudAlert(bankTransactionId: string, fraudScore: number, reasoning: string) {
  ...
  async getBankIntegrationStats(companyId: string): Promise<BankIntegrationStats> {
  ...
  async testBankConnection(connectionId: string): Promise<{ success: boolean; message: string; details?: any }> {
    try {
      const connection = await prisma.bankConnection.findUnique({
        where: { id: connectionId }
      });
