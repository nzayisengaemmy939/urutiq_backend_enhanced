...
export class MFAService {
   ...
   */
  static async setupMFA(userId: string, tenantId: string): Promise<MFASetupResult> {
  ...
  static async verifyMFA(
    ...
    tenantId: string,
    code: string,
    methodType: 'totp' | 'sms' | 'email' | 'backup_code' = 'totp'
   ...
   */
  static async getUserMFAMethods(userId: string, tenantId: string): Promise<MFAMethod[]> {
  ...
  static async disableMFAMethod(
    ...
    if (activeMethodsCount <= 1) {
      throw new ApiError(400, 'CANNOT_DISABLE_LAST_METHOD', 
        'Cannot disable the last active MFA method');
    }
  ...
  private static async verifySMSEmailCode(mfaMethodId: string, code: string): Promise<boolean> {
  ...
  private static async verifyBackupCode(mfaMethodId: string, code: string): Promise<boolean> {
    // Note: backupCodes field not available in current schema
    // This functionality would need to be implemented differently
    return false;
  }

  private static async getRemainingBackupCodesCount(mfaMethodId: string): Promise<number> {
    // Note: backupCodes field not available in current schema
    return 0;
  }
}
