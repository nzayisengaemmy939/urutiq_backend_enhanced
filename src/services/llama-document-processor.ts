...
export interface DocumentInsight {
  type: 'anomaly' | 'pattern' | 'compliance' | 'optimization';
  description: string;
  confidence: number;
...
export class LlamaDocumentProcessor {
  ...
  private visionModel: string = 'llava:7b'; // Vision model for image processing
  ...
  private async extractTextFromImage(imageUrl: string): Promise<string> {
    try {
      ...
      let imageForOllama: any = imageUrl;
      let bufferForHF: any = imageUrl as any;
  ...
  private buildReceiptAnalysisPrompt(extractedText: string, context: LlamaConversationContext): string {
...
Return ONLY a JSON object with exactly these fields. Example:
{
  ...
  private buildInvoiceAnalysisPrompt(documentContent: string, context: LlamaConversationContext): string {
...
Provide a detailed analysis with structured data extraction.`;
  ...
  private buildContractAnalysisPrompt(documentContent: string, context: LlamaConversationContext): string {
...
Provide a comprehensive analysis focusing on financial and compliance aspects.`;
  ...
  private parseReceiptData(analysisText: string, context: LlamaConversationContext, fallbackText?: string): ExtractedData {
    try {
  ...
  private parseInvoiceData(analysisText: string, context: LlamaConversationContext): ExtractedData {
    // Similar parsing logic for invoices
    return this.parseTextReceiptData(analysisText, context);
  }

  private parseContractData(analysisText: string, context: LlamaConversationContext): ExtractedData {
    // Similar parsing logic for contracts
    return this.parseTextReceiptData(analysisText, context);
  }

  private parseTextReceiptData(text: string, context: LlamaConversationContext): ExtractedData {
    ...
    if (email && email[1]) {
      ...
      if (domain.length >= 3) vendor = domain.charAt(0).toUpperCase() + domain.slice(1);
    }
    ...
    if (vendor === 'Unknown Vendor') {
      ...
      if (companyLine) vendor = companyLine.replace(/\s{2,}/g, ' ').trim();
    }
    ...
    for (const src of [issueLine, dueLine, safe]) {
      ...
      for (const rx of dateRxes) {
        const m = (typeof rx === 'object' ? src.match(rx as any) : null) as any;
        if (m) { dateStr = Array.isArray(m) ? m[0] : (m as any)[0]; break; }
      }
      if (dateStr) break;
    }
    ...
    if (taxLine) {
      ...
      if (rate) taxRate = parseFloat(rate[1]) / 100;
      const ta = taxLine.match(/\$\s*([\d,.]+)/) || taxLine.match(/\b([\d,.]+)\b/);
      if (ta) taxAmount = parseFloat(ta[1].replace(/[,]/g, ''));
    }
  ...
  private async generateContractInsights(
    extractedData: ExtractedData, 
    context: LlamaConversationContext
  ): Promise<DocumentInsight[]> {
    const insights: DocumentInsight[] = [];
  ...
  private getExpectedTaxRate(context: LlamaConversationContext): number {
