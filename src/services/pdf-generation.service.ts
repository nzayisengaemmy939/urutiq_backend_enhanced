...
export class PDFGenerationService {
  private async generateHTML(entryData: JournalEntryPDFData, options: {
  ...
  async generatePDF(entryId: string, options: {
    includeAuditTrail?: boolean;
    includeCompanyHeader?: boolean;
    format?: 'detailed' | 'summary';
  } = {}, retryCount: number = 0): Promise<Buffer> {
    const maxRetries = 2;
    ...
    try {
      ...
      // Prepare data
      const entryData: JournalEntryPDFData = {
        entry: {
          id: entry.id,
        ...
        auditTrail: options.includeAuditTrail ? auditTrail.map((audit: any) => ({
          action: audit.action,
  ...
  async generateMultiplePDFs(entryIds: string[], options: {
    includeAuditTrail?: boolean;
    includeCompanyHeader?: boolean;
    format?: 'detailed' | 'summary';
  } = {}): Promise<Buffer> {
