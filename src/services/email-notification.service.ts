...
function getTransporter() {
  if (!transporter) {
    ...
    const createTransport = (nodemailer as any).default?.createTransport || (nodemailer as any).createTransport || nodemailer;
    if (typeof createTransport === 'function') {
      transporter = createTransport(emailConfig);
    } else if (typeof (nodemailer as any).createTransport === 'function') {
      transporter = (nodemailer as any).createTransport(emailConfig);
    } else {
...
export interface EmailNotificationData {
  to: string | string[];
  subject: string;
  template: 'approval-request' | 'approval-approved' | 'approval-rejected' | 'entry-posted' | 'entry-reversed' | 'entry-created';
  data: {
    entryId: string;
...
export class EmailNotificationService {
  private async sendEmail(to: string | string[], subject: string, html: string, text?: string) {
    try {
  ...
  private generateEmailTemplate(template: string, data: EmailNotificationData['data']): string {
  ...
  async sendNotification(notificationData: EmailNotificationData): Promise<{ success: boolean; messageId?: string; error?: string }> {
    try {
  ...
  async sendApprovalRequest(entryId: string, approverEmails: string[]): Promise<{ success: boolean; messageId?: string; error?: string }> {
    try {
      // Get entry details
      ...
      const totalAmount = (entry.lines || []).reduce((sum: number, line: any) => 
        sum + Math.max(Number(line.debit) || 0, Number(line.credit) || 0), 0
      );
  ...
  async sendApprovalResponse(entryId: string, approved: boolean, approverId: string, reason?: string): Promise<{ success: boolean; messageId?: string; error?: string }> {
    try {
      // Get entry details
      const entry = await prisma.journalEntry.findUnique({
        where: { id: entryId },
  ...
  async sendStatusChange(entryId: string, status: 'POSTED' | 'REVERSED', reason?: string): Promise<{ success: boolean; messageId?: string; error?: string }> {
    try {
      // Get entry details
      const entry = await prisma.journalEntry.findUnique({
        where: { id: entryId },
  ...
  async sendEntryCreated(entryId: string): Promise<{ success: boolean; messageId?: string; error?: string }> {
    try {
      // Get entry details
      const entry = await prisma.journalEntry.findUnique({
        where: { id: entryId },
