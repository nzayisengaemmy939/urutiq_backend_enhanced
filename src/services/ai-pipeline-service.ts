...
export interface CreateDriftDetectionRequest {
  tenantId: string;
  companyId: string;
  modelId?: string;
...
export class AIPipelineService {
   ...
   */
  async getPipelines(tenantId: string, companyId: string, filters?: {
    status?: string;
    pipelineType?: string;
  }) {
    try {
   ...
   */
  async getPipelineById(pipelineId: string, tenantId: string, companyId: string) {
    try {
   ...
   */
  async updatePipeline(pipelineId: string, data: {
   ...
   */
  async createPipelineRun(data: CreatePipelineRunRequest) {
    try {
   ...
   * Update pipeline run
   */
  async updatePipelineRun(runId: string, data: UpdatePipelineRunRequest) {
    try {
   ...
   * Get pipeline runs
   */
  async getPipelineRuns(pipelineId: string, tenantId: string, companyId: string, limit = 50) {
    try {
   ...
   */
  async createDataQuality(data: CreateDataQualityRequest) {
    try {
   ...
   */
  async getDataQuality(tenantId: string, companyId: string, filters?: {
    tableName?: string;
    ...
    try {
      ...
      if (filters?.qualityMetric) where.qualityMetric = filters.qualityMetric;
      if (filters?.status) where.status = filters.status;
   ...
   */
  async createDriftDetection(data: CreateDriftDetectionRequest) {
    try {
   ...
   */
  async getDriftDetection(tenantId: string, companyId: string, filters?: {
    modelId?: string;
    driftType?: string;
    status?: string;
    days?: number;
    ...
    try {
      ...
      if (filters?.driftType) where.driftType = filters.driftType;
      if (filters?.status) where.status = filters.status;
   ...
   */
  async getPipelineStatistics(tenantId: string, companyId: string) {
    try {
      const [
        totalPipelines,
        activePipelines,
   ...
   */
  async deletePipeline(pipelineId: string, tenantId: string, companyId: string) {
    try {
