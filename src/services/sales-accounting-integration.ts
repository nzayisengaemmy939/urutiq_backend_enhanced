...
export interface SalesAccountingIntegration {
  ...
  processInvoicePayment(invoiceId: string, tenantId: string): Promise<{
  ...
  createSalesAccountingEntries(invoice: any, tenantId: string): Promise<string>;
  
  // Update inventory for sold products
  updateInventoryForSales(invoice: any, tenantId: string): Promise<string[]>;
}
...
export class SalesAccountingIntegrationService implements SalesAccountingIntegration {
   ...
   */
  async testJournalEntryLogic(): Promise<void> {
   ...
   */
  private async validateInventoryAvailability(invoice: any, tenantId: string): Promise<{
    valid: boolean;
    errors: string[];
    warnings: string[];
  }> {
      ...
      for (const line of invoice.lines) {
        ...
        if (product) {
          ...
          if (currentStock < quantity) {
            ...
            warnings.push(
              `Stock is exactly sufficient for "${productName}". ` +
              `Consider reordering soon.`
            );
          } else if (currentStock < quantity * 1.5) {
            warnings.push(
              `Low stock warning for "${productName}". ` +
              `Available: ${currentStock}, Required: ${quantity}`
            );
          }
        }
   ...
   */
  async processInvoicePayment(invoiceId: string, tenantId: string): Promise<{
    journalEntryId: string;
    inventoryMovements: string[];
    success: boolean;
  }> {
   ...
   */
  private async getOrCreateSalesAccounts(companyId: string, tenantId: string, db?: any): Promise<{
    cash: any;
