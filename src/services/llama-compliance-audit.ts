...
export interface ComplianceViolation {
  type: 'tax' | 'financial' | 'regulatory' | 'internal' | 'data_protection';
...
export interface ComplianceRecommendation {
  category: 'process' | 'documentation' | 'training' | 'system' | 'monitoring';
...
export class LlamaComplianceAudit {
  ...
  private async gatherComplianceData(companyId: string): Promise<any> {
    try {
  ...
  private async gatherPeriodComplianceData(
    companyId: string, 
    period: { start: Date; end: Date }
  ): Promise<any> {
    try {
  ...
  private buildComplianceAnalysisPrompt(data: any, context: LlamaConversationContext): string {
...
Provide specific violations, recommendations, and risk assessment with confidence levels.`;
  ...
  private buildTransactionAuditPrompt(transaction: any, context: LlamaConversationContext): string {
...
Provide specific violations and recommendations.`;
  ...
  private buildComplianceReportPrompt(
    data: any, 
    period: { start: Date; end: Date }, 
    context: LlamaConversationContext
  ): string {
...
Format as structured analysis with specific findings and actionable recommendations.`;
  ...
  private parseComplianceViolations(analysisText: string, context: LlamaConversationContext): ComplianceViolation[] {
  ...
  private parseTransactionViolations(
    analysisText: string, 
    transaction: any, 
    context: LlamaConversationContext
  ): ComplianceViolation[] {
  ...
  private parseComplianceRecommendations(analysisText: string, context: LlamaConversationContext): ComplianceRecommendation[] {
  ...
  private parseTransactionRecommendations(
    analysisText: string, 
    transaction: any, 
    context: LlamaConversationContext
  ...
  private async assessComplianceRisk(
    ...
    const riskFactors = violations.map(violation => ({
      ...
      level: violation.severity === 'critical' ? 'critical' as const : 
             violation.severity === 'high' ? 'high' as const :
             violation.severity === 'medium' ? 'medium' as const : 'low' as const,
    ...
    const riskScore = riskFactors.reduce((score, factor) => 
      score + (factor.probability * factor.impact), 0
    ) / Math.max(riskFactors.length, 1);

    const overallRisk = riskScore > 0.7 ? 'critical' as const :
                       riskScore > 0.5 ? 'high' as const :
                       riskScore > 0.3 ? 'medium' as const : 'low' as const;
  ...
  private async analyzeAuditTrail(companyId: string, context: LlamaConversationContext): Promise<AuditTrailEntry[]> {
    try {
      const auditLogs = await prisma.auditLog.findMany({
        where: { companyId },
        orderBy: { createdAt: 'desc' },
  ...
  private async analyzePeriodAuditTrail(
    companyId: string, 
    period: { start: Date; end: Date }, 
    context: LlamaConversationContext
  ...
  private assessAuditEntryRisk(log: any): 'low' | 'medium' | 'high' {
  ...
  private detectAuditAnomalies(log: any): string[] {
  ...
  private async getRegulatoryUpdates(context: LlamaConversationContext): Promise<RegulatoryUpdate[]> {
  ...
  private getDefaultRiskAssessment(): ComplianceRiskAssessment {
    return {
      overallRisk: 'medium',
      riskFactors: [],
      riskScore: 0.5,
