...
export interface ApprovalWorkflowConfig {
  ...
  name: string;
  description?: string;
  entityType: 'journal_entry' | 'invoice' | 'purchase_order' | 'expense' | 'bill' | 'document' | 'recurring_invoice';
  entitySubType?: string; // e.g., 'adjustment', 'reversal', 'high_value'
  isActive: boolean;
  steps: ApprovalStep[];
  conditions: ApprovalCondition[];
...
export class UnifiedApprovalEngine {
   ...
   */
  static async createWorkflow(config: Omit<ApprovalWorkflowConfig, 'id'>): Promise<ApprovalWorkflowConfig> {
  ...
  static async getWorkflows(
    ...
    return workflows.map(w => ({
      ...
      entityType: w.entityType as any,
  ...
  static async createApprovalRequest(
    ...
    const approvalAssignees = await Promise.all(
    ...
    return {
      ...
      status: approvalRequest.status as any,
  ...
  private static async resolveApprovers(
    ...
    switch (step.approverType) {
      case 'user':
        if (!step.approverId) throw new Error('User ID required for user approver type');
      ...
      case 'role':
        if (!step.role) throw new Error('Role required for role approver type');
   ...
   */
  private static async moveToNextStep(approvalRequestId: string, nextStep: number): Promise<void> {
   ...
   */
  private static async sendApprovalActionNotifications(
    approvalRequestId: string,
    action: string,
    comments?: string
  ): Promise<void> {
   ...
   */
  private static async getApprovalRequest(
    tenantId: string,
    approvalRequestId: string
  ): Promise<ApprovalRequest> {
