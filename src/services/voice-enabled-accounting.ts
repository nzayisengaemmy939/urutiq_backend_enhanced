...
// Types for Voice-Enabled Accounting
...
export class VoiceEnabledAccountingService {
  ...
  async initializeVoiceSettings(companyId: string, userId: string): Promise<VoiceSettings> {
    const defaultSettings: Omit<VoiceSettings, 'id'> = {
      companyId,
      userId,
      isEnabled: true,
      language: 'en-US',
  ...
  async getVoiceSettings(companyId: string, userId: string): Promise<VoiceSettings> {
  ...
  async startVoiceSession(companyId: string, userId: string, language?: string): Promise<VoiceSession> {
  ...
  async processVoiceCommand(
    ...
    try {
      ...
      await this.storeVoiceCommand({
        ...
        userId,
        transcription,
        commandType: parsedCommand.action as any,
  ...
  // Get voice analytics
  async getVoiceAnalytics(companyId: string, userId: string, periodDays: number = 30): Promise<VoiceAnalytics> {
  ...
  private async parseVoiceCommand(transcription: string, companyId: string): Promise<{
    action: string;
    parameters: Record<string, any>;
    confidence: number;
  }> {
  ...
  private async executeVoiceCommand(
    parsedCommand: { action: string; parameters: Record<string, any>; confidence: number },
    companyId: string,
    userId: string
  ...
  private async handleCategorizeTransaction(
    parameters: Record<string, any>,
    companyId: string
  ): Promise<{ response: string }> {
    try {
  ...
  private async handleReconcileAccount(
    parameters: Record<string, any>,
    companyId: string
  ): Promise<{ response: string }> {
    try {
  ...
  private async handleHelp(): Promise<{ response: string }> {
    return {
  ...
  private async storeVoiceCommand(data: {
    companyId: string;
    userId: string;
    transcription: string;
    commandType: string;
  ...
  private extractActionFromResponse(response: any): string {
    // Extract action from AI response
    if (response.action) return response.action;
    if (response.intent) return response.intent;
    return 'unknown';
  }

  private extractParametersFromResponse(response: any): Record<string, any> {
    // Extract parameters from AI response
    return response.parameters || response.entities || {};
  }

  private getDefaultCommands(): VoiceCommandConfig[] {
    return [
      {
        id: 'add_transaction',
  ...
  private mapVoiceSettingsFromDB(dbSettings: any): VoiceSettings {
    return {
      id: dbSettings.id,
  ...
  private mapVoiceCommandConfigFromDB(dbCommand: any): VoiceCommandConfig {
    return {
      id: dbCommand.id,
      name: dbCommand.name,
  ...
  private mapVoiceSessionFromDB(dbSession: any): VoiceSession {
    return {
      id: dbSession.id,
  ...
  private mapVoiceCommandFromDB(dbCommand: any): VoiceCommand {
    return {
      id: dbCommand.id,
