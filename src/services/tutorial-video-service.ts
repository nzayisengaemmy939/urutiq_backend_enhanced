...
export interface TutorialVideoMetadata {
  id: string;
  title: string;
  description?: string;
  category: string;
...
class TutorialVideoService {
  ...
  private generateDownloadUrl(videoId: string, tenantId: string): string {
  ...
  async uploadTutorialVideo(
    videoBuffer: Buffer,
    videoData: TutorialVideoData
  ): Promise<TutorialVideoMetadata> {
    try {
  ...
  async streamTutorialVideo(videoId: string, tenantId: string): Promise<{ stream: any; metadata: TutorialVideoMetadata }> {
    try {
      if (!mongoService.isConnected()) {
        throw new Error('MongoDB not connected');
      }
      ...
      const metadata: TutorialVideoMetadata = {
        id: videoRecord.id,
        title: videoRecord.title,
        description: videoRecord.description || undefined,
        category: videoRecord.category,
  ...
  async getTutorialVideoMetadata(videoId: string, tenantId: string): Promise<TutorialVideoMetadata | null> {
    try {
      const videoRecord = await prisma.tutorialVideo.findFirst({
        where: {
          id: videoId,
  ...
  async listTutorialVideos(tenantId: string, filters?: {
    category?: string;
    difficulty?: string;
    isPublished?: boolean;
    createdBy?: string;
  ...
  async recordVideoView(videoId: string, tenantId: string, userId: string, userName: string, duration?: number): Promise<void> {
    try {
      // Record the view
  ...
  async rateVideo(videoId: string, tenantId: string, userId: string, userName: string, rating: number, comment?: string): Promise<void> {
    try {
  ...
  async deleteTutorialVideo(videoId: string, tenantId: string): Promise<boolean> {
    try {
      // Get video metadata
      const videoRecord = await prisma.tutorialVideo.findFirst({
        where: {
  ...
  private async deleteVideoFromMongoDB(videoFileId: string): Promise<void> {
    try {
      if (!mongoService.isConnected()) {
        return;
      }
   ...
   */
  async streamVideo(videoId: string, res: any, req?: any): Promise<void> {
    try {
