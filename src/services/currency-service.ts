...
export class CurrencyService {
  ...
  private static rateCache = new Map<string, { rate: number; timestamp: number }>()
   ...
   */
  static async clearDatabaseRates(fromCurrency: string, toCurrency: string): Promise<void> {
   ...
   */
  static async forceRefreshHistoricalData(
    fromCurrency: string, 
    toCurrency: string, 
    startDate: string, 
    endDate: string
   ...
   */
  static async forceRefreshRate(fromCurrency: string, toCurrency: string): Promise<CurrencyRate> {
   ...
   */
  static async testExchangeRateAPI(baseCurrency: string = 'USD'): Promise<any> {
    try {
   ...
   */
  static getSupportedCurrencies(): Record<string, { symbol: string; decimals: number; name: string }> {
   ...
   */
  static getCurrencyInfo(currency: string): { symbol: string; decimals: number; name: string } | null {
    return this.SUPPORTED_CURRENCIES[currency as keyof typeof this.SUPPORTED_CURRENCIES] || null
  }
  ...
  private static async fetchExternalRate(fromCurrency: string, toCurrency: string): Promise<number> {
    ...
    const apis = [
      ...
      },
      {
        ...
        parser: (data: any) => data.quotes?.[`${fromCurrency}${toCurrency}`]
      }
    ]
   ...
   */
  static async getHistoricalRatesForChart(
    fromCurrency: string,
    toCurrency: string,
    startDate: Date,
    endDate: Date
   ...
   */
  private static async generateSampleHistoricalData(
    fromCurrency: string,
    toCurrency: string,
    startDate: Date,
    endDate: Date
   ...
   */
  private static async getBaseExchangeRate(fromCurrency: string, toCurrency: string): Promise<number> {
    try {
   ...
   */
  private static getMockRate(fromCurrency: string, toCurrency: string): number | null {
   ...
   */
  static parseCurrency(formattedAmount: string, currency: string): number {
    const currencyInfo = this.getCurrencyInfo(currency)
    if (!currencyInfo) {
   ...
   */
  static async getConversionSummary(
    amount: number,
    fromCurrency: string,
    toCurrency: string
  ): Promise<{
