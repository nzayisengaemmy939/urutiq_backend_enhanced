...
export function mountSupplierPortalRoutes(router: Router) {
  ...
  router.get('/profile/:supplierId', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
  ...
  router.put('/profile/:supplierId', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
    const { supplierId } = req.params;
  ...
  router.get('/stats/:supplierId', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
    const { supplierId } = req.params;
        ...
        ? paidBills.reduce((sum, bill) => {
            ...
            if (payment) {
              const daysDiff = Math.ceil(
                (new Date(payment.paymentDate).getTime() - new Date(bill.createdAt).getTime()) / (1000 * 60 * 60 * 24)
              );
              return sum + daysDiff;
            }
            return sum;
  ...
  router.get('/invoices/:supplierId', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
    const { supplierId } = req.params;
  ...
  router.get('/payments/:supplierId', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
    const { supplierId } = req.params;
    const { status, dateFrom, dateTo, page = 1, pageSize = 20 } = req.query;
    ...
    try {
      ...
      if (dateFrom || dateTo) {
        ...
        if (dateFrom) whereClause.paymentDate.gte = new Date(dateFrom as string);
        if (dateTo) whereClause.paymentDate.lte = new Date(dateTo as string);
      }
  ...
  router.get('/invoices/:supplierId/:invoiceId/pdf', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
  ...
  // Get invoice details
  router.get('/invoices/:supplierId/:invoiceId', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
    const { supplierId, invoiceId } = req.params;
  ...
  // Get payment details
  router.get('/payments/:supplierId/:paymentId', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
  ...
  router.put('/settings/:supplierId', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
    const { supplierId } = req.params;
  ...
  router.get('/notifications/:supplierId', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
    const { supplierId } = req.params;
  ...
  router.put('/notifications/:supplierId/:notificationId/read', requireRoles(['admin', 'accountant', 'supplier']), async (req: TenantRequest, res) => {
