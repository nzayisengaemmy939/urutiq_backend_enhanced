...
export function mountWorkflowRoutes(router: Router) {
  // Create workflow
  router.post('/workflows', async (req: TenantRequest, res) => {
    try {
      ...
      const firstDocument = await prisma.fileAsset.findFirst({
        where: { 
          ...
          companyId: req.query.companyId as string || 'seed-company-1'
        }
      });
  ...
  // List workflows
  router.get('/workflows', async (req: TenantRequest, res) => {
  ...
  // Workflow Statistics
  router.get('/workflows/stats', async (req: TenantRequest, res) => {
    try {
      const companyId = String(req.query.companyId || '');
      ...
      if (completedWorkflows.length > 0) {
        const validWorkflows = completedWorkflows.filter(workflow => 
          workflow.completedAt && workflow.assignedAt
        );
  ...
  // Workflow Templates
  router.get('/workflows/templates', async (req: TenantRequest, res) => {
    try {
