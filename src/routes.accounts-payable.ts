...
import { updateAccountBalances } from './utils/account-balance.js';
 ...
 * @access Private
 */
export function mountAccountsPayableRoutes(router: Router) {
  router.get('/invoices', requireRoles(['admin', 'accountant', 'manager']), async (req: TenantRequest, res: Response) => {
    try {
      const {
        page = '1',
        pageSize = '20',
      ...
      const where: any = {
        ...
        ...(vendorId && { vendorId }),
        ...(source && { source }),
        ...(dateFrom && { invoiceDate: { gte: new Date(dateFrom as string) } }),
        ...(dateTo && { invoiceDate: { lte: new Date(dateTo as string) } }),
        ...(q && {
          OR: [
            { invoiceNumber: { contains: q as string } },
            { vendor: { name: { contains: q as string } } }
          ]
        })
      };
   ...
   * @access Private
   */
  router.post('/invoices', requireRoles(['admin', 'accountant']), validateRequestMiddleware({ body: invoiceCaptureSchema }), async (req: TenantRequest, res: Response) => {
    try {
      const data = req.body;
   ...
   * @access Private
   */
  router.put('/invoices/:id', requireRoles(['admin', 'accountant']), validateRequestMiddleware({ body: invoiceCaptureSchema }), async (req: TenantRequest, res: Response) => {
    try {
      const { id } = req.params;
      const data = req.body;
      const tenantId = req.tenantId!;
   ...
   * @access Private
   */
  router.delete('/invoices/:id', requireRoles(['admin', 'accountant']), async (req: TenantRequest, res: Response) => {
    try {
      const { id } = req.params;
      const tenantId = req.tenantId!;
      const companyId = req.header('x-company-id') || 'demo-company';
   ...
   * @access Private
   */
  router.post('/invoices/:id/match', requireRoles(['admin', 'accountant']), validateRequestMiddleware({ body: invoiceMatchingSchema }), async (req: TenantRequest, res: Response) => {
    try {
      const { id } = req.params;
      const data = req.body;
      const tenantId = req.tenantId!;
   ...
   * @access Private
   */
  router.post('/invoices/:id/approve', requireRoles(['admin', 'accountant', 'manager']), validateRequestMiddleware({ body: invoiceApprovalSchema }), async (req: TenantRequest, res: Response) => {
    try {
      const { id } = req.params;
   ...
   * @access Private
   */
  router.post('/payment-schedule', requireRoles(['admin', 'accountant']), validateRequestMiddleware({ body: paymentScheduleSchema }), async (req: TenantRequest, res: Response) => {
    try {
      const data = req.body;
      const tenantId = req.tenantId!;
      const companyId = req.header('x-company-id') || 'demo-company';
   ...
   * @access Private
   */
  router.get('/aging-report', requireRoles(['admin', 'accountant', 'manager']), async (req: TenantRequest, res: Response) => {
    try {
      ...
      const bills = await prisma.bill.findMany({
        where: {
          tenantId,
          companyId: companyId as string || req.header('x-company-id') || 'demo-company',
      ...
      const agingReport = bills.map(bill => {
        ...
        if (daysPastDue > 90) agingBucket = 'over_90_days';
        else if (daysPastDue > 60) agingBucket = '61_90_days';
        else if (daysPastDue > 30) agingBucket = '31_60_days';
        else if (daysPastDue > 0) agingBucket = '1_30_days';
   ...
   * @access Private
   */
  router.post('/reconcile', requireRoles(['admin', 'accountant']), validateRequestMiddleware({ body: reconciliationSchema }), async (req: TenantRequest, res: Response) => {
    try {
      const data = req.body;
      const tenantId = req.tenantId!;
      const companyId = req.header('x-company-id') || 'demo-company';
   ...
   * @access Private
   */
  router.get('/dashboard', requireRoles(['admin', 'accountant', 'manager']), async (req: TenantRequest, res: Response) => {
    try {
      const tenantId = req.tenantId!;
      const companyId = req.header('x-company-id') || 'demo-company';
   ...
   * @access Private
   */
  router.post('/payments/process', requireRoles(['admin', 'accountant']), async (req: TenantRequest, res: Response) => {
    try {
      const data = req.body;
      const tenantId = req.tenantId!;
      const companyId = req.header('x-company-id') || 'demo-company';
