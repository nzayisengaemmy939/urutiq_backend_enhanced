...
export function mountMappingRoutes(router: Router) {
  ...
  router.post('/account-mappings', validateBody(mappingSchemas.mappingUpsert), async (req: TenantRequest, res) => {
    const { companyId, purpose, accountId } = req.body as any;
  ...
  router.get('/account-mappings/checklist', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
