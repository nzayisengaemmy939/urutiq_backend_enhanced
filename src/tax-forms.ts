...
// Interface definitions for tax models (to work without generated types)
...
export class TaxFormService {
  static async create(tenantId: string, companyId: string, data: z.infer<typeof CreateTaxFormSchema>) {
    const validatedData = CreateTaxFormSchema.parse(data);
    
    // Get form template
    const template = TaxFormTemplates[validatedData.formCode as keyof typeof TaxFormTemplates];
    if (!template) {
    ...
    const createdForms = await prisma.$queryRaw<(TaxForm & { jurisdiction: TaxJurisdiction })[]>`
  ...
  static async getAll(
    ...
    const forms = await prisma.$queryRaw<TaxForm[]>`
      SELECT tf.*, 
  ...
  static async getById(id: string, tenantId: string, companyId: string) {
    const forms = await prisma.$queryRaw<TaxForm[]>`
      SELECT tf.*, 
             tj.name as jurisdiction_name,
             tj.code as jurisdiction_code,
  ...
  static async update(
    ...
    tenantId: string,
    companyId: string,
    data: z.infer<typeof UpdateTaxFormSchema>
  ) {
  ...
  static async generateForm(
    ...
    const template = TaxFormTemplates[formCode as keyof typeof TaxFormTemplates];
    if (!template) {
  ...
  private static async getRevenueData(tenantId: string, companyId: string, startDate: Date, endDate: Date) {
    ...
    const total = revenueAccounts.reduce((sum, account) => {
      const accountTotal = account.journalLines.reduce((lineSum: number, line) => {
  ...
  private static async getExpenseData(tenantId: string, companyId: string, startDate: Date, endDate: Date) {
  ...
  private static async getAssetData(tenantId: string, companyId: string, asOfDate: Date) {
  ...
  private static async getLiabilityData(tenantId: string, companyId: string, asOfDate: Date) {
  ...
  private static async generateFormData(formCode: string, data: any) {
  ...
  private static async calculateCorporateTax(income: number, country: string) {
    ...
    const rate = taxRates[country as keyof typeof taxRates] || 0.21;
    return Math.max(0, income * rate);
  }

  private static async calculateVATData(companyId: string, taxYear: number) {
  ...
  static async submitForm(id: string, tenantId: string, companyId: string) {
    const forms = await prisma.$queryRaw<TaxForm[]>`
      SELECT * FROM TaxForm 
      WHERE id = ${id} AND tenantId = ${tenantId} AND companyId = ${companyId}
      LIMIT 1
    ...
    // Validate form data
    const template = TaxFormTemplates[form.formCode as keyof typeof TaxFormTemplates];
    ...
    const submissions = await prisma.$queryRaw<TaxSubmission[]>`
  ...
  private static validateFormData(formData: any, template: any) {
    const errors: string[] = [];
  ...
  private static async processSubmission(submissionId: string) {
  ...
  static async getSubmissionStatus(formId: string, tenantId: string, companyId: string) {
    return await prisma.$queryRaw<TaxSubmission[]>`
