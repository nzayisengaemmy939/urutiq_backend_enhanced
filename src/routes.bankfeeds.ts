...
async function syncBankTransactions(connectionId: string, syncType: string) {
...
async function matchTransaction(bankTransaction: any, companyId: string) {
...
router.get('/connections', async (req: TenantRequest, res) => {
  try {
    ...
    if (query.bankName) where.bankName = { contains: query.bankName };
...
router.get('/connections/:id', async (req: TenantRequest, res) => {
  try {
    const { companyId } = req;
    const { id } = req.params;
...
router.put('/connections/:id', async (req: TenantRequest, res) => {
  try {
    const { companyId } = req;
    const { id } = req.params;
...
router.delete('/connections/:id', async (req: TenantRequest, res) => {
  try {
    const { companyId } = req;
    const { id } = req.params;
...
router.post('/transactions', async (req: TenantRequest, res) => {
  try {
    const { companyId } = req;
...
router.put('/transactions/:id', async (req: TenantRequest, res) => {
  try {
    const { companyId } = req;
    const { id } = req.params;
...
router.get('/reconciliation-rules', async (req: TenantRequest, res) => {
  try {
    const { companyId } = req;
    if (!companyId) {
      return res.status(400).json({ error: 'Company ID is required' });
...
router.post('/reconciliation-rules', async (req: TenantRequest, res) => {
  try {
    const { companyId, userId } = req;
    if (!companyId || !userId) {
      return res.status(400).json({ error: 'Company ID and User ID are required' });
...
router.get('/sync-logs', async (req: TenantRequest, res) => {
  try {
    const { companyId } = req;
