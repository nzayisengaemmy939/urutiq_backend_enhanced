...
export function mountCardRoutes(router: Router) {
  ...
  router.post('/card-transactions/import', asyncHandler(async (req: Request, res: Response) => {
  ...
  // List exceptions
  router.get('/card-exceptions', asyncHandler(async (req: Request, res: Response) => {
    const tenantId = String(req.header('x-tenant-id') || 'tenant_demo')
  ...
  router.post('/card-exceptions/:id/resolve-create', asyncHandler(async (req: Request, res: Response) => {
    const id = String(req.params.id)
    const receiptDataUrl: string | undefined = req.body?.receiptDataUrl
  ...
  router.post('/card-exceptions/:id/resolve-match', asyncHandler(async (req: Request, res: Response) => {
    const id = String(req.params.id)
  ...
  // Dismiss
  router.delete('/card-exceptions/:id', asyncHandler(async (req: Request, res: Response) => {
    const id = String(req.params.id)
    try {
      await prisma.cardException.delete({ where: { id } })
      res.json({ ok: true })
