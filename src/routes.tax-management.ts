...
// Generate tax form
router.post('/tax/forms/generate',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res) => {
    const params = taxFormSchema.parse(req.body);

    const taxForm = await TaxManagementService.generateTaxForm(
      req.tenantId!,
      params.companyId,
      params.formType,
      params.taxYear,
...
// Submit tax form
router.post('/tax/forms/:formId/submit',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin']),
  asyncHandler(async (req: TenantRequest, res) => {
  ...
  asyncHandler(async (req: TenantRequest, res) => {
    ...
    if (!companyId || !jurisdiction || !taxYear) {
      throw new ApiError(400, 'MISSING_PARAMETERS', 
        'companyId, jurisdiction, and taxYear are required');
    }

    const complianceStatus = await TaxManagementService.getComplianceStatus(
      req.tenantId!,
      companyId as string,
...
router.get('/tax/optimization',
  ...
  asyncHandler(async (req: TenantRequest, res) => {
    ...
    const recommendations = await TaxManagementService.getTaxOptimizationRecommendations(
      req.tenantId!,
      companyId as string,
      jurisdiction as string,
      parseInt(taxYear as string)
...
router.get('/tax/forms/available',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res) => {
...
router.get('/tax/brackets',
  ...
  asyncHandler(async (req: TenantRequest, res) => {
    ...
    if (!jurisdiction || !taxYear || !entityType) {
      throw new ApiError(400, 'MISSING_PARAMETERS', 
        'jurisdiction, taxYear, and entityType are required');
    }
...
router.get('/tax/forms',
  ...
  asyncHandler(async (req: TenantRequest, res) => {
    ...
    if (taxYear) {
      whereClause.taxYear = parseInt(taxYear as string);
    }
    if (status) {
