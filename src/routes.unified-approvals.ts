...
export function mountUnifiedApprovalRoutes(router: Router) {
  ...
  router.get('/approval-workflows', async (req: TenantRequest, res) => {
    try {
      ...
      const entitySubType = req.query.entitySubType as string;
  ...
  router.post('/approval-workflows', validateBody(approvalSchemas.workflowCreate), async (req: TenantRequest, res) => {
    try {
  ...
  router.put('/approval-workflows/:id', validateBody(approvalSchemas.workflowUpdate), async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
      ...
      if (updateData.steps !== undefined) updateFields.steps = JSON.stringify(updateData.steps);
      if (updateData.conditions !== undefined) updateFields.conditions = JSON.stringify(updateData.conditions);
      if (updateData.autoApproval !== undefined) updateFields.autoApproval = updateData.autoApproval;
      if (updateData.escalationRules !== undefined) updateFields.escalationRules = JSON.stringify(updateData.escalationRules);
  ...
  router.post('/approval-requests', validateBody(approvalSchemas.approvalRequestCreate), async (req: TenantRequest, res) => {
    try {
      const companyId = String(req.body.companyId || req.query.companyId || '');
      if (!companyId) {
        return res.status(400).json({ error: 'company_id_required', message: 'Company ID is required' });
  ...
  router.get('/approval-requests', async (req: TenantRequest, res) => {
    try {
      const companyId = String(req.query.companyId || '');
      const entityType = req.query.entityType as string;
  ...
  router.get('/approval-requests/:id', async (req: TenantRequest, res) => {
    try {
      const { id } = req.params;
  ...
  router.post('/approval-requests/:id/actions/:assigneeId', validateBody(approvalSchemas.approvalAction), async (req: TenantRequest, res) => {
    try {
      const { id, assigneeId } = req.params;
      const { action, comments, escalationReason } = req.body;

      const approvalRequest = await UnifiedApprovalEngine.processApprovalAction(
        req.tenantId!,
        id,
        assigneeId,
        action,
  ...
  router.get('/approval-dashboard', async (req: TenantRequest, res) => {
    try {
      const companyId = String(req.query.companyId || '');
  ...
  router.get('/approval-templates', async (req: TenantRequest, res) => {
    try {
      const companyId = String(req.query.companyId || '');
      const entityType = req.query.entityType as string;
  ...
  router.post('/approval-templates', async (req: TenantRequest, res) => {
    try {
      const companyId = String(req.body.companyId || req.query.companyId || '');
      if (!companyId) {
        return res.status(400).json({ error: 'company_id_required', message: 'Company ID is required' });
