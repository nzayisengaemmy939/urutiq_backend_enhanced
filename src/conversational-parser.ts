...
// Interfaces for the conversational parser
...
Respond with only the JSON object.`;
  ...
  private applyAccountingRules(originalText: string, txn: ParsedTransaction) {
    ...
    const lines: JournalEntry[] = [] as any;
  ...
  private inferExpenseAccount(text: string): string | null {
    const t = text.toLowerCase();
    if (/fuel|gas|petrol|diesel/.test(t)) return 'Fuel Expense';
    if (/rent|lease/.test(t)) return 'Rent Expense';
    if (/internet|phone|telecom/.test(t)) return 'Utilities';
    if (/office|stationery|supplies/.test(t)) return 'Office Supplies';
    if (/insurance/.test(t)) return 'Insurance';
    if (/travel|transport/.test(t)) return 'Travel';
    if (/consult|professional|service/.test(t)) return 'Professional Services';
    return null;
  }

  private validateParsedTransaction(parsed: any): string[] {
    const errors: string[] = [];
    ...
    if (parsed.journalEntries && Array.isArray(parsed.journalEntries)) {
      const totalDebits = parsed.journalEntries.reduce((sum: number, entry: any) => sum + (entry.debit || 0), 0);
      const totalCredits = parsed.journalEntries.reduce((sum: number, entry: any) => sum + (entry.credit || 0), 0);
  ...
  private async getOrCreateAccounts(
    tenantId: string,
    companyId: string,
    parsedTransaction: ParsedTransaction
