...
export function mountJournalRoutes(router: Router) {
  ...
  router.get('/trial-balance', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
  ...
  router.post('/', validateBody(schemas.journalPost), async (req: TenantRequest, res) => {
    const { date, memo, reference, lines, companyId } = req.body as any;
    if (!Array.isArray(lines) || lines.length < 2) return res.status(400).json({ error: 'at_least_two_lines' });
    const debits = sum(lines.map((l: any) => Number(l.debit || 0)));
    const credits = sum(lines.map((l: any) => Number(l.credit || 0)));
    if (Math.round((debits - credits) * 100) !== 0) return res.status(400).json({ error: 'unbalanced' });
  ...
  router.put('/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
    const { date, memo, reference, lines } = req.body as any;
  ...
  // Post journal entry
  router.post('/:id/post', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.get('/ledger', async (req: TenantRequest, res) => {
    const accountId = String(req.query.accountId || '');
  ...
  router.post('/journal/:id/post', validateBody(schemas.journalPostAction), async (req: TenantRequest, res, next) => {
    try {
      const { id } = req.params;
      const entry = await prisma.journalEntry.findFirst({ where: { id, tenantId: req.tenantId }, include: { lines: true } });
      if (!entry) throw new ApiError(404, 'not_found', 'Journal entry not found');
      if (entry.status === 'POSTED') throw new ApiError(400, 'already_posted', 'Journal entry is already posted');
      const debits = entry.lines.reduce((s, l) => s + Number(l.debit || 0), 0);
      const credits = entry.lines.reduce((s, l) => s + Number(l.credit || 0), 0);
      if (Math.round((debits - credits) * 100) !== 0) throw new ApiError(400, 'unbalanced', 'Journal entry is not balanced');

      const { createTransaction, transaction } = req.body as any;
