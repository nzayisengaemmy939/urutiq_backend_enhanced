...
export function mountAIInsightsRoutes(router: Router) {
  ...
  router.get('/ai/insights', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), asyncHandler(async (req: TenantRequest, res) => {
  ...
  router.get('/ai-insights/dashboard', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), asyncHandler(async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
  ...
  router.post('/ai-insights/generate', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), asyncHandler(async (req: TenantRequest, res) => {
...
async function calculateFinancialHealthScore(tenantId: string, companyId: string, startDate: Date, endDate: Date) {
  // Get financial data
  ...
  if (paymentRate >= 90) score += 30;
  else if (paymentRate >= 80) score += 20;
  else if (paymentRate >= 70) score += 10;
  ...
  if (overduePercentage <= 5) score += 20;
  else if (overduePercentage <= 10) score += 15;
  else if (overduePercentage <= 20) score += 10;
...
async function getAIMetrics(tenantId: string, companyId: string) {
  // Get AI anomaly logs
...
async function getRevenuePredictions(tenantId: string, companyId: string, startDate: Date, endDate: Date) {
  try {
...
async function getCashFlowPredictions(tenantId: string, companyId: string, startDate: Date, endDate: Date) {
  try {
    ...
    let status = "Healthy";
    if (projectedNetCashFlow < -1000) status = "Critical";
    else if (projectedNetCashFlow < 0) status = "Needs Attention";
    else if (projectedNetCashFlow < 1000) status = "Stable";
...
async function getAnomalies(tenantId: string, companyId: string) {
...
async function getRecommendations(tenantId: string, companyId: string) {
...
async function getTaxOptimizationData(tenantId: string, companyId: string) {
...
async function getPerformanceInsights(tenantId: string, companyId: string, startDate: Date, endDate: Date) {
...
function getAnomalySuggestion(anomalyType: string): string {
...
function calculateAverageCollectionPeriod(invoices: any[]): number {
...
async function generateAIInsights(tenantId: string, companyId: string, insightType: string) {
