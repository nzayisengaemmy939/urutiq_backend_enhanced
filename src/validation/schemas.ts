...
export const journalEntrySchema = z.object({
  ...
  })).min(2, 'At least 2 lines required').refine(
    (lines) => {
...
export const validateCurrency = (amount: number) => currencySchema.safeParse(amount);
export const validatePercentage = (percentage: number) => percentageSchema.safeParse(percentage);
export const validateDate = (date: string) => dateSchema.safeParse(date);
export const validateUUID = (uuid: string) => uuidSchema.safeParse(uuid);
export const validateSKU = (sku: string) => skuSchema.safeParse(sku);
...
export const customValidators = {
  ...
  uniqueAccountCode: async (code: string, companyId: string, excludeId?: string) => {
    // This would be implemented with actual database check
    return true;
  },
  
  // Check if SKU is unique within tenant
  uniqueSKU: async (sku: string, tenantId: string, excludeId?: string) => {
    // This would be implemented with actual database check
    return true;
  },
  
  // Check if PO number is unique within company
  uniquePONumber: async (poNumber: string, companyId: string, excludeId?: string) => {
    // This would be implemented with actual database check
    return true;
  },
  
  // Validate journal entry balance
  validateJournalBalance: (lines: Array<{ debit: number; credit: number }>) => {
    const totalDebits = lines.reduce((sum, line) => sum + Number(line.debit), 0);
    const totalCredits = lines.reduce((sum, line) => sum + Number(line.credit), 0);
  ...
  validateStockAvailability: async (productId: string, quantity: number) => {
    // This would be implemented with actual database check
    return true;
  }
};
