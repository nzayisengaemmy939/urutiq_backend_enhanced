...
export function mountDashboardRoutes(router: Router) {
  ...
  router.get('/dashboard', async (req: TenantRequest, res) => {
    ...
    try {
      ...
      const pendingInvoices = allInvoices.filter(inv => 
        ['draft', 'DRAFT', 'pending', 'PENDING'].includes(inv.status)
      ).length;
      ...
      res.json({
        ...
        recentActivity,
        monthlyTrend: [],
        invoiceStatusBreakdown: allInvoices.reduce((acc: any, inv) => {
