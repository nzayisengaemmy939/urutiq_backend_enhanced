...
export class TaxCalendarService {
  static async initializeCalendar(tenantId: string, companyId: string, country: string, taxYear: number) {
    ...
    const events = [];
    const defaults = TaxCalendarDefaults[country as keyof typeof TaxCalendarDefaults];
    ...
    for (const jurisdiction of jurisdictions) {
      const jurisdictionDefaults = defaults[jurisdiction.level as keyof typeof defaults];
      if (!jurisdictionDefaults) continue;
        ...
        for (const dueDate of dueDates) {
          ...
          await prisma.$executeRaw`
            ...
            VALUES (
              ...
              ${eventDef.title}, ${(eventDef as any).description || ''}, ${dueDate},
          ...
          const event = await prisma.$queryRaw<TaxCalendar[]>`
  ...
  private static async getOrCreateJurisdictions(tenantId: string, companyId: string, country: string) {
    // Check existing jurisdictions
    let jurisdictions = await prisma.$queryRaw<TaxJurisdiction[]>`
      ...
      for (const jur of defaultJurisdictions) {
        ...
        await prisma.$executeRaw`
          ...
          VALUES (
            ...
            ${jur.country}, ${(jur as any).region || null}, ${(jur as any).locality || null}, ${jur.level},
            true, ${new Date()}, ${new Date()}
          )
        `;
        
        const created = await prisma.$queryRaw<TaxJurisdiction[]>`
  ...
  private static getDefaultJurisdictions(country: string) {
    const jurisdictions = {
      US: [
    ...
    return jurisdictions[country as keyof typeof jurisdictions] || [];
  }

  private static calculateDueDates(eventDef: any, taxYear: number, fiscalYearStart?: string): Date[] {
    const dates: Date[] = [];
  ...
  private static async createReminders(calendarId: string, dueDate: Date, reminderDays: number[]) {
  ...
  static async createEvent(tenantId: string, companyId: string, data: z.infer<typeof CreateTaxCalendarEventSchema>) {
    ...
    const result = await prisma.$queryRaw<(TaxCalendar & { jurisdiction: TaxJurisdiction })[]>`
  ...
  static async getEvents(
    ...
    const events = await prisma.$queryRawUnsafe<any[]>(query, ...values);
    
    // Get reminders for each event
    for (const event of events) {
      const reminders = await prisma.$queryRaw<TaxReminder[]>`
  ...
  static async getUpcomingEvents(tenantId: string, companyId: string, daysAhead: number = 30) {
  ...
  static async getOverdueEvents(tenantId: string, companyId: string) {
    const today = new Date();
  ...
  static async updateEvent(
    ...
    tenantId: string,
    companyId: string,
    data: z.infer<typeof UpdateTaxCalendarEventSchema>
  ) {
  ...
  static async markCompleted(id: string, tenantId: string, companyId: string) {
    return await this.updateEvent(id, tenantId, companyId, {
      isCompleted: true,
    });
  }

  static async deleteEvent(id: string, tenantId: string, companyId: string) {
  ...
  static async getCalendarSummary(tenantId: string, companyId: string) {
    const today = new Date();
    ...
    const nextDeadline = await prisma.$queryRaw<(TaxCalendar & { jurisdiction: TaxJurisdiction })[]>`
      SELECT tc.*, tj.id as jurisdiction_id, tj.name as jurisdiction_name, tj.code as jurisdiction_code,
             tj.country as jurisdiction_country, tj.level as jurisdiction_level
      FROM TaxCalendar tc
      JOIN TaxJurisdiction tj ON tc.jurisdictionId = tj.id
