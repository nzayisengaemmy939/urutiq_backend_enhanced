...
export function mountImportShipmentRoutes(router: Router) {
  // Get all import shipments
  router.get('/import-shipments', async (req: TenantRequest, res) => {
  ...
  router.get('/import-shipments/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.post('/import-shipments', validateBody(importShipmentSchemas.create), async (req: TenantRequest, res) => {
  ...
  router.put('/import-shipments/:id', validateBody(importShipmentSchemas.update), async (req: TenantRequest, res) => {
    const { id } = req.params;
    const data = req.body as any;
  ...
  // Add customs event
  router.post('/import-shipments/:id/customs-events', validateBody(importShipmentSchemas.customsEvent), async (req: TenantRequest, res) => {
    const { id } = req.params;
    const data = req.body as any;
  ...
  router.get('/import-shipments/:id/customs-events', async (req: TenantRequest, res) => {
    const { id } = req.params;
  ...
  router.post('/import-shipments/:id/allocate-costs', async (req: TenantRequest, res) => {
    const { id } = req.params;
    const { allocationMethod = 'value', customAllocations } = req.body as any; // value, quantity, custom
    ...
    try {
      ...
      let allocations: Array<{ lineId: string; allocatedCost: number }> = [];
      ...
      if (allocationMethod === 'value') {
        ...
        const totalShare = customAllocations.reduce((s: number, a: any) => s + Number(a.share || 0), 0) || 1;
        allocations = lines.map(line => {
          const custom = customAllocations.find((a: any) => a.lineId === line.id)
  ...
  router.get('/import-shipments/stats', async (req: TenantRequest, res) => {
    const companyId = String(req.query.companyId || '');
    ...
    try {
      ...
      shipments.forEach(shipment => {
        stats.statusBreakdown[shipment.status as keyof typeof stats.statusBreakdown]++;
      });
  ...
  router.delete('/import-shipments/:id', async (req: TenantRequest, res) => {
    const { id } = req.params;
