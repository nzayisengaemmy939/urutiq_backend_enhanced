...
export interface AIPromptConfig {
  ...
  anomalyDetection: {
    systemPrompt: string;
    duplicateAnalysisPrompt: string;
    fraudAnalysisPrompt: string;
    responseFormat: string;
  };
...
export interface AICategoryConfig {
  ...
  assetCategories: CategoryDefinition[];
  liabilityCategories: CategoryDefinition[];
  industrySpecific: IndustryCategoryMapping[];
}
...
export interface IndustryConfig {
  ...
  aiPrompts: Partial<AIPromptConfig>;
  behaviorSettings: Partial<AIBehaviorConfig>;
}

// Default Industry Configurations
export const DEFAULT_INDUSTRY_CONFIGS: IndustryConfig[] = [
  {
    name: 'Technology',
    code: 'TECH',
...
export const DEFAULT_BEHAVIOR_CONFIG: AIBehaviorConfig = {
  confidenceThresholds: {
    categorization: 70,
    anomalyDetection: 75,
    fraudDetection: 85,
...
export class AIConfigurationService {
  async getConfiguration(tenantId: string, companyId: string, configType: string): Promise<AIConfig | null> {
    ...
    if (result) {
      return {
        ...result,
        configType: result.configType as 'prompts' | 'categories' | 'behavior' | 'industry'
      };
    }
    return null;
  }

  async saveConfiguration(tenantId: string, companyId: string, configType: string, configData: any): Promise<AIConfig> {
  ...
  async getIndustryConfig(industryCode: string): Promise<IndustryConfig | null> {
    return DEFAULT_INDUSTRY_CONFIGS.find(config => config.code === industryCode) || null;
  }

  async getCustomizedPrompts(tenantId: string, companyId: string): Promise<AIPromptConfig> {
  ...
  async getCustomizedBehavior(tenantId: string, companyId: string): Promise<AIBehaviorConfig> {
  ...
  async getCustomizedCategories(tenantId: string, companyId: string): Promise<AICategoryConfig> {
  ...
  async updatePromptConfiguration(tenantId: string, companyId: string, prompts: Partial<AIPromptConfig>): Promise<void> {
  ...
  async updateBehaviorConfiguration(tenantId: string, companyId: string, behavior: Partial<AIBehaviorConfig>): Promise<void> {
  ...
  async updateCategoryConfiguration(tenantId: string, companyId: string, categories: Partial<AICategoryConfig>): Promise<void> {
  ...
  async applyIndustryConfiguration(tenantId: string, companyId: string, industryCode: string): Promise<void> {
