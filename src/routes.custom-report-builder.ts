...
import { CustomReportBuilderService, ReportTemplate, ReportSchedule } from './services/custom-report-builder.service.js';
...
router.post('/reports/templates',
  ...
  asyncHandler(async (req: TenantRequest, res) => {
    ...
    const template = await CustomReportBuilderService.createReportTemplate(
      req.tenantId!,
...
router.get('/reports/templates/:templateId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant', 'viewer']),
  asyncHandler(async (req: TenantRequest, res) => {
...
router.put('/reports/templates/:templateId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res) => {
    const { templateId } = req.params;
    ...
    const template = await CustomReportBuilderService.updateReportTemplate(
      req.tenantId!,
      templateId,
      updates
    );
...
router.post('/reports/templates/:templateId/execute',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant', 'viewer']),
  asyncHandler(async (req: TenantRequest, res) => {
    const { templateId } = req.params;
    ...
    const result = await CustomReportBuilderService.executeReportTemplate(
      req.tenantId!,
      templateId,
      parameters
    );
...
router.get('/reports/data-fields',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res) => {
    ...
    if (!companyId || !dataSourceType) {
      throw new ApiError(400, 'MISSING_PARAMETERS', 
        'companyId and dataSourceType are required');
    }

    const fields = await CustomReportBuilderService.getAvailableDataFields(
      req.tenantId!,
      companyId as string,
...
router.get('/reports/templates/predefined',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant', 'viewer']),
  asyncHandler(async (req: TenantRequest, res) => {
...
router.post('/reports/schedules',
  ...
  asyncHandler(async (req: TenantRequest, res) => {
    ...
    const schedule = await CustomReportBuilderService.scheduleReport(
      req.tenantId!,
...
router.put('/reports/schedules/:scheduleId',
  ...
  asyncHandler(async (req: TenantRequest, res) => {
    ...
    if (updates.nextRun) updateData.nextRun = new Date(updates.nextRun);
...
router.get('/custom-reports/executions',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant']),
  asyncHandler(async (req: TenantRequest, res) => {
...
router.get('/custom-reports/executions/:executionId',
  authMiddleware(process.env.JWT_SECRET || 'dev-secret'),
  requireRoles(['admin', 'accountant', 'viewer']),
  asyncHandler(async (req: TenantRequest, res) => {
