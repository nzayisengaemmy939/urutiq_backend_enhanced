...
export function mountAnalyticsRoutes(router: express.Router) {
  ...
  router.get('/spend-trend', asyncHandler(async (req: Request, res: Response) => {
    const period = String(req.query.period || 'last_12_months')
  ...
  router.get('/vendor-spend', asyncHandler(async (req: Request, res: Response) => {
    const period = String(req.query.period || 'last_12_months')
    const currency = String(req.query.currency || 'USD')
    const items = [
  ...
  router.get('/budget-vs-actual', asyncHandler(async (req: Request, res: Response) => {
    const period = String(req.query.period || 'last_12_months')
    const currency = String(req.query.currency || 'USD')
    const items = [
