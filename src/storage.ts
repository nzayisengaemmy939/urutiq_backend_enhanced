...
export function tenantCompanyDir(tenantId: string, companyId: string) {
...
export function createMulter(tenantId: string, companyId: string) {
  const dest = tenantCompanyDir(tenantId, companyId);
  const storage = multer.diskStorage({
    destination: (_req: any, _file: any, cb: (error: any, dest?: string) => void) => cb(null, dest),
