...
export class EnhancedConversationalParser {
    aiProviders = ['ollama', 'openai', 'anthropic'];
    ...
    async initializeAIConfiguration() {
        try {
...
Respond with a JSON object containing:
{
  ...
  "validation": {
    ...
    assessRisk(text, amount) {
        let riskScore = 0;
        const lowerText = text.toLowerCase();
        // Amount-based risk
        if (amount > 1000000)
            riskScore += 30; // Large amounts
        if (amount > 100000)
            riskScore += 20;
        if (amount > 10000)
            riskScore += 10;
        ...
        // Unusual patterns
        if (text.includes('cash') && amount > 50000)
            riskScore += 20; // Large cash transactions
        if (text.includes('anonymous') || text.includes('unknown'))
            riskScore += 30;
        return Math.min(100, riskScore);
    }
    detectComplianceFlags(text) {
        const flags = [];
        const lowerText = text.toLowerCase();
    ...
    detectPaymentAccount(description) {
    ...
    createEnhancedJournalEntries(transactionType, category, amount, description) {
        const entries = [];
    ...
    validateTransaction(entries, amount, riskScore) {
    ...
    normalizeParsedTransaction(parsed, originalText) {
        return {
    ...
    generateAIInsights(parsed, originalText) {
        const insights = {
