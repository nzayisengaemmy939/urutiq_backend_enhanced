...
export class NotificationService {
     ...
     */
    static async sendPaymentReminders(tenantId) {
        try {
     ...
     */
    static async sendPaymentReminder(invoice, type) {
        try {
     ...
     */
    static async sendPaymentConfirmation(invoiceId, paymentData) {
        try {
     ...
     */
    static async sendInvoiceNotification(invoiceId, type) {
        try {
            const invoice = await prisma.invoice.findFirst({
                where: { id: invoiceId },
                include: { customer: true, company: true }
     ...
     */
    static getPaymentReminderTemplate(type) {
        if (type === 'overdue') {
            return {
...
{{companyName}}`,
     ...
     */
    static getInvoiceNotificationTemplate(type) {
        if (type === 'sent') {
            return {
                id: 'invoice_sent',
                name: 'Invoice Sent',
