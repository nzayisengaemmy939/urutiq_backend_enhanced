...
export function mountTransactionRoutes(router) {
    router.get('/transactions', async (req, res) => {
        const page = Math.max(1, parseInt(req.query.page || '1', 10));
        const pageSize = Math.min(100, Math.max(1, parseInt(req.query.pageSize || '20', 10)));
        const type = req.query.transactionType || undefined;
        const status = req.query.status || undefined;
        const where = { tenantId: req.tenantId, transactionType: type, status };
    ...
    router.post('/transactions', validateBody(transactionCreateSchema), async (req, res) => {
        const { transactionType, amount, currency, transactionDate, status, companyId, linkedJournalEntryId } = req.body;
        // Determine company id from body or header
        const resolvedCompanyId = companyId || req.headers['x-company-id'] || '';
    ...
    router.post('/transactions/override', validateBody(transactionCreateSchema.extend({
        ...
        const { transactionType, amount, currency, transactionDate, status, companyId, linkedJournalEntryId, overrideClosedPeriod, justification } = req.body;
