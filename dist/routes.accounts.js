...
async function getCompanyOrThrow(tenantId, companyId) {
    ...
    if (!company)
        throw new ApiError(400, 'COMPANY_NOT_FOUND', 'Company not found');
    return company;
}
async function getAccountTypeOrThrow(tenantId, typeId) {
    const accountType = await prisma.accountType.findFirst({ where: { id: typeId, tenantId } });
    if (!accountType)
        throw new ApiError(400, 'ACCOUNT_TYPE_NOT_FOUND', 'Account type not found');
    return accountType;
}
async function getParentAccountOrThrow(tenantId, parentId) {
    const parentAccount = await prisma.account.findFirst({ where: { id: parentId, tenantId } });
    if (!parentAccount)
        throw new ApiError(400, 'PARENT_ACCOUNT_NOT_FOUND', 'Parent account not found');
...
export function mountAccountRoutes(router) {
    ...
    router.post('/account-types', requireRoles(['admin', 'accountant']), validateBody(schemas.accountTypeCreate), asyncHandler(async (req, res) => {
    ...
    router.put('/account-types/:id', requireRoles(['admin', 'accountant']), validateBody(schemas.accountTypeUpdate), asyncHandler(async (req, res) => {
        const { id } = req.params;
    ...
    router.delete('/account-types/:id', requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
        const { id } = req.params;
    ...
    router.post('/accounts', requireRoles(['admin', 'accountant']), validateBody(schemas.accountCreate), asyncHandler(async (req, res) => {
        ...
        if (parentId)
            await getParentAccountOrThrow(req.tenantId, parentId);
    ...
    router.put('/accounts/:id', requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
        const { id } = req.params;
    ...
    router.delete('/accounts/:id', requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
        const { id } = req.params;
