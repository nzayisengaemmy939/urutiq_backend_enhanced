 ...
 * @access Private
 */
router.get('/', requireRoles(['admin', 'accountant', 'manager']), async (req, res) => {
    try {
 ...
 * @access Private
 */
router.get('/:id', requireRoles(['admin', 'accountant', 'manager']), async (req, res) => {
    try {
        const { id } = req.params;
 ...
 * @access Private
 */
router.post('/', requireRoles(['admin', 'accountant']), validateRequestMiddleware({ body: billSchema }), async (req, res) => {
    try {
 ...
 * @access Private
 */
router.put('/:id', requireRoles(['admin', 'accountant']), validateRequestMiddleware({ body: billSchema.partial() }), async (req, res) => {
    try {
        const { id } = req.params;
        const data = req.body;
        const tenantId = req.tenantId;
 ...
 * @access Private
 */
router.delete('/:id', requireRoles(['admin', 'accountant']), async (req, res) => {
    try {
        const { id } = req.params;
        const tenantId = req.tenantId;
        const companyId = req.header('x-company-id') || 'demo-company';
 ...
 * @access Private
 */
router.post('/:id/post', requireRoles(['admin', 'accountant']), async (req, res) => {
    try {
        const { id } = req.params;
        const tenantId = req.tenantId;
        const companyId = req.header('x-company-id') || 'demo-company';
 ...
 * @access Private
 */
router.post('/:id/payment', requireRoles(['admin', 'accountant']), validateRequestMiddleware({ body: paymentSchema }), async (req, res) => {
    try {
        const { id } = req.params;
        const data = req.body;
        const tenantId = req.tenantId;
 ...
 * @access Private
 */
router.get('/analytics/aging', requireRoles(['admin', 'accountant', 'manager']), async (req, res) => {
    try {
        const tenantId = req.tenantId;
        const companyId = req.header('x-company-id') || 'demo-company';
        const bills = await prisma.bill.findMany({
        ...
        aging.details.forEach(item => {
            ...
            if (item.category === 'current')
                aging.current += item.balance;
            else if (item.category === 'days1_30')
                aging.days1_30 += item.balance;
            else if (item.category === 'days31_60')
                aging.days31_60 += item.balance;
            else if (item.category === 'days61_90')
                aging.days61_90 += item.balance;
            else if (item.category === 'over90')
                aging.over90 += item.balance;
        });
        res.json(aging);
    }
    catch (error) {
