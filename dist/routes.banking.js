...
export function mountBankingRoutes(router) {
    ...
    router.post('/bank-accounts', validateBody(bankingSchemas.bankAccountCreate), async (req, res) => {
        const { companyId, bankName, accountNumber, accountType = 'checking', currency = 'USD', routingNumber, swiftCode, iban, accountHolder, branchCode, branchName, notes } = req.body;
    ...
    router.post('/payments', validateBody(bankingSchemas.paymentCreate), async (req, res) => {
        const { companyId, transactionId, bankAccountId, method, reference, amount, paymentDate, applications, fxGainLoss } = req.body;
        ...
        if (!cash || !ar || !ap)
            return res.status(400).json({ error: 'missing_accounts', message: 'Require mappings for CASH, AR, and AP' });
    ...
    router.post('/bank-transactions', async (req, res) => {
        const { bankAccountId, transactionDate, amount, currency = 'USD', description, merchantName, merchantCategory, transactionType, reference, checkNumber, memo, category, tags, fees = 0, exchangeRate, originalAmount, originalCurrency, location, authorizationCode } = req.body;
    ...
    router.post('/bank-transactions/:id/reconcile', validateBody(bankingSchemas.reconcileBankTxn), async (req, res) => {
        const { id } = req.params;
        const { paymentId, reconciledBy } = req.body;
    ...
    router.post('/bank-feed', async (req, res) => {
        const feed = req.body;
    ...
    router.post('/bank-transactions/:id/create-journal', async (req, res) => {
        try {
            const { id } = req.params;
            const companyId = String(req.query.companyId || '');
            const journalEntry = await BankIntegrationService.createJournalEntriesForBankTransaction(req.tenantId, companyId, id);
    ...
    router.post('/bank-feeds/process', async (req, res) => {
        try {
            const { bankAccountId, feedData } = req.body;
            const companyId = String(req.query.companyId || '');
            const results = await BankIntegrationService.processBankFeedData(req.tenantId, companyId, bankAccountId, feedData);
    ...
    router.get('/cash-flow-forecast', async (req, res) => {
        try {
            ...
            const forecast = await BankIntegrationService.generateCashFlowForecast(req.tenantId, companyId, bankAccountId, days);
            res.json(forecast);
        }
        catch (error) {
    ...
    router.post('/bank-transactions/categorize', async (req, res) => {
        try {
            ...
            const result = await AICategorizationService.autoCategorizeTransactions(req.tenantId, companyId, transactionIds);
            res.json({
                success: true,
    ...
    router.post('/bank-transactions/:id/categorize', async (req, res) => {
        try {
            ...
            const categorization = await AICategorizationService.categorizeTransaction(req.tenantId, companyId, {
    ...
    router.post('/bank-transactions/:id/correct-category', async (req, res) => {
        try {
            const { id } = req.params;
            ...
            await AICategorizationService.learnFromCorrection(req.tenantId, companyId, id, category, subcategory);
    ...
    router.get('/categorization-stats', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const stats = await AICategorizationService.getCategorizationStats(req.tenantId, companyId);
            res.json(stats);
        }
        catch (error) {
    ...
    router.post('/bank-transactions/:id/find-matches', async (req, res) => {
        try {
            const { id } = req.params;
            const companyId = String(req.query.companyId || '');
            const matches = await SmartReconciliationService.findMatches(req.tenantId, companyId, id);
    ...
    router.post('/bank-transactions/:id/auto-reconcile', async (req, res) => {
        try {
            const { id } = req.params;
            const companyId = String(req.query.companyId || '');
            const result = await SmartReconciliationService.autoReconcile(req.tenantId, companyId, id);
            res.json(result);
        }
        catch (error) {
    ...
    router.post('/bank-transactions/auto-reconcile-all', async (req, res) => {
        try {
            const { bankAccountId } = req.body;
            const companyId = String(req.query.companyId || '');
            const result = await SmartReconciliationService.runAutoReconciliation(req.tenantId, companyId, bankAccountId);
    ...
    router.get('/reconciliation-stats', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const stats = await SmartReconciliationService.getReconciliationStats(req.tenantId, companyId);
            res.json(stats);
        }
        catch (error) {
    ...
    router.get('/exchange-rates/:fromCurrency/:toCurrency', async (req, res) => {
        try {
            const { fromCurrency, toCurrency } = req.params;
            const { date } = req.query;
            const rate = await CurrencyService.getExchangeRate(fromCurrency.toUpperCase(), toCurrency.toUpperCase(), date ? new Date(date) : undefined);
    ...
    router.post('/convert-currency', async (req, res) => {
        try {
            ...
            const conversion = await CurrencyService.convertCurrency(Number(amount), fromCurrency.toUpperCase(), toCurrency.toUpperCase(), date ? new Date(date) : undefined);
    ...
    router.get('/historical-rates/:fromCurrency/:toCurrency', async (req, res) => {
        try {
            const { fromCurrency, toCurrency } = req.params;
            ...
            const rates = await CurrencyService.getHistoricalRatesForChart(fromCurrency.toUpperCase(), toCurrency.toUpperCase(), new Date(startDate), new Date(endDate));
    ...
    router.post('/clear-rate-cache', async (req, res) => {
        try {
    ...
    router.post('/clear-database-rates/:fromCurrency/:toCurrency', async (req, res) => {
        try {
            const { fromCurrency, toCurrency } = req.params;
    ...
    router.post('/force-refresh-rate/:fromCurrency/:toCurrency', async (req, res) => {
        try {
            const { fromCurrency, toCurrency } = req.params;
            const rate = await CurrencyService.forceRefreshRate(fromCurrency.toUpperCase(), toCurrency.toUpperCase());
            res.json({ success: true, rate });
        }
        catch (error) {
    ...
    router.get('/test-exchange-api/:baseCurrency?', async (req, res) => {
        try {
    ...
    router.post('/force-refresh-historical/:fromCurrency/:toCurrency', async (req, res) => {
        try {
            const { fromCurrency, toCurrency } = req.params;
            const { startDate, endDate } = req.body;
            const rates = await CurrencyService.forceRefreshHistoricalData(fromCurrency.toUpperCase(), toCurrency.toUpperCase(), startDate || new Date(Date.now() - 30 * 24 * 60 * 60 * 1000).toISOString().split('T')[0], endDate || new Date().toISOString().split('T')[0]);
            res.json({ success: true, rates });
        }
        catch (error) {
    ...
    router.post('/payment-processors/initialize', async (req, res) => {
        try {
            ...
            const processorConfig = await PaymentProcessorService.initializeProcessor(req.tenantId, companyId, processorType, config);
    ...
    router.post('/payment-intents', async (req, res) => {
        try {
            ...
            const paymentIntent = await PaymentProcessorService.createPaymentIntent(req.tenantId, companyId, Number(amount), currency, { customerId, paymentMethodId, description, metadata, invoiceId });
    ...
    router.post('/payment-intents/:id/confirm', async (req, res) => {
        try {
            ...
            const companyId = String(req.query.companyId || '');
            const paymentIntent = await PaymentProcessorService.confirmPaymentIntent(req.tenantId, companyId, id, paymentMethodId);
            res.json({ success: true, paymentIntent });
        }
        catch (error) {
    ...
    router.post('/payment-customers', async (req, res) => {
        try {
            ...
            const customer = await PaymentProcessorService.createCustomer(req.tenantId, companyId, { email, name, phone, address, metadata });
    ...
    router.post('/payment-methods', async (req, res) => {
        try {
            ...
            const paymentMethod = await PaymentProcessorService.addPaymentMethod(req.tenantId, companyId, customerId, { type, card, bankAccount, isDefault });
    ...
    router.get('/payment-processors/stats', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const stats = await PaymentProcessorService.getProcessorStats(req.tenantId, companyId);
    ...
    router.get('/payment-intents', async (req, res) => {
        try {
            ...
            const paymentIntents = await PaymentProcessorService.getPaymentIntents(req.tenantId, companyId, {
                limit: Number(limit),
                status: status,
                processor: processor
            });
    ...
    router.get('/payment-customers', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const { limit = 20, search } = req.query;
            const customers = await PaymentProcessorService.getCustomers(req.tenantId, companyId, {
                limit: Number(limit),
                search: search
            });
    ...
    router.get('/payment-methods', async (req, res) => {
        try {
            ...
            const paymentMethods = await PaymentProcessorService.getPaymentMethods(req.tenantId, companyId, {
    ...
    router.get('/analytics/insights', async (req, res) => {
        try {
            ...
            const insights = await AdvancedAnalyticsService.generateFinancialInsights(req.tenantId, companyId, industry);
    ...
    router.get('/analytics/benchmarks/:industry', async (req, res) => {
        try {
    ...
    router.get('/analytics/cash-flow-forecast', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const { months } = req.query;
            const forecast = await AdvancedAnalyticsService.generateCashFlowForecast(req.tenantId, companyId, months ? Number(months) : 6);
    ...
    router.get('/institutions', async (req, res) => {
        try {
            ...
            const institutions = await BankConnectionService.getInstitutions(provider || 'plaid', country || 'US', search);
    ...
    router.post('/connections', async (req, res) => {
        try {
            ...
            const connection = await BankConnectionService.createConnection(req.tenantId, companyId, provider, institutionId, credentials);
    ...
    router.get('/connections', async (req, res) => {
        try {
            ...
            console.log('Header companyId:', headerCompanyId);
            console.log('Final companyId:', companyId);
            const connections = await BankConnectionService.getCompanyConnections(req.tenantId, companyId);
    ...
    router.get('/connections/stats', async (req, res) => {
        try {
            ...
            console.log('Header companyId:', headerCompanyId);
            console.log('Final companyId:', companyId);
            const stats = await BankConnectionService.getConnectionStats(req.tenantId, companyId);
    ...
    router.get('/mobile/stats', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const stats = await MobileBankingService.getMobileStats(req.tenantId, companyId);
            res.json({ success: true, ...stats });
        }
        catch (error) {
    ...
    router.get('/mobile/transactions', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const limit = parseInt(String(req.query.limit || '20'));
            const transactions = await MobileBankingService.getMobileTransactions(req.tenantId, companyId, limit);
    ...
    router.get('/mobile/accounts', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const accounts = await MobileBankingService.getMobileAccounts(req.tenantId, companyId);
    ...
    router.get('/mobile/insights', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const insights = await MobileBankingService.getMobileInsights(req.tenantId, companyId);
            res.json({ success: true, insights });
        }
        catch (error) {
    ...
    router.get('/mobile/notifications', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const notifications = await MobileBankingService.getMobileNotifications(req.tenantId, companyId);
    ...
    router.get('/mobile/quick-actions', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const quickActions = await MobileBankingService.getQuickActions(req.tenantId, companyId);
    ...
    router.post('/mobile/quick-actions/:actionId', async (req, res) => {
        try {
            const { actionId } = req.params;
            const companyId = String(req.query.companyId || '');
            const params = req.body;
            const result = await MobileBankingService.executeQuickAction(req.tenantId, companyId, actionId, params);
            res.json({ success: true, ...result });
        }
        catch (error) {
    ...
    router.get('/mobile-money/providers', async (req, res) => {
        try {
            const { country } = req.query;
            const providers = await MobileMoneyService.getProviders(country);
    ...
    router.get('/mobile-money/providers/:id', async (req, res) => {
        try {
            const { id } = req.params;
    ...
    router.post('/mobile-money/accounts', async (req, res) => {
        try {
            ...
            const account = await MobileMoneyService.createAccount(req.tenantId, companyId, provider, accountNumber, accountName, phoneNumber, currency);
    ...
    router.get('/mobile-money/accounts', async (req, res) => {
        try {
            ...
            console.log('Header companyId:', headerCompanyId);
            console.log('Final companyId:', companyId);
            const accounts = await MobileMoneyService.getCompanyAccounts(req.tenantId, companyId);
    ...
    router.post('/mobile-money/payments', async (req, res) => {
        try {
            ...
            console.log('Header companyId:', headerCompanyId);
            console.log('Final companyId:', companyId);
            const result = await MobileMoneyService.initiatePayment(req.tenantId, companyId, paymentRequest);
    ...
    router.post('/mobile-money/callback/:provider', async (req, res) => {
        try {
    ...
    router.get('/mobile-money/transactions', async (req, res) => {
        try {
            ...
            const transactions = await MobileMoneyService.getTransactionHistory(req.tenantId, companyId, provider, limit);
    ...
    router.get('/mobile-money/balance', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            ...
            const balance = await MobileMoneyService.getAccountBalance(req.tenantId, companyId, provider, phoneNumber);
    ...
    router.get('/mobile-money/stats', async (req, res) => {
        try {
            ...
            console.log('Header companyId:', headerCompanyId);
            console.log('Final companyId:', companyId);
            const stats = await MobileMoneyService.getStats(req.tenantId, companyId);
