...
export class OCRService {
     ...
     */
    static async describeImageForOCR(imageUrl) {
        try {
     ...
     */
    static async quickOCRProcessing(imageUrl) {
        return new Promise((resolve) => {
            const timeout = setTimeout(() => {
                resolve(null); // Return null if timeout
            }, 5000); // 5 second timeout
            // Try to use AI with a very simple prompt
            AIService.callOllama(`What text do you see in this image? ${imageUrl.substring(0, 100)}...`, 'Extract text from images.').then(response => {
     ...
     */
    static getManualReviewFallback(imageUrl, startTime) {
...
This ensures accurate accounting data entry.`;
     ...
     */
    static async processImage(imageUrl, imageData) {
...
This ensures accurate data entry.`,
     ...
     */
    static async extractReceiptData(text) {
        try {
     ...
     */
    static async processReceipt(imageUrl, imageData) {
     ...
     */
    static async batchProcessReceipts(imageUrls) {
        const results = [];
        let successful = 0;
        let failed = 0;
