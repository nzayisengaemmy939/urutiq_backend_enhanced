...
export class AIService {
     ...
     */
    static async callOllama(prompt, systemPrompt) {
     ...
     */
    static async analyzeDocument(documentName, mimeType, content) {
     ...
     */
    static async generateInsights(documents) {
     ...
     */
    static async generateCategorizationSuggestions(documents) {
     ...
     */
    static async generateSummary(documentName, mimeType, content) {
        // Check if Ollama is available first
        const ollamaAvailable = await isOllamaAvailable();
        if (!ollamaAvailable) {
    ...
    static getFallbackInsights(documents) {
    ...
    static getFallbackCategorization(documents) {
        const uncategorizedDocs = documents.filter(doc => !doc.categoryId);
        return [
            {
    ...
    static async analyzeDocumentQuality(documents) {
        // Check if Ollama is available first
        const ollamaAvailable = await isOllamaAvailable();
        if (!ollamaAvailable) {
            return {
    ...
    // Generate AI tags
    static async generateTags(documents) {
        // Check if Ollama is available first
        const ollamaAvailable = await isOllamaAvailable();
        if (!ollamaAvailable) {
    ...
    static async generateExtractions(documents) {
        // Check if Ollama is available first
        const ollamaAvailable = await isOllamaAvailable();
        if (!ollamaAvailable) {
    ...
    static async generateSummaries(documents) {
        // Check if Ollama is available first
        const ollamaAvailable = await isOllamaAvailable();
        if (!ollamaAvailable) {
    ...
    // Fallback methods
    static getFallbackTags(documents) {
        const tagCounts = {};
        documents.forEach(doc => {
            const mimeType = doc.mimeType || '';
            if (mimeType.includes('pdf'))
                tagCounts['PDF Documents'] = (tagCounts['PDF Documents'] || 0) + 1;
            if (mimeType.includes('image'))
                tagCounts['Images'] = (tagCounts['Images'] || 0) + 1;
            if (mimeType.includes('text'))
                tagCounts['Text Files'] = (tagCounts['Text Files'] || 0) + 1;
            if (mimeType.includes('spreadsheet'))
                tagCounts['Spreadsheets'] = (tagCounts['Spreadsheets'] || 0) + 1;
        });
    ...
    static getFallbackExtractions(documents) {
    ...
    static getFallbackSummaries(documents) {
        return [
            {
                id: '1',
                title: 'Document Collection Overview',
