...
export class FixedAssetsService {
    ...
    async listCategories(companyId) {
        try {
            return await prisma.fixedAssetCategory.findMany({ where: { companyId } });
        }
        catch {
            return this.memory.categories.filter(c => c.companyId === companyId);
        }
    }
    async upsertCategory(cat) {
        try {
            ...
            if (exists)
                return await prisma.fixedAssetCategory.update({ where: { id: cat.id }, data: cat });
            return await prisma.fixedAssetCategory.create({ data: cat });
        }
        catch {
            const idx = this.memory.categories.findIndex(x => x.id === cat.id);
            if (idx >= 0)
                this.memory.categories[idx] = cat;
            else
                this.memory.categories.push(cat);
            return cat;
        }
    }
    async listAssets(companyId) {
        try {
            return await prisma.fixedAsset.findMany({ where: { companyId } });
        }
        catch {
            return this.memory.assets.filter(a => a.companyId === companyId);
        }
    }
    async upsertAsset(asset) {
        try {
            ...
            if (exists)
                return await prisma.fixedAsset.update({ where: { id: asset.id }, data: asset });
            return await prisma.fixedAsset.create({ data: asset });
        }
        catch {
            const idx = this.memory.assets.findIndex(x => x.id === asset.id);
            if (idx >= 0)
                this.memory.assets[idx] = asset;
            else
                this.memory.assets.push(asset);
            return asset;
        }
    }
    async deleteAsset(id) {
        try {
            ...
            this.memory.assets = this.memory.assets.filter(a => a.id !== id);
        }
        return { id };
    }
    // Depreciation preview
    previewDepreciation(asset, category, asOf) {
        ...
        for (let m = 0; m < Math.min(months, useful); m++) {
            ...
            if (category.method === 'straight_line')
                dep = +(base / useful).toFixed(2);
            else if (category.method === 'declining_balance')
                dep = +((base - accumulated) * (2 / useful)).toFixed(2);
            else {
                const n = useful;
                const sum = (n * (n + 1)) / 2;
    ...
    async previewCompanyDepreciation(companyId, asOf) {
        const [assets, cats] = await Promise.all([this.listAssets(companyId), this.listCategories(companyId)]);
        const catById = new Map(cats.map((c) => [c.id, c]));
        const items = assets.map((a) => ({ asset: a, category: catById.get(a.categoryId), preview: this.previewDepreciation(a, catById.get(a.categoryId), asOf) }));
        return { asOf, items };
    }
    async postCompanyDepreciationJournal(tenantId, companyId, asOf) {
        const preview = await this.previewCompanyDepreciation(companyId, asOf);
        const items = preview.items.filter((i) => i.category && i.preview?.schedule?.length);
        if (items.length === 0)
            return { created: null };
    ...
    async disposeAssetJournal(tenantId, companyId, assetId, disposeDate, proceeds, proceedsAccountId) {
        const [assets, cats] = await Promise.all([this.listAssets(companyId), this.listCategories(companyId)]);
        const asset = assets.find(a => a.id === assetId);
        if (!asset)
            return { created: null, error: 'asset_not_found' };
        const category = cats.find(c => c.id === asset.categoryId);
        if (!category)
            return { created: null, error: 'category_not_found' };
        const prev = this.previewDepreciation(asset, category, disposeDate);
        ...
        const created = await prisma.$transaction(async (tx) => {
            ...
            // Remove asset
            if (accumulated > 0)
                await tx.journalLine.create({ data: { tenantId, entryId: entry.id, accountId: category.accounts.accDepId, debit: accumulated, credit: 0, memo: 'Reverse accumulated depreciation' } });
            if (cost > 0)
                await tx.journalLine.create({ data: { tenantId, entryId: entry.id, accountId: category.accounts.assetId, debit: 0, credit: cost, memo: 'Remove asset cost' } });
            // Proceeds
