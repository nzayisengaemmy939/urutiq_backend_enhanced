...
export class EnhancedFinancialReportingEngine {
    async generateBalanceSheet(companyId, date, tenantId) {
        if (!tenantId) {
    ...
    async generateProfitAndLoss(companyId, period, tenantId) {
        if (!tenantId) {
            throw new Error('Tenant ID is required for financial reporting');
        }
        const actualTenantId = tenantId;
        ...
        const revenueBalances = await this.calculateComprehensivePeriodBalances(accounts.filter(a => a.code.startsWith('4')), // Revenue accounts
        period, actualTenantId, companyId);
        const cogsBalances = await this.calculateComprehensivePeriodBalances(accounts.filter(a => a.code.startsWith('5')), // COGS accounts
        period, actualTenantId, companyId);
        const expenseBalances = await this.calculateComprehensivePeriodBalances(accounts.filter(a => a.code.startsWith('6')), // Expense accounts
        period, actualTenantId, companyId);
        // Group by categories
    ...
    async generateCashFlow(companyId, period, tenantId) {
        if (!tenantId) {
            throw new Error('Tenant ID is required for financial reporting');
        }
        const actualTenantId = tenantId;
    ...
    async generateCustomReport(specification, tenantId) {
    ...
    // Helper methods
    async calculateAccountBalances(accounts, date, tenantId, companyId) {
        const balances = [];
    ...
    async calculatePeriodBalances(accounts, period, tenantId, companyId) {
        const balances = [];
        for (const account of accounts) {
    ...
    async calculateComprehensivePeriodBalances(accounts, period, tenantId, companyId) {
        const balances = [];
        for (const account of accounts) {
            let balance = 0;
    ...
    groupEquity(balances) {
        const contributedCapital = balances.filter(b => b.accountNumber.startsWith('30') || b.accountNumber.startsWith('31'));
        const retainedEarnings = balances.filter(b => b.accountNumber.startsWith('32') || b.accountNumber.startsWith('39'));
        const otherEquity = balances.filter(b => b.accountNumber.startsWith('33') || b.accountNumber.startsWith('34') ||
    ...
    groupOperatingExpenses(balances) {
    ...
    calculateBalanceSheetChanges(current, previous) {
    ...
    calculateFinancialRatios(assets, liabilities, equity) {
    ...
    async calculateOperatingCashFlows(companyId, period, tenantId) {
        // Get cash account
    ...
    async calculateInvestingCashFlows(companyId, period, tenantId) {
        // Get cash account
        const cashAccount = await prisma.account.findFirst({
            where: {
                tenantId,
    ...
    async calculateFinancingCashFlows(companyId, period, tenantId) {
        // Get cash account
        const cashAccount = await prisma.account.findFirst({
            where: {
                tenantId,
    ...
    async getCashBalance(cashAccounts, date, tenantId, companyId) {
        let totalCash = 0;
    ...
    async processAccountItem(item, accounts, filters, tenantId, companyId) {
                ...
                if (report.totalAssets !== undefined) {
                    ...
                    if (report.assets) {
                        ...
                        if (report.assets.currentAssets && report.assets.currentAssets.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.assets.currentAssets.forEach((asset) => {
                        ...
                        if (report.assets.fixedAssets && report.assets.fixedAssets.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.assets.fixedAssets.forEach((asset) => {
                                doc.text(`${asset.accountNumber} - ${asset.accountName}`, 90, yPosition);
                                doc.text(`$${asset.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                        ...
                        if (report.assets.otherAssets && report.assets.otherAssets.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.assets.otherAssets.forEach((asset) => {
                                doc.text(`${asset.accountNumber} - ${asset.accountName}`, 90, yPosition);
                                doc.text(`$${asset.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                    ...
                    if (report.liabilities) {
                        ...
                        if (report.liabilities.currentLiabilities && report.liabilities.currentLiabilities.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.liabilities.currentLiabilities.forEach((liability) => {
                        ...
                        if (report.liabilities.longTermLiabilities && report.liabilities.longTermLiabilities.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.liabilities.longTermLiabilities.forEach((liability) => {
                                doc.text(`${liability.accountNumber} - ${liability.accountName}`, 90, yPosition);
                                doc.text(`$${liability.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                    ...
                    if (report.equity) {
                        ...
                        if (report.equity.contributedCapital && report.equity.contributedCapital.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.equity.contributedCapital.forEach((equity) => {
                        ...
                        if (report.equity.retainedEarnings && report.equity.retainedEarnings.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.equity.retainedEarnings.forEach((equity) => {
                                doc.text(`${equity.accountNumber} - ${equity.accountName}`, 90, yPosition);
                                doc.text(`$${equity.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                        ...
                        if (report.equity.otherEquity && report.equity.otherEquity.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.equity.otherEquity.forEach((equity) => {
                                doc.text(`${equity.accountNumber} - ${equity.accountName}`, 90, yPosition);
                                doc.text(`$${equity.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                    ...
                    if (report.ratios) {
                        ...
                        yPosition += 20;
                        doc.fontSize(10);
                        Object.entries(report.ratios).forEach(([key, value]) => {
                    ...
                    if (report.changes) {
                        ...
                        yPosition += 20;
                        doc.fontSize(10);
                        Object.entries(report.changes).forEach(([key, value]) => {
                    ...
                    if (report.revenue) {
                        ...
                        if (report.revenue.salesRevenue && report.revenue.salesRevenue.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.revenue.salesRevenue.forEach((item) => {
                        ...
                        if (report.revenue.serviceRevenue && report.revenue.serviceRevenue.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.revenue.serviceRevenue.forEach((item) => {
                                doc.text(`${item.accountNumber} - ${item.accountName}`, 90, yPosition);
                                doc.text(`$${item.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                        ...
                        if (report.revenue.otherRevenue && report.revenue.otherRevenue.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.revenue.otherRevenue.forEach((item) => {
                                doc.text(`${item.accountNumber} - ${item.accountName}`, 90, yPosition);
                                doc.text(`$${item.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                    ...
                    if (report.costOfGoodsSold) {
                        ...
                        if (report.costOfGoodsSold.directMaterials && report.costOfGoodsSold.directMaterials.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.costOfGoodsSold.directMaterials.forEach((item) => {
                                doc.text(`${item.accountNumber} - ${item.accountName}`, 90, yPosition);
                                doc.text(`$${item.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                        ...
                        if (report.costOfGoodsSold.directLabor && report.costOfGoodsSold.directLabor.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.costOfGoodsSold.directLabor.forEach((item) => {
                                doc.text(`${item.accountNumber} - ${item.accountName}`, 90, yPosition);
                                doc.text(`$${item.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                        ...
                        if (report.costOfGoodsSold.overhead && report.costOfGoodsSold.overhead.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.costOfGoodsSold.overhead.forEach((item) => {
                                doc.text(`${item.accountNumber} - ${item.accountName}`, 90, yPosition);
                                doc.text(`$${item.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                    ...
                    if (report.operatingExpenses) {
                        ...
                        if (report.operatingExpenses.sellingExpenses && report.operatingExpenses.sellingExpenses.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.operatingExpenses.sellingExpenses.forEach((item) => {
                                doc.text(`${item.accountNumber} - ${item.accountName}`, 90, yPosition);
                                doc.text(`$${item.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                        ...
                        if (report.operatingExpenses.administrativeExpenses && report.operatingExpenses.administrativeExpenses.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.operatingExpenses.administrativeExpenses.forEach((item) => {
                                doc.text(`${item.accountNumber} - ${item.accountName}`, 90, yPosition);
                                doc.text(`$${item.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                        ...
                        if (report.operatingExpenses.researchExpenses && report.operatingExpenses.researchExpenses.length > 0) {
                            ...
                            yPosition += 15;
                            doc.fontSize(10);
                            report.operatingExpenses.researchExpenses.forEach((item) => {
                                doc.text(`${item.accountNumber} - ${item.accountName}`, 90, yPosition);
                                doc.text(`$${item.balance?.toLocaleString() || '0'}`, 450, yPosition);
                                yPosition += 12;
                            });
                    ...
                    if (report.margins) {
                        ...
                        yPosition += 20;
                        doc.fontSize(10);
                        Object.entries(report.margins).forEach(([key, value]) => {
