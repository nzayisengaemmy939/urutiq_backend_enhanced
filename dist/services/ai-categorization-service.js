...
export class AICategorizationService {
     ...
     */
    static async categorizeTransaction(tenantId, companyId, transaction) {
        ...
        for (const [categoryName, pattern] of Object.entries(this.CATEGORY_PATTERNS)) {
            ...
            if (confidence > bestMatch.confidence) {
                ...
                const subcategories = this.SUBCATEGORY_MAPPINGS[categoryName] || ['Other'];
     ...
     */
    static generateTags(category, subcategory, keywordMatches, merchantMatches) {
     ...
     */
    static suggestAccountMapping(category, subcategory) {
        ...
        return accountMappings[category] || 'EXPENSE';
    }
    /**
     * Auto-categorize multiple transactions
     */
    static async autoCategorizeTransactions(tenantId, companyId, transactionIds) {
        const results = [];
        let success = 0;
        let failed = 0;
     ...
     */
    static async learnFromCorrection(tenantId, companyId, transactionId, userCategory, userSubcategory) {
        try {
            const transaction = await prisma.bankTransaction.findFirst({
                where: { id: transactionId, tenantId }
            });
     ...
     */
    static async getCategorizationStats(tenantId, companyId) {
