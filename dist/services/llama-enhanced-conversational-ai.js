...
export class LlamaEnhancedConversationalAI {
    ollama;
    hfInference;
    modelName = process.env.LLAMA_DEFAULT_MODEL || 'gemma:4b'; // Start with faster model
    advancedModelName = process.env.LLAMA_ADVANCED_MODEL || process.env.LLAMA_DEFAULT_MODEL || 'llama3.1:8b'; // Keep advanced for complex tasks
    responseCache = new Map();
    defaultTtlMs = 60_000; // 60 seconds cache TTL
    ...
    async initializeModels() {
        try {
            ...
            try {
                ...
                const suitableModel = availableModels.find(model => model.includes('gemma') ||
    ...
    async warmUpModel() {
        ...
        await this.withTimeout(this.ollama.generate({
            model: this.modelName,
            prompt: 'ok',
            keep_alive: '30m',
            options: {
    ...
    async processNaturalLanguageInput(prompt, context, options) {
        try {
...
Remember: You're not just answering a question, you're helping a human being navigate their business journey.`;
    ...
    determineModelComplexity(prompt, context, forcedMode) {
        if (forcedMode === 'fast')
            return this.modelName;
        if (forcedMode === 'accurate')
            return this.advancedModelName;
        ...
        const isComplex = complexKeywords.some(keyword => prompt.toLowerCase().includes(keyword)) || context.financialContext.transactionVolume > 1000;
        return isComplex ? this.advancedModelName : this.modelName;
    }
    async generateResponse(prompt, model, opts) {
    ...
    async generateMockResponse(prompt, opts) {
    ...
    async generateWithHuggingFace(prompt, opts) {
        try {
            // Fallback to Hugging Face API
            const response = await this.withTimeout(this.hfInference.textGeneration({
    ...
    parseLlamaResponse(response, context) {
        try {
    ...
    parseTextResponse(response, context) {
    ...
    extractEntities(response) {
        ...
        if (amountMatches) {
            entities.amounts = amountMatches.map(amount => parseFloat(amount.replace(/[$,]/g, '')));
        }
        // Extract dates
    ...
    getImmediateResponse(prompt, context) {
        const lowerPrompt = prompt.toLowerCase();
        const financialMetrics = context.financialContext?.keyMetrics || {};
        const expenses = financialMetrics.totalExpenses || 0;
        const revenue = financialMetrics.totalRevenue || 0;
    ...
    generateQuickExpenseResponse(prompt, context) {
        const financialMetrics = context.financialContext?.keyMetrics || {};
        const expenses = financialMetrics.totalExpenses || 0;
        const revenue = financialMetrics.totalRevenue || 0;
    ...
    async getInstantResponse(prompt, context) {
    ...
    getGenericInstantResponse(prompt, context) {
        const financialMetrics = context.financialContext?.keyMetrics || {};
        const expenses = financialMetrics.totalExpenses || 0;
        const revenue = financialMetrics.totalRevenue || 0;
        const profit = revenue - expenses;
    ...
    async getExpenseAnalysisResponse(prompt, context, analysis) {
        const realExpenseData = this.getRealExpenseData(context);
...
• Configure data retention policies`,
    ...
    getRevenueAnalysisResponse(prompt, context, analysis) {
        const financialMetrics = context.financialContext?.keyMetrics || {};
        const revenue = financialMetrics.totalRevenue || 0;
        const timePeriod = analysis.timePeriod || 'current month';
        if (revenue === 0) {
    ...
    getFinancialGeneralResponse(prompt, context, analysis) {
...
${isProfitable ? '✅ Your business is profitable and sustainable' : '🚨 URGENT: Your business is unsustainable without immediate revenue generation'}`,
    ...
    getSystemInfoResponse(prompt, context) {
        return {
...
What would you like to know more about?`,
    ...
    getHelpSupportResponse(prompt, context) {
        return {
...
Just ask me anything in natural language - I understand context and can provide specific, actionable answers based on your actual data.`,
    ...
    getWiseEmotionalResponse(prompt, context) {
        const financialMetrics = context.financialContext?.keyMetrics || {};
        const revenue = financialMetrics.totalRevenue || 0;
        const expenses = financialMetrics.totalExpenses || 0;
        const profit = revenue - expenses;
...
**You're not alone in this journey. Take it one step at a time, and remember that every successful entrepreneur has faced moments like this.**`,
    ...
    getBusinessAnalysisResponse(prompt, context, analysis) {
        const revenue = analysis.revenue;
        const expenses = analysis.expenses;
        const profit = revenue - expenses;
        return {
...
**What specific aspect of your business would you like to explore?**`,
    ...
    getIntelligentFallbackResponse(prompt, context, analysis) {
        const revenue = analysis.revenue;
        const expenses = analysis.expenses;
        const profit = analysis.profit;
        const lowerPrompt = prompt.toLowerCase();
...
Could you rephrase your question or ask me something specific about your finances?`,
    ...
    isGeneralKnowledgeQuestion(text) {
    ...
    handlePersonalQuestion(prompt, context, analysis) {
...
What would you like to explore about your business?`,
    ...
    extractPersonalAskingAbout(text) {
        // Extract what personal information they're asking about
        const patterns = [
    ...
    handleGeneralKnowledgeQuestion(prompt, context, analysis) {
        const lowerPrompt = prompt.toLowerCase();
    ...
    **Your Current Financial Status:**
    ...
    handleFinancialHumor(prompt, context, analysis) {
        const lowerPrompt = prompt.toLowerCase();
        const financialMetrics = context.financialContext?.keyMetrics || {};
        const revenue = financialMetrics.totalRevenue || 0;
        const expenses = financialMetrics.totalExpenses || 0;
    ...
    **Your Financial Status:**
    ...
    handleFinancialAdvice(prompt, context, analysis) {
        const lowerPrompt = prompt.toLowerCase();
        const financialMetrics = context.financialContext?.keyMetrics || {};
        const revenue = financialMetrics.totalRevenue || 0;
        const expenses = financialMetrics.totalExpenses || 0;
    ...
    **📈 Next Steps:**
    ...
    handleFinancialConcepts(prompt, context, analysis) {
        const lowerPrompt = prompt.toLowerCase();
    ...
    **Practical Application**: Understanding ${concept} helps you make better financial decisions for your business.`;
    ...
    handleEntertainmentQuestion(prompt, context, analysis) {
        const lowerPrompt = prompt.toLowerCase();
    ...
    **Your Business Status:**
    ...
    extractEntertainmentAskingAbout(text) {
        // Extract what entertainment they're asking about
        const patterns = [
    ...
    handlePredictiveAnalysis(prompt, context, analysis) {
        const revenue = analysis.revenue;
        const expenses = analysis.expenses;
        const profit = analysis.profit;
        const burnRate = expenses;
...
`}`,
    ...
    handleComparativeAnalysis(prompt, context, analysis) {
        const revenue = analysis.revenue;
        const expenses = analysis.expenses;
        const profit = analysis.profit;
        const profitMargin = revenue > 0 ? (profit / revenue) * 100 : 0;
...
• ${profit > 0 ? 'Profitable operations' : 'Growth-focused investment'}`,
    ...
    getAccountNameResponse(prompt, context) {
...
This is your company account name. You can see it displayed in the top-right corner of your dashboard in the account dropdown.`,
    ...
    getUrgentResponse(prompt, context, revenue, expenses, profit) {
        return {
...
Your burn rate is $${expenses.toLocaleString()}/month with $${revenue.toLocaleString()} revenue. This is unsustainable.`,
    ...
    getActionableResponse(prompt, context, revenue, expenses, profit) {
        return {
...
- Consider reinvestment opportunities`}`,
    ...
    async enhanceResponseWithAI(prompt, context, instantResponse, options) {
        try {
    ...
    async generateAIResponse(prompt, context, options) {
        try {
    ...
    analyzeQuestion(prompt, context) {
        const lowerPrompt = prompt.toLowerCase();
        const financialMetrics = context.financialContext?.keyMetrics || {};
        const revenue = financialMetrics.totalRevenue || 0;
        const expenses = financialMetrics.totalExpenses || 0;
    ...
    analyzeUrgency(text, profit, expenses) {
    ...
    analyzeEmotionalTone(text) {
        ...
        if (frustratedWords.some(word => text.includes(word)))
            return 'frustrated';
        if (urgentWords.some(word => text.includes(word)))
            return 'urgent';
        if (concernedWords.some(word => text.includes(word)))
            return 'concerned';
        if (curiousWords.some(word => text.includes(word)))
            return 'curious';
        return 'neutral';
    }
    analyzeBusinessContext(revenue, expenses, profit) {
        if (revenue === 0 && expenses > 0)
            return 'startup';
        if (profit < -expenses * 0.3)
            return 'crisis';
    ...
    calculateConfidence(text, urgency, complexity, emotionalTone) {
    ...
    performAdvancedCategorization(text, entities, urgency, emotionalTone, businessContext) {
    ...
    extractTimePeriod(text) {
        ...
        if (text.includes('4 year') || text.includes('four year'))
            return 'last 4 years';
        if (text.includes('3 year') || text.includes('three year'))
            return 'last 3 years';
        if (text.includes('2 year') || text.includes('two year'))
            return 'last 2 years';
        if (text.includes('1 year') || text.includes('one year'))
            return 'last year';
    ...
    getActualTimePeriod(expenseData) {
        ...
        const allCurrentMonth = dates.every(d => d.getMonth() === currentMonth && d.getFullYear() === currentYear);
        if (allCurrentMonth) {
            return 'current month';
        }
    ...
    async handleSpecificExpenseCategory(prompt, expenseData, category, timePeriod) {
...
${this.generateCategorySuggestions(expenseData, category)}`,
    ...
    async analyzeExpenseDataForCategory(expenseData, category) {
        ...
        for (const expense of expenseData) {
            ...
            if (amount >= rules.amountThreshold) {
                ...
                const isTypicalAmount = rules.typicalAmounts.some(typical => Math.abs(amount - typical) <= typical * 0.2 // Within 20% of typical amount
                );
                if (isTypicalAmount) {
                    matchScore += 2;
    ...
    generateCategorySuggestions(expenseData, category) {
        ...
        const recurringAmounts = expenseData.filter((expense, index, arr) => arr.filter(e => e.amount === expense.amount).length > 1);
    ...
    getTimePeriod(context) {
        const expenseTransactions = context.financialContext?.expenseTransactions || [];
    ...
    generateExpenseCategories(totalExpenses) {
    ...
    generateBusinessForecastResponse(context) {
        const financialMetrics = context.financialContext?.keyMetrics || {};
        const revenue = financialMetrics.totalRevenue || 0;
        const expenses = financialMetrics.totalExpenses || 0;
        const profit = revenue - expenses;
...
${forecasts.slice(0, 6).map(f => `• **${f.month}**: Revenue $${f.revenue.toLocaleString()}, Expenses $${f.expenses.toLocaleString()}, Net ${f.profit >= 0 ? '+' : ''}$${f.profit.toLocaleString()} (${Math.round(f.confidence * 100)}% confidence)`).join('\n')}
...
• ${totalYearProfit < 0 ? 'Consider cost reduction measures' : 'Reinvest profits for sustainable growth'}`,
    ...
    generateFinancialAnalysisResponse(context) {
        const financialMetrics = context.financialContext?.keyMetrics || {};
        const revenue = financialMetrics.totalRevenue || 0;
        const expenses = financialMetrics.totalExpenses || 0;
        const profit = revenue - expenses;
...
• Consider business model adjustments`}`,
    ...
    async analyzeFinancialTrends(context) {
...
Remember: Output ONLY valid JSON, no other text.`;
    ...
    async processDocumentQuery(documentContent, query, context) {
...
let llamaEnhancedConversationalAIInstance = null;
