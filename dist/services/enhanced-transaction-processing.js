...
export class EnhancedTransactionProcessingService {
    ...
    async processReceipt(imageUrl, companyId) {
        try {
    ...
    async simulateOCRProcessing(imageUrl) {
    ...
    async parseReceiptWithAI(extractedText, companyId) {
        const aiContext = {
            userId: 'demo-user-id',
            companyId,
            tenantId: 'demo-tenant-id',
    ...
    async generateSmartInvoice(customerId, items, companyId, templateId) {
        try {
    ...
    async findVendorMatch(vendorName, companyId, context) {
        try {
    ...
    parseVendorMatches(message, existingVendors) {
        const matches = [];
        ...
        for (const line of lines) {
            if (line.includes('Vendor:') && line.includes('Confidence:')) {
                ...
                if (parts.length >= 5) {
                    ...
                    const existingVendor = existingVendors.find(v => v.name.toLowerCase().includes(vendorName.toLowerCase()));
                    matches.push({
    ...
    async getDefaultVendorMatches(vendorName, companyId) {
        const existingVendors = await prisma.vendor.findMany({
            where: {
                companyId,
                name: {
    ...
    async findCustomerMatch(customerName, companyId, context) {
        try {
    ...
    parseCustomerMatches(message, existingCustomers) {
        const matches = [];
        const lines = message.split('\n');
        for (const line of lines) {
            if (line.includes('Customer:') && line.includes('Confidence:')) {
                ...
                if (parts.length >= 5) {
                    ...
                    const reasoning = parts[4].replace('Reasoning:', '').trim();
                    const existingCustomer = existingCustomers.find(c => c.name.toLowerCase().includes(customerName.toLowerCase()));
                    matches.push({
    ...
    async getDefaultCustomerMatches(customerName, companyId) {
        const existingCustomers = await prisma.customer.findMany({
            where: {
                companyId,
                name: {
    ...
    async analyzeTransactionPatterns(companyId, periodDays = 90) {
        try {
    ...
    analyzeTimingPatterns(transactions) {
        ...
        // Find busiest days
        const busiestDay = Array.from(dayCounts.entries()).reduce((a, b) => dayCounts.get(a[0]) > dayCounts.get(b[0]) ? a : b);
        if (busiestDay[1] >= 3) {
    ...
    async getTransactionStats(companyId, periodDays = 30) {
        const endDate = new Date();
        const startDate = new Date(endDate.getTime() - periodDays * 24 * 60 * 60 * 1000);
        const transactions = await prisma.transaction.findMany({
            where: {
