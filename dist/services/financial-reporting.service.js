...
export class FinancialReportingService {
    async generateBalanceSheet(companyId, period, previousPeriod) {
    ...
    async generateIncomeStatement(companyId, period, previousPeriod) {
        const periodStart = new Date(period);
        const periodEnd = new Date(periodStart.getFullYear(), periodStart.getMonth() + 1, 0);
        // Get chart of accounts
        const accounts = await prisma.account.findMany({
    ...
    async generateCashFlowStatement(companyId, period, previousPeriod) {
        const periodStart = new Date(period);
        const periodEnd = new Date(periodStart.getFullYear(), periodStart.getMonth() + 1, 0);
        // Get account balances for current period
        const currentBalances = await this.getAccountBalances(companyId, periodStart, periodEnd);
    ...
    async getAccountBalances(companyId, startDate, endDate) {
        ...
        for (const entry of journalEntries) {
            for (const line of entry.lines) {
                ...
                if (account) {
                    ...
                    const debit = typeof line.debit?.toNumber === 'function'
                        ? line.debit.toNumber()
                        : typeof line.debit === 'object' && line.debit !== null && 'toString' in line.debit
                            ? parseFloat(line.debit.toString())
                            : line.debit;
                    const credit = typeof line.credit?.toNumber === 'function'
                        ? line.credit.toNumber()
                        : typeof line.credit === 'object' && line.credit !== null && 'toString' in line.credit
                            ? parseFloat(line.credit.toString())
                            : line.credit;
    ...
    async getFinancialRatios(companyId, period) {
