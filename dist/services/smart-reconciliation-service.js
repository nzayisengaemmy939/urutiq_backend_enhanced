...
export class SmartReconciliationService {
     ...
     */
    static async findMatches(tenantId, companyId, bankTransactionId) {
        ...
        const exactAmountMatches = await this.findExactAmountMatches(tenantId, companyId, bankTransaction);
        matches.push(...exactAmountMatches);
        // Rule 2: Reference number matches
        const referenceMatches = await this.findReferenceMatches(tenantId, companyId, bankTransaction);
        matches.push(...referenceMatches);
        // Rule 3: Recurring transaction matches
        const recurringMatches = await this.findRecurringMatches(tenantId, companyId, bankTransaction);
     ...
     */
    static async findExactAmountMatches(tenantId, companyId, bankTransaction) {
     ...
     */
    static async findReferenceMatches(tenantId, companyId, bankTransaction) {
     ...
     */
    static async findRecurringMatches(tenantId, companyId, bankTransaction) {
        ...
        const matchingTransactions = similarTransactions.filter(t => Math.abs(Number(t.amount) - Number(bankTransaction.amount)) <= amountTolerance);
     ...
     */
    static async autoReconcile(tenantId, companyId, bankTransactionId) {
        try {
     ...
     */
    static async runAutoReconciliation(tenantId, companyId, bankAccountId) {
        const whereClause = {
            tenantId,
            bankAccount: { companyId },
            status: 'unreconciled'
     ...
     */
    static async getReconciliationStats(tenantId, companyId) {
