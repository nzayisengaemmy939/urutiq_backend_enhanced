...
export class SeniorAccountCodeManager {
     ...
     */
    async validateAccountUsage(accountCode, usageContext, tenantId, companyId) {
        const result = {
            isValid: true,
            errors: [],
            warnings: [],
        ...
        try {
            ...
            if (!mapping.usage.includes(usageContext)) {
                ...
                const alternatives = this.STANDARD_ACCOUNT_MAPPINGS.filter(m => m.usage.includes(usageContext) && m.type === mapping.type);
     ...
     */
    async getRecommendedAccount(usageContext, tenantId, companyId) {
        try {
            // Find accounts that match the usage context
            const matchingMappings = this.STANDARD_ACCOUNT_MAPPINGS.filter(m => m.usage.includes(usageContext));
     ...
     */
    async auditAccountCodes(tenantId, companyId) {
        try {
     ...
     */
    async createMissingStandardAccounts(tenantId, companyId) {
     ...
     */
    getUsageContext(description, keywords) {
