...
export class EnhancedJournalManagementService {
    ...
    async createJournalEntry(request) {
        ...
        if (request.description && this.isPurchaseTransaction(request.description)) {
            await this.updateInventoryFromJournalEntry(request.description, balancedEntries.reduce((sum, entry) => sum + entry.debit.toNumber(), 0), request.companyId, request.tenantId || 'tenant_demo');
        }
    ...
    async createJournalEntryFromSelectedSuggestions(description, amount, companyId, selectedSuggestions, context, tenantId = 'tenant_demo') {
        try {
            ...
            if (selectedSuggestions.length >= 2) {
                ...
                const cashAccounts = selectedSuggestions.filter(s => s.accountName.toLowerCase().includes('cash') ||
                ...
                const operatingAccounts = selectedSuggestions.filter(s => !s.accountName.toLowerCase().includes('cash') &&
                    !s.accountName.toLowerCase().includes('bank') &&
                    !s.accountName.toLowerCase().includes('checking') &&
                    !s.accountName.toLowerCase().includes('savings'));
    ...
    async generateJournalEntry(description, amount, companyId, context, tenantId = 'tenant_demo') {
        try {
            ...
            if (context?.useSelectedSuggestions && context?.selectedAccountSuggestions?.length > 0) {
                return await this.createJournalEntryFromSelectedSuggestions(description, amount, companyId, context.selectedAccountSuggestions, context, tenantId);
            }
    ...
    async createEnhancedJournalEntries(description, amount, companyId, tenantId) {
        const lowerDesc = description.toLowerCase();
    ...
    async parseAIJournalEntry(message, totalAmount, companyId, tenantId = 'tenant_demo') {
        const entries = [];
    ...
    analyzeTransactionType(description) {
        const lowerDesc = description.toLowerCase();
        // Purchase indicators
    ...
    async createBasicJournalEntry(description, amount, companyId, context, tenantId = 'tenant_demo') {
        try {
            const entries = [];
            const lowerDesc = description.toLowerCase();
    ...
    async getAccountIdFromName(accountName, companyId, tenantId = 'tenant_demo') {
        try {
    ...
    async updateInventoryFromJournalEntry(description, amount, companyId, tenantId = 'tenant_demo') {
        try {
    ...
    isPurchaseTransaction(description) {
        const lowerDesc = description.toLowerCase();
    ...
    parseInventoryFromDescription(description) {
        const lowerDesc = description.toLowerCase();
    ...
    getAccountTypeCategory(accountCode) {
    ...
    // Ensure double-entry bookkeeping
    async ensureDoubleEntry(entries) {
        ...
        if (difference > 0) {
            // Need more credit
            const largestEntry = adjustedEntries.reduce((largest, entry) => entry.credit.toNumber() > largest.credit.toNumber() ? entry : largest);
            ...
            // Need more debit
            const largestEntry = adjustedEntries.reduce((largest, entry) => entry.debit.toNumber() > largest.debit.toNumber() ? entry : largest);
            const index = adjustedEntries.findIndex(entry => entry.id === largestEntry.id);
    ...
    async validateJournalEntry(request) {
        const errors = [];
        const warnings = [];
        const suggestions = [];
    ...
    async getAccountSuggestions(description, amount, companyId, context) {
        try {
            const aiContext = {
                userId: 'demo-user-id',
                companyId,
    ...
    async parseAccountSuggestions(message, companyId, tenantId = 'tenant_demo') {
        const suggestions = [];
        const lines = message.split('\n');
        for (const line of lines) {
    ...
    async getDefaultAccountSuggestions(description, amount, companyId, tenantId = 'tenant_demo') {
        const suggestions = [];
        const lowerDesc = description.toLowerCase();
        try {
            ...
            if (lowerDesc.includes('purchased') || lowerDesc.includes('bought') || lowerDesc.includes('acquired') || lowerDesc.includes('inventory')) {
                ...
                const inventoryAccount = accounts.find(acc => acc.code === '1200' ||
                    acc.name?.toLowerCase().includes('inventory'));
                const bankAccount = accounts.find(acc => acc.code === '13000' ||
            ...
            if (suggestions.length === 0) {
                const commonAccounts = accounts.filter(acc => acc.name?.toLowerCase().includes('receivable') ||
    ...
    async updateLedgerBalances(journalEntryId) {
    ...
    async getLedgerBalances(companyId, startDate, endDate) {
    ...
    // Post journal entry
    async postJournalEntry(journalEntryId, postedBy) {
    ...
    // Void journal entry
    async voidJournalEntry(journalEntryId, voidedBy, reason) {
        const journalEntry = await prisma.journalEntry.findUnique({
            where: { id: journalEntryId },
            include: { lines: true }
        });
    ...
    async getJournalEntries(companyId, startDate, endDate, status) {
        const where = {
            companyId,
            date: {
                gte: startDate,
    ...
    async detectAnomalies(companyId, periodDays = 30) {
    ...
    findDuplicateEntries(journalEntries) {
        const duplicates = [];
        const seen = new Map();
        for (const entry of journalEntries) {
            const entryTotalDebit = entry.lines.reduce((sum, line) => sum + Number(line.debit || 0), 0);
    ...
    async processInventoryFromJournalEntry(journalEntry) {
        try {
    ...
    async createInventoryFromJournalLine(journalEntry, line, account) {
        try {
    ...
    extractProductInfoFromDescription(description) {
        const lowerDesc = description.toLowerCase();
        ...
        if (lowerDesc.includes('phone'))
            ...
            productName = 'Laptop';
        else if (lowerDesc.includes('computer'))
            productName = 'Computer';
        else if (lowerDesc.includes('tablet'))
            productName = 'Tablet';
        else {
