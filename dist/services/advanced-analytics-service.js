...
export class AdvancedAnalyticsService {
     ...
     */
    static async generateFinancialInsights(tenantId, companyId, industry) {
        const insights = [];
        // Get company data
        ...
        insights.push(...await this.generateIndustrySpecificInsights(detectedIndustry, transactions, invoices));
     ...
     */
    static async generateIndustrySpecificInsights(industry, transactions, invoices) {
        const insights = [];
     ...
     */
    static generateRetailInsights(transactions, invoices) {
        const insights = [];
        ...
        const inventoryExpenses = transactions.filter(t => t.category?.toLowerCase().includes('inventory') ||
     ...
     */
    static generateSaaSInsights(transactions, invoices) {
        const insights = [];
        // MRR (Monthly Recurring Revenue) analysis
        const recurringRevenue = transactions.filter(t => t.description?.toLowerCase().includes('subscription') ||
        ...
        const marketingExpenses = transactions.filter(t => t.category?.toLowerCase().includes('marketing') ||
     ...
     */
    static generateConsultingInsights(transactions, invoices) {
        const insights = [];
        // Utilization rate analysis
        const projectRevenue = transactions.filter(t => t.description?.toLowerCase().includes('project') ||
     ...
     */
    static generateManufacturingInsights(transactions, invoices) {
        const insights = [];
        // Cost of goods sold analysis
        const materialCosts = transactions.filter(t => t.category?.toLowerCase().includes('materials') ||
     ...
     */
    static generateHealthcareInsights(transactions, invoices) {
        const insights = [];
        // Revenue cycle management
        const insurancePayments = transactions.filter(t => t.description?.toLowerCase().includes('insurance') ||
     ...
     */
    static generateGeneralInsights(transactions, invoices) {
        const insights = [];
        // Cash flow trend
     ...
     * Generate cash flow insights
     */
    static async generateCashFlowInsights(transactions) {
        const insights = [];
     ...
     * Generate expense insights
     */
    static async generateExpenseInsights(transactions, industry) {
        const insights = [];
     ...
     * Generate revenue insights
     */
    static async generateRevenueInsights(invoices, transactions) {
        const insights = [];
     ...
     */
    static async generateAnomalyInsights(transactions) {
        const insights = [];
    ...
    static detectSeasonalPattern(monthlyRevenue) {
    ...
    static categorizeExpenses(transactions) {
        const categories = {};
    ...
    static getIndustryBenchmarks(industry) {
        const benchmarks = {
    ...
    static getTotalRevenue(transactions) {
        return transactions
    ...
    static calculateCOGSRatio(materialCosts, transactions) {
        const totalRevenue = this.getTotalRevenue(transactions);
        return (materialCosts / totalRevenue) * 100;
    }
    static calculateAveragePaymentTime(payments) {
     ...
     */
    static async getIndustryBenchmarks(industry) {
        const benchmarks = [
            {
                industry,
     ...
     */
    static async generateCashFlowForecast(tenantId, companyId, months = 6) {
        const transactions = await prisma.bankTransaction.findMany({
            where: {
                tenantId,
                bankAccount: { companyId }
    ...
    static projectInflow(transactions, monthOffset) {
    ...
    static projectOutflow(transactions, monthOffset) {
    ...
    static getForecastFactors(transactions, monthOffset) {
