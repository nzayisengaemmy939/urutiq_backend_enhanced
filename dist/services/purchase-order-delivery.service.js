...
export class PurchaseOrderDeliveryService {
    ...
    async processDelivery(data) {
        return await prisma.$transaction(async (tx) => {
            ...
            if (totalInventoryValue > 0) {
                try {
                    journalEntry = await this.createInventoryJournalEntry(tx, // Pass transaction context
                    purchaseOrder, totalInventoryValue, totalTaxAmount, data.deliveredDate, data.journalEntryData, data.userId // Pass user ID
                    );
                }
                catch (error) {
     ...
     */
    async createInventoryJournalEntry(tx, // Transaction context
    purchaseOrder, totalInventoryValue, totalTaxAmount, deliveryDate, journalData, userId // Add user ID parameter
    ) {
     ...
     */
    async getOrCreateInventoryAccount(tx, tenantId, companyId) {
     ...
     */
    async getOrCreateAccountsPayableAccount(tx, tenantId, companyId) {
     ...
     */
    async getOrCreateJournalEntryType(tx, tenantId, companyId, category) {
     ...
     */
    async getOrCreateTaxPayableAccount(tx, tenantId, companyId) {
     ...
     */
    async getDeliveryStatus(purchaseOrderId, tenantId) {
        ...
        const totalDelivered = purchaseOrder.receipts.reduce((sum, receipt) => sum + receipt.items.reduce((itemSum, item) => itemSum + Number(item.quantityAccepted), 0), 0);
        return {
            purchaseOrderId,
