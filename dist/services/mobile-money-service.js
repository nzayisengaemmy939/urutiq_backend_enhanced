...
export class MobileMoneyService {
     ...
     */
    static async getProviders(country) {
        if (country) {
            return this.PROVIDERS.filter(provider => provider.country.toLowerCase() === country.toLowerCase() ||
     ...
     */
    static async createAccount(tenantId, companyId, provider, accountNumber, accountName, phoneNumber, currency) {
     ...
     */
    static async getCompanyAccounts(tenantId, companyId) {
     ...
     */
    static async initiatePayment(tenantId, companyId, paymentRequest) {
        try {
     ...
     */
    static async processCallback(provider, externalReference, status, metadata) {
     ...
     */
    static async getTransactionHistory(tenantId, companyId, provider, limit = 50) {
     ...
     * Get account balance
     */
    static async getAccountBalance(tenantId, companyId, provider, phoneNumber) {
        const account = await prisma.mobileMoneyAccount.findFirst({
     ...
     */
    static calculateFees(amount, provider, transactionType) {
        const providerConfig = this.PROVIDERS.find(p => p.id === provider);
        if (!providerConfig)
            return 0;
        const feeRate = providerConfig.fees[transactionType] || 0;
        return amount * (feeRate / 100);
    }
    /**
     * Simulate API call to provider
     */
    static async callProviderAPI(provider, data) {
