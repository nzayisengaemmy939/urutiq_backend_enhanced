...
export class GamificationService {
    ...
    async initializeUserAchievements(companyId, userId) {
        try {
            // Get achievement templates
            const templates = await this.getAchievementTemplates();
            // Create achievements for user
            const achievements = await Promise.all(templates.map(template => prisma.achievement.create({
                data: {
                    companyId,
                    userId,
    ...
    async getUserAchievements(companyId, userId) {
    ...
    async checkAndUpdateAchievements(companyId, userId) {
        try {
    ...
    async initializeUserProgress(companyId, userId) {
    ...
    async getUserProgress(companyId, userId) {
    ...
    async updateUserProgress(companyId, userId, points) {
    ...
    async updateStreak(companyId, userId) {
    ...
    async joinChallenge(challengeId, companyId, userId) {
    ...
    async updateChallengeProgress(challengeId, userId, currentValue) {
    ...
    async updateLeaderboard(leaderboardId, companyId) {
        ...
        await Promise.all(entries.map(entry => prisma.leaderboardEntry.upsert({
            where: {
    ...
    async getLeaderboards(companyId) {
    ...
    // Gamification Stats
    async getGamificationStats(companyId, userId) {
        const progress = await this.getUserProgress(companyId, userId);
        const achievements = await this.getUserAchievements(companyId, userId);
        ...
        const leaderboardRankings = await Promise.all(leaderboards.map(async (leaderboard) => {
    ...
    async getUserFinancialData(companyId, userId) {
    ...
    async calculateAchievementProgress(achievement, userData) {
    ...
    async calculateLeaderboardValue(type, companyId, userId) {
        switch (type) {
            case 'points':
                const progress = await this.getUserProgress(companyId, userId);
    ...
    async getNextMilestone(companyId, userId) {
        const achievements = await this.getUserAchievements(companyId, userId);
    ...
    mapAchievementFromDB(dbAchievement) {
        return {
            id: dbAchievement.id,
    ...
    mapUserProgressFromDB(dbProgress) {
        return {
            id: dbProgress.id,
    ...
    mapChallengeFromDB(dbChallenge) {
        return {
            id: dbChallenge.id,
    ...
    mapChallengeParticipantFromDB(dbParticipant) {
        return {
            id: dbParticipant.id,
    ...
    mapLeaderboardFromDB(dbLeaderboard) {
        return {
            id: dbLeaderboard.id,
    ...
    mapLeaderboardEntryFromDB(dbEntry) {
        return {
            id: dbEntry.id,
