...
export class ConversationalAccountingService {
     ...
     */
    static async processMessage(tenantId, companyId, userId, message, sessionId, conversationHistory = []) {
        try {
     ...
     */
    static async executeAction(tenantId, action) {
        try {
            switch (action.type) {
                case 'create_invoice':
     ...
     */
    static async getConversationHistory(tenantId, sessionId, limit = 50) {
        try {
            // TODO: Replace with actual Prisma model when available
            const messages = []; // Temporary mock data
     ...
     * Get conversation context
     */
    static async getConversationContext(tenantId, companyId, userId, sessionId) {
        try {
     ...
     * Prepare context for AI
     */
    static prepareAIContext(context, conversationHistory, currentMessage) {
        const contextString = `
...
`;
     ...
     * Call Ollama AI model
     */
    static async callOllamaAI(context, message) {
        try {
     ...
     * Parse AI response and determine actions
     */
    static async parseAIResponse(aiResponse, context) {
        try {
     ...
     */
    static async storeConversation(tenantId, sessionId, userId, userMessage, aiResponse) {
        try {
            // TODO: Replace with actual Prisma model when available
            // Store user message
     ...
     * Get recent transactions
     */
    static async getRecentTransactions(tenantId, companyId) {
        try {
     ...
     * Get financial summary
     */
    static async getFinancialSummary(tenantId, companyId) {
        try {
            const invoices = await prisma.invoice.findMany({
                where: { tenantId, companyId }
            });
            ...
            const overdueInvoices = invoices.filter(inv => inv.status === 'sent' && inv.dueDate && new Date(inv.dueDate) < new Date()).length;
            return {
                totalRevenue,
                totalExpenses,
                netIncome,
     ...
     */
    static async simulateOllamaResponse(context, message) {
...
What would you like to do today? Just tell me in your own words!`;
     ...
     */
    static async createInvoiceFromConversation(tenantId, data) {
        try {
     ...
     */
    static async createExpenseFromConversation(tenantId, data) {
        try {
     ...
     */
    static async createCustomerFromConversation(tenantId, data) {
        try {
     ...
     */
    static async generateReportFromConversation(tenantId, data) {
        try {
     ...
     */
    static async analyzeDataFromConversation(tenantId, data) {
        try {
