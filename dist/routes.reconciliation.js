...
export function mountReconciliationRoutes(router) {
    ...
    // Create rule
    router.post('/reconciliation/rules', asyncHandler(async (req, res) => {
        const data = req.body || {};
    ...
    // Update rule
    router.put('/reconciliation/rules/:id', asyncHandler(async (req, res) => {
    ...
    // Delete rule
    router.delete('/reconciliation/rules/:id', asyncHandler(async (req, res) => {
        const id = String(req.params.id);
        await prisma.bankReconciliationRule.delete({ where: { id } });
        res.json({ ok: true });
    }));
    // Reconciliation status (aggregated)
    router.get('/reconciliation/status', asyncHandler(async (req, res) => {
        ...
        const whereBase = {};
        if (bankAccountId)
            whereBase.bankAccountId = bankAccountId;
        ...
        if (companyId) {
            whereBase.bankAccount = { ...whereBase.bankAccount, companyId };
        }
    ...
    router.post('/reconciliation/match', asyncHandler(async (req, res) => {
        ...
        res.json({ success: true, bankTransaction: { id: updated.id, isReconciled: updated.isReconciled, reconciledAt: updated.reconciledAt } });
    }));
    // Unmatch a bank transaction (minimal update)
    router.post('/reconciliation/unmatch', asyncHandler(async (req, res) => {
