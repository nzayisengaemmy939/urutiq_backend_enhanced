...
import { mountJournalHubRoutes } from "./routes.journal-hub";
import { mountSupplierPortalRoutes } from "./routes.supplier-portal";
import { mountTransactionRoutes } from "./routes.transactions";
import { mountSalesRoutes } from "./routes.sales";
import { mountSalesAccountingRoutes } from "./routes/sales-accounting";
import { mountPurchaseRoutes } from "./routes.purchases";
import { mountPurchaseOrderRoutes } from "./routes.purchase-orders";
import { mountAccountsPayableRoutes } from "./routes.accounts-payable";
import { paymentsRouter } from "./routes.payments";
import purchaseOrderPDFRouter from "./routes.purchase-order-pdf";
import goodReceiptPDFRouter from "./routes.good-receipt-pdf";
import { mountExpenseRoutes } from "./routes.expenses";
import billsRouter from "./routes.bills";
import { mountApprovalRoutes } from "./routes.approvals";
import { mountUnifiedApprovalRoutes } from "./routes.unified-approvals";
import { mountImportShipmentRoutes } from "./routes.import-shipments";
import { mountBankingRoutes } from "./routes.banking";
...
import { mountMappingRoutes } from "./routes.mappings";
import { mountAiRoutes } from "./routes.ai";
import { mountEnhancedAIRoutes } from "./routes.ai-enhanced";
import { mountAIConfigRoutes } from "./routes.ai-config";
import aiRoutes from "./routes/ai-routes";
import { mountConversationalParserRoutes } from "./routes.conversational-parser";
import { mountAnalyticsRoutes } from "./routes.analytics";
...
import { mountCollaborationRoutes } from "./routes.collaboration";
import { mountWorkspaceRoutes } from "./routes.workspaces";
import { mountAdminRoutes } from "./routes.admin";
import enhancedFinancialReportsRouter from "./routes/enhanced-financial-reports";
import enhancedConversationalAIRouter from "./routes/enhanced-conversational-ai";
import enhancedBankIntegrationRouter from './routes/enhanced-bank-integration';
import enhancedJournalManagementRouter from "./routes/enhanced-journal-management";
import aiFinancialCoachRouter from "./routes/ai-financial-coach";
import enhancedTransactionProcessingRouter from './routes/enhanced-transaction-processing';
import llamaAIRouter from "./routes/llama-ai";
import periodCloseRouter from './routes/period-close';
import bankRulesRouter from './routes/bank-rules';
import voiceRouter from './routes/voice-enabled-accounting';
import gamificationRouter from './routes/gamification';
import inventoryAiRouter from './routes/inventory-ai';
import { errorHandler, ApiError, asyncHandler } from "./errors";
import { validateBody, schemas, budgetSchemas } from "./validate";
import { createRateLimiters, securityConfig } from "./security";
...
import { mountReconciliationRoutes } from './routes.reconciliation';
...
import threeWayMatchRouter from './routes/three-way-match';
import fixedAssetsRouter from './routes/fixed-assets';
import revenueRecognitionRouter from './routes/revenue-recognition';
import currencyAlertsRouter from './routes/currency-alerts';
import financialReportingRouter from './routes/financial-reporting';
import taxCalculationRouter from './routes/tax-calculation';
import inventoryManagementRouter from './routes/inventory-management';
import budgetManagementRouter from './routes/budget-management';
import customReportBuilderRouter from './routes/custom-report-builder';
import seedRouter from './routes/seed';
import securityRouter from './routes/security';
import helpRouter from './routes/help';
import filesRouter from './routes/files';
import tutorialVideosRouter from './routes/tutorial-videos';
...
app.get('/api/images/:fileId', asyncHandler(async (req, res) => {
...
app.options("/api/tutorial-videos/stream/:videoId", (req, res) => {
...
app.options("/api/documents/stream/:documentId", (req, res) => {
    res.setHeader('Access-Control-Allow-Origin', '*');
    res.setHeader('Access-Control-Allow-Methods', 'GET, OPTIONS, HEAD');
    res.setHeader('Access-Control-Allow-Headers', 'Range, Content-Range, Content-Length, Content-Type, x-tenant-id, x-company-id, Authorization, Origin, X-Requested-With, Accept');
    res.setHeader('Access-Control-Expose-Headers', 'Content-Range, Accept-Ranges, Content-Length, Content-Type, Content-Disposition');
...
app.get("/api/documents/stream/:documentId", async (req, res) => {
    const documentId = req.params.documentId;
    const tenantId = req.query.tenantId || 'tenant_demo';
    const companyId = req.query.companyId || 'seed-company-1';
...
app.get("/api/tutorial-videos/stream/:videoId", async (req, res) => {
    const videoId = req.params.videoId;
    const tenantId = req.query.tenantId || 'tenant_demo';
    const companyId = req.query.companyId || 'cmg0qxjh9003nao3ftbaz1oc1';
    const authToken = req.query.token;
    try {
        // Set CORS headers BEFORE any response is sent
        res.setHeader('Access-Control-Allow-Origin', '*');
...
app.post("/api/auth/demo-token", authLimiter, asyncHandler(async (req, res) => {
    const secret = process.env.JWT_SECRET || "dev-secret";
    const tenantId = String(req.header("x-tenant-id") || "tenant_demo");
    const roles = req.body?.roles || ["admin"];
    // Validate roles
    const validRoles = ["admin", "accountant", "auditor", "employee"];
    const validUserRoles = roles.filter(role => validRoles.includes(role));
...
app.get("/reports/financial", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["accountant"]), asyncHandler(async (_req, res) => {
    res.json({
        ok: true,
...
app.get("/api/companies", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["admin", "accountant"]), asyncHandler(async (req, res) => {
    const { country, currency, q, status } = req.query;
    ...
    if (total === 0 && process.env.NODE_ENV !== 'production') {
        ...
        for (const c of defaults) {
            await prisma.company.upsert({
                where: { id: c.id },
                update: {},
                create: { tenantId: req.tenantId, ...c }
            });
        }
...
app.post("/api/companies", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["admin"]), validateBody(schemas.companyCreate), asyncHandler(async (req, res) => {
    const { name, industry, taxId, country, currency, fiscalYearStart, email, phone, website, address, city, state, postalCode, description, employees, foundedYear, status, businessType, registrationNumber, timezone, settings } = req.body;
...
app.put("/api/companies/:id", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["admin"]), validateBody(schemas.companyUpdate), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { name, industry, taxId, country, currency, fiscalYearStart, email, phone, website, address, city, state, postalCode, description, employees, foundedYear, status, businessType, registrationNumber, timezone, settings } = req.body;
    // Handle nested address structure
    let addressData = {
...
app.patch("/api/companies/:id/archive", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["admin"]), asyncHandler(async (req, res) => {
    const { id } = req.params;
    // Verify company exists and belongs to tenant
    const existing = await prisma.company.findFirst({
        where: { id, tenantId: req.tenantId }
...
app.patch("/api/companies/:id/unarchive", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["admin"]), asyncHandler(async (req, res) => {
    const { id } = req.params;
    // Verify company exists and belongs to tenant
    const existing = await prisma.company.findFirst({
        where: { id, tenantId: req.tenantId }
...
app.post("/api/companies/:id/recover", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["admin"]), asyncHandler(async (req, res) => {
    const { id } = req.params;
...
app.get("/api/companies/:id/recovery-status", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["admin"]), asyncHandler(async (req, res) => {
    const { id } = req.params;
    try {
...
app.delete("/api/companies/:id", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["admin"]), asyncHandler(async (req, res) => {
    const { id } = req.params;
...
app.get("/api/companies/:id", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["admin", "accountant", "auditor"]), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const company = await prisma.company.findFirst({
        where: { id, tenantId: req.tenantId },
        include: {
...
app.post("/api/companies/:id/logo", authMiddleware(process.env.JWT_SECRET || "dev-secret"), requireRoles(["admin"]), upload.single('logo'), (err, req, res, next) => {
...
app.get('/api/pdf/:filename', async (req, res) => {
    ...
    const tenantId = req.query.tenantId || req.headers['x-tenant-id'] || 'tenant_demo';
...
app.get('/api/budgets', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), asyncHandler(async (req, res) => {
...
app.post('/api/budgets', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), validateBody(budgetSchemas.budgetCreate), asyncHandler(async (req, res) => {
...
app.put('/api/budgets/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), validateBody(budgetSchemas.budgetUpdate), asyncHandler(async (req, res) => {
    const { id } = req.params;
    ...
    if (alertThreshold !== undefined)
        updateData.alertThreshold = alertThreshold ? parseFloat(alertThreshold) : null;
...
app.delete('/api/budgets/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), asyncHandler(async (req, res) => {
    const { id } = req.params;
    // Verify budget exists and belongs to tenant
    const existing = await prisma.budget.findFirst({
        where: { id, tenantId: req.tenantId }
...
app.get('/api/budgets/analytics', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), asyncHandler(async (req, res) => {
    const { companyId } = req.query;
    const whereClause = {
        tenantId: req.tenantId
    };
...
app.get('/api/budget-analysis', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), asyncHandler(async (req, res) => {
