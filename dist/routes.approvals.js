...
export function mountApprovalRoutes(router) {
    ...
    router.post('/approval-workflows', validateBody(approvalSchemas.workflow), async (req, res) => {
        const data = req.body;
        try {
    ...
    router.put('/approval-workflows/:id', validateBody(approvalSchemas.workflow), async (req, res) => {
        const { id } = req.params;
        const data = req.body;
        try {
    ...
    router.get('/approvals', async (req, res) => {
        ...
        const approverId = req.query.approverId || undefined;
        try {
            const where = {
                tenantId: req.tenantId,
                companyId,
    ...
    router.post('/approvals', validateBody(approvalSchemas.approval), async (req, res) => {
        const data = req.body;
        try {
    ...
    router.put('/approvals/:id/action', validateBody(approvalSchemas.approvalAction), async (req, res) => {
        const { id } = req.params;
        const data = req.body;
        try {
            const approval = await prisma.approval.findFirst({
    ...
    router.get('/approvals/entity/:entityType/:entityId', async (req, res) => {
    ...
    router.post('/approvals/bulk', async (req, res) => {
        ...
        try {
            ...
            const approvals = await Promise.all(approvers.map((approver, index) => prisma.approval.create({
                data: {
                    tenantId: req.tenantId,
    ...
    async function checkWorkflowCompletion(workflowId, entityType, entityId) {
        try {
            const workflow = await prisma.approvalWorkflow.findFirst({
                where: { id: workflowId }
            });
