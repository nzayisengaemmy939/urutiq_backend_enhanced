...
function generateTOTP(secretBase32, timeStep = 30, digits = 6, algo = 'sha1') {
...
function signAccessToken(payload, expiresIn = '30m') {
    ...
    const token = signDemoToken(payload, secret, expiresIn);
    return token;
}
...
export function mountAuthRoutes(router) {
    ...
    router.get('/auth/users', authLimiter, asyncHandler(async (req, res) => {
    ...
    router.post('/auth/users', authLimiter, asyncHandler(async (req, res) => {
        const tenantId = String(req.header('x-tenant-id') || 'tenant_demo');
    ...
    router.put('/auth/users/:userId', authLimiter, asyncHandler(async (req, res) => {
        const tenantId = String(req.header('x-tenant-id') || 'tenant_demo');
    ...
    router.delete('/auth/users/:userId', authLimiter, asyncHandler(async (req, res) => {
        const tenantId = String(req.header('x-tenant-id') || 'tenant_demo');
        const { userId } = req.params;
        // Check if user exists
        const existingUser = await prisma.appUser.findFirst({
    ...
    router.post('/auth/login', authLimiter, asyncHandler(async (req, res) => {
        ...
        if (user.mfaEnabled && user.mfaSecret) {
            ...
            const challenge = signDemoToken({ sub: user.id, tenantId, purpose: 'mfa', ts: Date.now() }, process.env.JWT_SECRET || 'dev-secret', '5m');
    ...
    router.post('/auth/refresh', authLimiter, asyncHandler(async (req, res) => {
        ...
        if (!token || token.revokedAt)
            throw new ApiError(401, 'invalid_token', 'Refresh token invalid');
    ...
    router.post('/auth/logout', authLimiter, asyncHandler(async (req, res) => {
        const tenantId = String(req.header('x-tenant-id') || 'tenant_demo');
        const { refreshToken } = (req.body || {});
        if (!refreshToken)
            throw new ApiError(400, 'invalid_request', 'refreshToken is required');
    ...
    router.post('/auth/mfa/setup/start', authLimiter, asyncHandler(async (req, res) => {
        const tenantId = String(req.header('x-tenant-id') || 'tenant_demo');
        const { userId } = req;
        if (!userId)
    ...
    router.post('/auth/mfa/setup/verify', authLimiter, asyncHandler(async (req, res) => {
        const tenantId = String(req.header('x-tenant-id') || 'tenant_demo');
        const { userId } = req;
        if (!userId)
            throw new ApiError(401, 'unauthorized', 'Login required');
    ...
    router.post('/auth/mfa/disable', authLimiter, asyncHandler(async (req, res) => {
        const tenantId = String(req.header('x-tenant-id') || 'tenant_demo');
        const { userId } = req;
        if (!userId)
            throw new ApiError(401, 'unauthorized', 'Login required');
        ...
        const { hash } = hashPassword(password, user.passwordSalt);
        if (hash !== user.passwordHash)
            throw new ApiError(401, 'invalid_credentials', 'Password incorrect');
    ...
    router.post('/auth/mfa/backup/regenerate', authLimiter, asyncHandler(async (req, res) => {
        const tenantId = String(req.header('x-tenant-id') || 'tenant_demo');
        const { userId } = req;
        if (!userId)
            throw new ApiError(401, 'unauthorized', 'Login required');
        ...
        if (!user || !user.mfaEnabled)
            throw new ApiError(400, 'mfa_not_enabled', 'MFA not enabled');
        const { plain, hashed } = generateBackupCodes(10);
    ...
    router.post('/auth/mfa/login/verify', authLimiter, asyncHandler(async (req, res) => {
        const tenantId = String(req.header('x-tenant-id') || 'tenant_demo');
        ...
        try {
            ...
            if (!payload?.sub || payload?.tenantId !== tenantId)
                throw new Error('bad');
            ...
            if (!ok)
                throw new ApiError(401, 'invalid_token', 'Invalid MFA code');
            const roles = [user.role];
            const accessToken = signAccessToken({ sub: user.id, tenantId, roles }, '30m');
            const refreshToken = crypto.randomBytes(32).toString('hex');
            const expiresAt = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000);
