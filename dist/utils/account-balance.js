 ...
 */
export async function updateAccountBalances(tenantId, companyId, journalEntryId) {
    try {
 ...
 */
export async function recalculateAllAccountBalances(tenantId, companyId) {
    try {
 ...
 */
export async function getAccountBalance(tenantId, companyId, accountId) {
    try {
 ...
 */
export async function getAccountBalances(tenantId, companyId, accountIds) {
    try {
        const accounts = await prisma.account.findMany({
            where: {
                id: { in: accountIds },
        ...
        for (const account of accounts) {
            balances.set(account.id, Number(account.balance || 0));
        }
        return balances;
    }
    catch (error) {
