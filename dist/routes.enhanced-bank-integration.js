...
import { EnhancedBankIntegrationService } from './services/enhanced-bank-integration.service';
...
router.post('/banking/connect', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const bankData = connectBankSchema.parse(req.body);
    const bankConnection = await EnhancedBankIntegrationService.connectBankAccount(req.tenantId, bankData.companyId, bankData);
    res.json({
        success: true,
        data: bankConnection,
...
router.get('/banking/connections', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'viewer']), asyncHandler(async (req, res) => {
    const { companyId } = req.query;
    const connections = await EnhancedBankIntegrationService.getBankConnections(req.tenantId, companyId);
    res.json({
        success: true,
        data: connections
    });
...
router.post('/banking/sync', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const { bankConnectionId, forceSync } = syncBankSchema.parse(req.body);
    const syncResult = await EnhancedBankIntegrationService.syncBankTransactions(bankConnectionId, forceSync);
    res.json({
...
router.post('/banking/forecast', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'viewer']), asyncHandler(async (req, res) => {
    const { companyId, forecastPeriod } = forecastSchema.parse(req.body);
    const forecast = await EnhancedBankIntegrationService.generateCashFlowForecast(req.tenantId, companyId, forecastPeriod);
    res.json({
        success: true,
        data: forecast,
...
router.get('/banking/providers', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const { country } = req.query;
    const providers = await EnhancedBankIntegrationService.getBankProviders(country);
    res.json({
        success: true,
        data: providers
    });
...
router.get('/banking/transactions', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'viewer']), asyncHandler(async (req, res) => {
    const { bankConnectionId, startDate, endDate, status, isReconciled, limit = 100, offset = 0 } = req.query;
...
router.post('/banking/transactions/:transactionId/reconcile', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
...
router.get('/banking/forecasts', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'viewer']), asyncHandler(async (req, res) => {
...
router.get('/banking/forecasts/:forecastId', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'viewer']), asyncHandler(async (req, res) => {
...
router.put('/banking/connections/:connectionId', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
...
router.delete('/banking/connections/:connectionId', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin']), asyncHandler(async (req, res) => {
    const { connectionId } = req.params;
    const connection = await prisma.bankConnection.findUnique({
        where: { id: connectionId, tenantId: req.tenantId }
    });
...
router.get('/banking/balance', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'viewer']), asyncHandler(async (req, res) => {
    const { companyId } = req.query;
    const connections = await EnhancedBankIntegrationService.getBankConnections(req.tenantId, companyId);
    // Get latest balance from each connection
    const balances = await Promise.all(connections.map(async (connection) => {
...
router.get('/banking/categories', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'viewer']), asyncHandler(async (req, res) => {
...
router.post('/banking/transactions/categorize', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
