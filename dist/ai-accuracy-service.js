...
export class AIAccuracyService {
    ...
    async getAccuracyMetrics(tenantId, companyId) {
        try {
    ...
    categorizePattern(pattern) {
        if (pattern.startsWith('amount_'))
            return 'amount';
        if (pattern.startsWith('currency_'))
            return 'currency';
        ...
        if (pattern.startsWith('category_'))
            return 'category';
        if (pattern.startsWith('text_'))
            return 'text';
        return 'other';
    }
    generatePatternImprovements(pattern, analysis) {
        const improvements = [];
        if (pattern.startsWith('amount_')) {
    ...
    async getPatternSuggestions(text) {
        try {
    ...
    async getLearningRecommendations(tenantId, companyId) {
        try {
    ...
    async exportLearningData(tenantId, companyId) {
        try {
            const data = await prisma.aiLearningData.findMany({
                where: { tenantId, companyId },
                orderBy: { createdAt: 'desc' }
    ...
    async clearLearningData(tenantId, companyId) {
        try {
