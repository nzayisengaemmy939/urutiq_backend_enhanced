...
export function mountSalesRoutes(router) {
    function escapePdfText(input) {
        return (input || '').replace(/[\\()]/g, (m) => `\\${m}`);
    }
    async function buildInvoicePdf(tenantId, invoiceId) {
        ...
        return new Promise(async (resolve, reject) => {
            try {
                ...
                const issueDate = inv.issueDate ? new Date(inv.issueDate).toLocaleDateString() : '';
                ...
                for (let i = 0; i < lines.length; i++) {
                    ...
                    doc.fillColor(textColor)
                        .fontSize(9)
                        .text(line.description || 'Item', 60, currentY + 6)
    ...
    router.post('/customers', validateBody(schemas.customerCreate), async (req, res) => {
        const data = req.body;
    ...
    router.post('/customers/:id/activities', async (req, res) => {
        const { id } = req.params;
        const data = req.body;
        try {
    ...
    // Invoices (enhanced)
    router.post('/invoices', validateBody(schemas.invoiceCreate), async (req, res) => {
        // Debug removed
        const { companyId, customerId, invoiceNumber, issueDate, dueDate, currency = 'USD', lines, 
        // Enhanced fields
        subtotal, taxAmount, discountAmount, shippingAmount, notes, terms, footer, paymentTerms, lateFeeRate, deliveryMethod, taxInclusive = false, taxExemptionReason, createdBy } = req.body;
    ...
    router.post('/invoices/:id/post', validateBody(schemas.invoicePostAction), async (req, res) => {
        const { id } = req.params;
        ...
        if (!ar || !revenue)
            return res.status(400).json({ error: 'missing_accounts', message: 'Require mappings for AR and REVENUE' });
        ...
        try {
            enqueueWebhooks('invoice.posted', { invoiceId: invoice.id, companyId: invoice.companyId });
        }
        catch (e) {
            console.error('enqueueWebhooks failed', e);
        }
        res.json(result);
    });
    ...
    router.post('/invoices/:id/send', upload.any(), async (req, res) => {
        try {
            const { id } = req.params;
            ...
            if (!to)
                return res.status(400).json({ error: 'missing_to', message: 'Recipient email (to) is required' });
            ...
            res.json({ ok: true, id: info.messageId || true });
        }
        catch (e) {
    ...
    router.get('/invoices/:id/activity', async (req, res) => {
        const { id } = req.params;
        try {
    ...
    router.post('/invoices/:id/attachments', async (req, res) => {
        const { id } = req.params;
    ...
    // Invoice reminders
    router.get('/invoices/:id/reminders', async (req, res) => {
        const { id } = req.params;
        try {
    ...
    // Create estimate
    router.post('/estimates', validateBody(schemas.estimateCreate), async (req, res) => {
        const { companyId, customerId, estimateNumber, issueDate, expiryDate, currency, notes, terms, lines } = req.body;
        // Resolve tax rates by taxId or taxName, falling back to provided numeric taxRate
        const resolvedLines = await Promise.all(lines.map(async (l) => {
            let effectiveRate = Number(l.taxRate || 0);
            if (l.taxId) {
        ...
        const totals = resolvedLines.map((l) => calcLineTotal(l.quantity ?? 1, l.unitPrice ?? 0, l.effectiveRate ?? 0));
    ...
    router.post('/estimates/:id/convert', async (req, res) => {
        ...
        if (estimate.status === 'converted')
            return res.status(400).json({ error: 'already_converted' });
    ...
    router.get('/estimates/next-number', async (req, res) => {
        const companyId = String(req.query.companyId || '');
    ...
    router.post('/recurring-invoices', validateBody(schemas.recurringInvoiceCreate), async (req, res) => {
        try {
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                const recurring = await tx.recurringInvoice.create({
                    data: {
                        ...
                        nextRunDate,
                        totalAmount: lines.reduce((sum, line) => sum + calcLineTotal(line.quantity, line.unitPrice, line.taxRate), 0)
                    }
                });
                // Create lines
    ...
    router.put('/recurring-invoices/:id', validateBody(schemas.recurringInvoiceUpdate), async (req, res) => {
        try {
            const { id } = req.params;
            const { lines, ...data } = req.body;
            const result = await prisma.$transaction(async (tx) => {
                ...
                const recurring = await tx.recurringInvoice.update({
                    ...
                    data: {
                        ...
                        totalAmount: lines ? lines.reduce((sum, line) => sum + calcLineTotal(line.quantity, line.unitPrice, line.taxRate), 0) : undefined
                    }
                });
    ...
    router.patch('/recurring-invoices/:id/status', async (req, res) => {
        try {
            const { id } = req.params;
            ...
            if (oldRecurring?.customer?.email && oldRecurring.status !== status) {
                try {
                    ...
                    await recurringEmailService.sendRecurringInvoiceStatusChange(recurring, oldRecurring.customer, oldRecurring.status, status);
                }
                catch (emailError) {
    ...
    router.get('/recurring-invoices/:id/history', async (req, res) => {
        try {
            const { id } = req.params;
            const page = Math.max(1, parseInt(String(req.query.page || '1'), 10));
            const pageSize = Math.min(100, Math.max(1, parseInt(String(req.query.pageSize || '20'), 10)));
    ...
    router.post('/recurring-invoices/:id/generate', async (req, res) => {
        try {
            const { id } = req.params;
    ...
    function calculateNextRunDate(startDate, frequency, interval) {
    ...
    router.post('/invoices/:id/send-reminder', async (req, res) => {
        ...
        try {
            ...
            await NotificationService.sendPaymentReminder(invoice, type);
    ...
    router.post('/invoices/:id/send-notification', async (req, res) => {
        const { id } = req.params;
        const { type = 'sent' } = req.body;
        try {
            await NotificationService.sendInvoiceNotification(id, type);
    ...
    router.post('/invoices/:id/send-confirmation', async (req, res) => {
        const { id } = req.params;
    ...
    // Bulk send reminders
    router.post('/invoices/send-bulk-reminders', async (req, res) => {
    ...
    router.post('/approval-workflows', async (req, res) => {
        ...
        try {
            const workflow = await InvoiceApprovalService.createInvoiceApprovalWorkflow(req.tenantId, companyId, {
                name,
                description,
                entityType: 'invoice',
    ...
    router.post('/invoices/:id/trigger-approval', async (req, res) => {
        const { id } = req.params;
    ...
    router.post('/approvals/:id/action', async (req, res) => {
        ...
        try {
            await InvoiceApprovalService.processApprovalAction(req.tenantId, id, action, comments, escalationReason);
    ...
    router.get('/invoices/:id/approval-status', async (req, res) => {
        const { id } = req.params;
        try {
    ...
    router.post('/invoices/process-document', async (req, res) => {
        try {
            ...
            const ocrResult = await InvoiceOCRService.processInvoiceDocument(req.tenantId, fileBuffer, fileName, mimeType || 'application/pdf');
            res.json({
                success: true,
                result: ocrResult,
    ...
    router.post('/invoices/create-from-ocr', async (req, res) => {
        ...
        try {
            ...
            const invoice = await InvoiceOCRService.createInvoiceFromOCR(req.tenantId, companyId, customerId, ocrResult, additionalData);
            res.json({
                success: true,
                invoice,
    ...
    router.get('/invoices/:id/match-expenses', async (req, res) => {
        const { id } = req.params;
        try {
    ...
    router.post('/invoices/:id/apply-expense-matches', async (req, res) => {
        const { id } = req.params;
    ...
    router.post('/invoices/parse-text', async (req, res) => {
    ...
    router.post('/invoices/create-from-text', async (req, res) => {
        ...
        try {
            ...
            const result = await InvoiceNLPService.createInvoiceFromNLP(req.tenantId, companyId, parsedData, {
                customerId,
    ...
    router.get('/invoices/suggestions', async (req, res) => {
        const { companyId, text } = req.query;
        try {
            if (!companyId) {
                return res.status(400).json({ error: 'Company ID is required' });
            }
            const suggestions = await InvoiceNLPService.getInvoiceSuggestions(req.tenantId, companyId, text || '');
            res.json({
                success: true,
                suggestions,
    ...
    router.post('/invoices/validate-parsed-data', async (req, res) => {
    ...
    router.post('/ai-chat/message', async (req, res) => {
        ...
        try {
            ...
            const response = await ConversationalAccountingService.processMessage(req.tenantId, companyId, userId, message, sessionId || `session_${Date.now()}`, conversationHistory || []);
            res.json({
                success: true,
                response,
    ...
    router.post('/ai-chat/execute-action', async (req, res) => {
        const { action } = req.body;
        try {
            if (!action || !action.type) {
                return res.status(400).json({ error: 'Action type and data are required' });
            }
            const result = await ConversationalAccountingService.executeAction(req.tenantId, action);
            res.json({
    ...
    router.get('/ai-chat/history/:sessionId', async (req, res) => {
        ...
        try {
            const history = await ConversationalAccountingService.getConversationHistory(req.tenantId, sessionId, limit ? parseInt(limit) : 50);
            res.json({
                success: true,
                history,
    ...
    router.get('/ai-chat/insights/:companyId', async (req, res) => {
    ...
    router.post('/ai-chat/start-session', async (req, res) => {
