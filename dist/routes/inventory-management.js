...
router.get('/inventory-management/:companyId/products', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        const products = await inventoryManagementService.getProducts(companyId, categoryId);
        res.json({
            success: true,
            data: products
        });
...
router.get('/inventory-management/:companyId/inventory', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        const inventoryItems = await inventoryManagementService.getInventoryItems(companyId, locationId, productId);
        res.json({
            success: true,
            data: inventoryItems
        });
...
router.get('/inventory-management/:companyId/serial-numbers', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        const serialNumbers = await inventoryManagementService.getSerialNumbers(companyId, productId, locationId, status);
        res.json({
            success: true,
            data: serialNumbers
        });
...
router.post('/inventory-management/:companyId/serial-numbers', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        const serial = await inventoryManagementService.createSerialNumber(companyId, productId, locationId, serialNumber, status);
        res.status(201).json({
            success: true,
            data: serial
        });
...
router.get('/inventory-management/:companyId/transactions', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        const transactions = await inventoryManagementService.getTransactions(companyId, productId, locationId, transactionType, limit ? parseInt(limit) : 100);
        res.json({
            success: true,
            data: transactions
        });
...
router.get('/inventory-management/:companyId/valuation', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        const valuation = await inventoryManagementService.getInventoryValuation(companyId, locationId);
        res.json({
            success: true,
            data: valuation
        });
...
router.get('/inventory-management/:companyId/cost/:productId/:method', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        switch (method) {
            case 'FIFO':
                cost = await inventoryManagementService.calculateFIFOCost(productId, parseInt(quantity) || 1);
                break;
            case 'LIFO':
                cost = await inventoryManagementService.calculateLIFOCost(productId, parseInt(quantity) || 1);
                break;
            case 'AVERAGE':
...
router.get('/inventory-management/:companyId/cycle-counts', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        const cycleCounts = await inventoryManagementService.getCycleCounts(companyId, locationId, status);
        res.json({
            success: true,
            data: cycleCounts
        });
...
router.post('/inventory-management/:companyId/cycle-counts', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        const cycleCount = await inventoryManagementService.createCycleCount(companyId, locationId, productId, expectedQuantity);
        res.status(201).json({
            success: true,
            data: cycleCount
        });
...
router.put('/inventory-management/:companyId/cycle-counts/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        const cycleCount = await inventoryManagementService.updateCycleCount(id, updateData.countedQuantity, updateData.countedBy, updateData.notes);
        res.json({
            success: true,
            data: cycleCount
        });
...
router.get('/inventory-management/:companyId/reports', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), tenantMiddleware, async (req, res) => {
    try {
        ...
        const report = await inventoryManagementService.getInventoryReport(companyId, reportType, locationId, categoryId, startDate, endDate);
        res.json({
            success: true,
            data: report
        });
