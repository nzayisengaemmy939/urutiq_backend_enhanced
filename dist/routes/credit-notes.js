...
router.get('/credit-notes/:companyId/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const { id } = req.params;
...
router.post('/credit-notes/:companyId', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const created = await creditNotesService.create(req.user?.sub || 'demo-tenant-id', companyId, req.body, req.user?.sub);
    res.status(201).json({ success: true, data: created });
}));
// Update credit note
router.put('/credit-notes/:companyId/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const updated = await creditNotesService.update(req.user?.sub || 'demo-tenant-id', id, req.body);
    res.json({ success: true, data: updated });
}));
// Delete credit note
router.delete('/credit-notes/:companyId/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const { id } = req.params;
