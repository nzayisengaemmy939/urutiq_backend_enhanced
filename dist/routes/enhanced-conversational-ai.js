...
router.get('/history/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { limit = 50, sessionId } = req.query;
    try {
        const history = await enhancedConversationalAIService.getConversationHistory('demo-user-id', companyId, Number(limit));
        res.json({
            success: true,
            data: {
                history,
...
router.put('/preferences', asyncHandler(async (req, res) => {
    ...
    try {
        await enhancedConversationalAIService.updateUserPreferences('demo-user-id', preferences);
        res.json({
            success: true,
            data: {
...
router.get('/insights/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    try {
        const insights = await enhancedConversationalAIService.getLearningInsights('demo-user-id', companyId);
        res.json({
            success: true,
            data: insights
        });
...
router.get('/examples', asyncHandler(async (req, res) => {
    ...
    res.json({
        success: true,
        data: examples[language] || examples.en
    });
}));
