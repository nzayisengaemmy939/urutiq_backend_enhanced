...
router.get('/balance-sheet', asyncHandler(async (req, res) => {
    ...
    try {
        const balanceSheet = await enhancedFinancialReportingEngine.generateBalanceSheet(companyId, date, req.tenantId);
        res.json({
            success: true,
            data: balanceSheet,
...
router.get('/profit-loss', auth, asyncHandler(async (req, res) => {
    ...
    try {
        const profitAndLoss = await enhancedFinancialReportingEngine.generateProfitAndLoss(companyId, period, req.tenantId);
        res.json({
            success: true,
            data: profitAndLoss,
...
router.get('/cash-flow', auth, asyncHandler(async (req, res) => {
    const { companyId, startDate, endDate } = req.query;
    if (!companyId || !startDate || !endDate) {
        return res.status(400).json({ error: 'Company ID, start date, and end date are required' });
    }
    ...
    try {
        const cashFlow = await enhancedFinancialReportingEngine.generateCashFlow(companyId, period, req.tenantId);
        res.json({
            success: true,
            data: cashFlow,
...
router.post('/custom-report', auth, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        switch (reportType) {
            case 'balance-sheet':
                ...
                reportData = await enhancedFinancialReportingEngine.generateBalanceSheet(companyId, new Date(asOfDate), req.tenantId);
                break;
            case 'profit-loss':
                if (!startDate || !endDate) {
                    return res.status(400).json({ error: 'Start date and end date are required for profit & loss' });
                }
                reportData = await enhancedFinancialReportingEngine.generateProfitAndLoss(companyId, {
                    startDate: new Date(startDate),
                    endDate: new Date(endDate)
                }, req.tenantId);
                break;
            case 'cash-flow':
                ...
                reportData = await enhancedFinancialReportingEngine.generateCashFlow(companyId, {
                    startDate: new Date(startDate),
                    endDate: new Date(endDate)
                }, req.tenantId);
                break;
...
router.get('/ratios', auth, asyncHandler(async (req, res) => {
    const { companyId, asOfDate } = req.query;
    if (!companyId) {
        return res.status(400).json({ error: 'Company ID is required' });
    }
    ...
    try {
        ...
        const profitAndLoss = await enhancedFinancialReportingEngine.generateProfitAndLoss(companyId, { startDate: yearStart, endDate: yearEnd }, req.tenantId);
...
router.get('/comparative', auth, asyncHandler(async (req, res) => {
    ...
    try {
        ...
        if (reportType === 'balance-sheet') {
            ...
            const previousDate = new Date(previousPeriod);
            currentReport = await enhancedFinancialReportingEngine.generateBalanceSheet(companyId, currentDate, req.tenantId);
            previousReport = await enhancedFinancialReportingEngine.generateBalanceSheet(companyId, previousDate, req.tenantId);
        }
        else if (reportType === 'profit-loss') {
            const currentDates = JSON.parse(currentPeriod);
            const previousDates = JSON.parse(previousPeriod);
            currentReport = await enhancedFinancialReportingEngine.generateProfitAndLoss(companyId, { startDate: new Date(currentDates.startDate), endDate: new Date(currentDates.endDate) }, req.tenantId);
            previousReport = await enhancedFinancialReportingEngine.generateProfitAndLoss(companyId, { startDate: new Date(previousDates.startDate), endDate: new Date(previousDates.endDate) }, req.tenantId);
        }
        // Calculate changes and percentages
        const comparativeAnalysis = calculateComparativeAnalysis(currentReport, previousReport, reportType);
        res.json({
            success: true,
            data: {
...
// Save Custom Report
router.post('/save-report', auth, asyncHandler(async (req, res) => {
...
// Get Saved Reports
router.get('/saved-reports', auth, asyncHandler(async (req, res) => {
...
function calculateComparativeAnalysis(current, previous, reportType) {
    const analysis = {};
    if (reportType === 'balance-sheet') {
