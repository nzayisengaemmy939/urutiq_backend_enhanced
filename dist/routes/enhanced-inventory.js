...
export function mountEnhancedInventoryRoutes(router) {
    // ===== PRODUCT MANAGEMENT =====
    ...
    // Create location
    router.post('/locations', validateBody(locationCreateSchema), async (req, res) => {
        try {
            const data = req.body;
            const companyId = String(req.query.companyId || '');
    ...
    router.post('/movements', validateBody(inventoryMovementSchema), async (req, res) => {
        try {
            const data = req.body;
    ...
    // Create transfer
    router.post('/transfers', validateBody(transferCreateSchema), async (req, res) => {
        try {
            const data = req.body;
            const companyId = String(req.query.companyId || '');
            const result = await prisma.$transaction(async (tx) => {
                ...
                const lines = await Promise.all(data.lines.map((line) => tx.inventoryTransfer.create({
                    data: {
                        tenantId: req.tenantId,
    ...
    router.get('/analytics', async (req, res) => {
        try {
            const companyId = String(req.query.companyId || '');
            const [totalProducts, totalValue, lowStockCount, outOfStockCount, recentMovements, topProducts, categoryBreakdown,] = await Promise.all([
    ...
    router.get('/products/:id/serial-numbers', async (req, res) => {
        try {
            const { id } = req.params;
            const status = req.query.status;
            const where = { tenantId: req.tenantId, productId: id };
            if (status)
                where.status = status;
            const serialNumbers = await prisma.product.findMany({
                where,
                include: {
    ...
    router.get('/products/:id/batches', async (req, res) => {
        try {
            ...
            if (status)
                where.status = status;
            const batches = await prisma.product.findMany({
                where,
                include: {
