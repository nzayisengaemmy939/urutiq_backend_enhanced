...
router.get('/advice/:companyId/:userId', async (req, res) => {
    try {
        ...
        const advice = await coachService.getFinancialAdvice(companyId, userId, type);
        res.json({ success: true, data: advice });
    }
    catch (error) {
...
router.get('/content/:companyId/:userId', async (req, res) => {
    try {
        ...
        const content = await coachService.getEducationalContent(category, difficulty, topic);
...
router.post('/progress/:companyId/:userId/:contentId', async (req, res) => {
    try {
        ...
        const learningProgress = await coachService.updateLearningProgress(companyId, userId, contentId, progress, timeSpent, quizScore);
...
router.get('/analytics/:companyId/:userId', async (req, res) => {
    try {
        ...
        const analytics = await coachService.getCoachAnalytics(companyId, userId, periodDays ? parseInt(periodDays) : 30);
