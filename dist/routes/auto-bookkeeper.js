...
router.put('/config/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
...
router.post('/initialize/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { tenantId } = req;
    if (!companyId) {
        return res.status(400).json({ error: 'Company ID is required' });
...
router.post('/categorize/:transactionId', asyncHandler(async (req, res) => {
    ...
    try {
        const categorization = await autoBookkeeperService.categorizeTransaction(transactionId, companyId, forceAuto || false);
        res.json({
            success: true,
...
router.post('/journal-entry/:transactionId', asyncHandler(async (req, res) => {
    const { transactionId } = req.params;
    const { companyId, forceAuto } = req.body;
    const { tenantId } = req;
    if (!transactionId || !companyId) {
    ...
    try {
        const journalEntry = await autoBookkeeperService.generateJournalEntry(transactionId, companyId, forceAuto || false);
        res.json({
            success: true,
...
router.post('/reconcile/:bankTransactionId', asyncHandler(async (req, res) => {
    ...
    try {
        const reconciliation = await autoBookkeeperService.reconcileTransaction(bankTransactionId, companyId, forceAuto || false);
        res.json({
            success: true,
...
router.post('/process-pending/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { tenantId } = req;
    if (!companyId) {
        return res.status(400).json({ error: 'Company ID is required' });
...
router.get('/stats/:companyId', asyncHandler(async (req, res) => {
    ...
    try {
        const stats = await autoBookkeeperService.getStats(companyId, periodDays ? parseInt(periodDays) : 30);
        res.json({
            success: true,
            data: stats
        });
...
router.get('/insights/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { tenantId } = req;
    if (!companyId) {
        return res.status(400).json({ error: 'Company ID is required' });
...
router.get('/categorizations/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
...
router.get('/reconciliations/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
...
// Add automation rule
router.post('/rules/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { rule } = req.body;
    const { tenantId } = req;
    if (!companyId || !rule) {
...
router.put('/rules/:companyId/:ruleId', asyncHandler(async (req, res) => {
    ...
    try {
        const config = await autoBookkeeperService.getConfig(companyId);
        const updatedRules = config.rules.map(r => r.id === ruleId ? { ...r, ...rule } : r);
        const updatedConfig = await autoBookkeeperService.updateConfig(companyId, {
            rules: updatedRules
        });
        res.json({
...
router.delete('/rules/:companyId/:ruleId', asyncHandler(async (req, res) => {
    const { companyId, ruleId } = req.params;
    const { tenantId } = req;
    if (!companyId || !ruleId) {
...
router.get('/performance/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { periodDays } = req.query;
    const { tenantId } = req;
    if (!companyId) {
    ...
    try {
        ...
        const startDate = new Date(endDate.getTime() - (periodDays ? parseInt(periodDays) : 30) * 24 * 60 * 60 * 1000);
...
router.get('/dashboard/:companyId', asyncHandler(async (req, res) => {
    const { companyId } = req.params;
    const { tenantId } = req;
    if (!companyId) {
        return res.status(400).json({ error: 'Company ID is required' });
