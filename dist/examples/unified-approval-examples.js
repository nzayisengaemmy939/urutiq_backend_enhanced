...
export async function createJournalEntryApproval() {
    ...
    const approvalRequest = await UnifiedApprovalEngine.createApprovalRequest('tenant_demo', // tenantId
...
export async function createInvoiceApproval() {
    ...
    const approvalRequest = await UnifiedApprovalEngine.createApprovalRequest('tenant_demo', 'cmgfgiqos0001szdhlotx8vhg', 'invoice', 'inv-67890', 'high_value', 'user-456', {
        amount: 100000,
        customerId: 'cust-123',
        currency: 'USD',
        dueDate: '2024-12-31'
...
export async function createPurchaseOrderApproval() {
    ...
    const approvalRequest = await UnifiedApprovalEngine.createApprovalRequest('tenant_demo', 'cmgfgiqos0001szdhlotx8vhg', 'purchase_order', 'po-54321', 'equipment', 'user-789', {
        amount: 25000,
        vendorId: 'vendor-456',
...
export async function processApproval() {
    ...
    const result = await UnifiedApprovalEngine.processApprovalAction('tenant_demo', // tenantId
...
export async function rejectApproval() {
    ...
    const result = await UnifiedApprovalEngine.processApprovalAction('tenant_demo', 'approval-request-123', 'assignee-456', 'reject', 'Insufficient documentation provided', undefined);
...
export async function escalateApproval() {
    ...
    const result = await UnifiedApprovalEngine.processApprovalAction('tenant_demo', 'approval-request-123', 'assignee-456', 'escalate', 'Requires higher authority approval', 'Amount exceeds department limit');
...
export async function createJournalEntryWithApproval() {
    ...
    const approvalRequest = await UnifiedApprovalEngine.createApprovalRequest('tenant_demo', 'cmgfgiqos0001szdhlotx8vhg', 'journal_entry', journalEntry.id, 'adjustment', 'user-123', { amount: journalEntry.total });
...
export async function createInvoiceWithApproval() {
    ...
    // 2. Create approval request
    const approvalRequest = await UnifiedApprovalEngine.createApprovalRequest('tenant_demo', 'cmgfgiqos0001szdhlotx8vhg', 'invoice', invoice.id, 'high_value', 'user-456', { amount: invoice.total, customerId: 'cust-123' });
