...
export async function createJournalEntryWithUnifiedApproval(tenantId, companyId, journalEntryData) {
    ...
    const journalEntry = await prisma.journalEntry.create({
        data: {
            ...
            total: journalEntryData.lines.reduce((sum, line) => sum + (line.debit || 0) + (line.credit || 0), 0)
        }
    });
    ...
    const totalAmount = journalEntryData.lines.reduce((sum, line) => sum + (line.debit || 0) + (line.credit || 0), 0);
    const needsApproval = totalAmount > 1000; // Example threshold
    if (needsApproval) {
        // Step 4: Create unified approval request
        const approvalRequest = await UnifiedApprovalEngine.createApprovalRequest(tenantId, companyId, 'journal_entry', journalEntry.id, 'adjustment', // or determine based on entry type
...
export async function createInvoiceWithUnifiedApproval(tenantId, companyId, invoiceData) {
    ...
    if (needsApproval) {
        // Step 4: Create unified approval request
        const approvalRequest = await UnifiedApprovalEngine.createApprovalRequest(tenantId, companyId, 'invoice', invoice.id, 'high_value', invoiceData.createdById, {
...
export async function createPurchaseOrderWithUnifiedApproval(tenantId, companyId, poData) {
    ...
    if (needsApproval) {
        // Step 4: Create unified approval request
        const approvalRequest = await UnifiedApprovalEngine.createApprovalRequest(tenantId, companyId, 'purchase_order', purchaseOrder.id, 'equipment', // or determine based on category
        poData.createdById, {
            amount: poData.total,
            vendorId: poData.vendorId,
            poNumber: poData.poNumber,
...
export async function handleApprovalCallback(tenantId, approvalRequestId, action) {
...
export async function processBatchApprovals(tenantId, companyId, entityType, entityIds, requestedBy) {
    const results = [];
    for (const entityId of entityIds) {
        try {
            ...
            const approvalRequest = await UnifiedApprovalEngine.createApprovalRequest(tenantId, companyId, entityType, entityId, 'batch_processing', requestedBy, { amount, ...metadata });
