...
router.post('/', asyncHandler(async (req, res) => {
    try {
        ...
        const { name, email, phone, businessName, contactPerson, industry, website, address, city, state, postalCode, country, currency, paymentTerms, creditLimit, taxNumber, hasPortalAccess, emailNotifications, smsNotifications, preferredLanguage, notes, tags, source, assignedTo } = req.body;
        if (!name || !email) {
...
router.post('/:id/share-document', asyncHandler(async (req, res) => {
    try {
        const tenantId = req.header('x-tenant-id') || 'default';
        const companyId = req.header('x-company-id') || 'default';
