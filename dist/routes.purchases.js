...
export function mountPurchaseRoutes(router) {
    // Vendors
    ...
    router.post('/vendors', validateBody(schemas.vendorCreate), async (req, res) => {
        const data = req.body;
    ...
    router.post('/bills', validateBody(schemas.billCreate), async (req, res) => {
        console.log('📝 Bill creation request body:', req.body);
        const { companyId, vendorId, billNumber, billDate, dueDate, currency, lines, purchaseType, vendorCurrency, exchangeRate, freightCost = 0, customsDuty = 0, otherImportCosts = 0, allocateLandedCost = false } = req.body;
        if (!companyId) {
    ...
    router.post('/bills/:id/post', validateBody(schemas.billPostAction), async (req, res) => {
        const { id } = req.params;
        ...
        if (!ap)
            return res.status(400).json({ error: 'missing_accounts', message: 'Require mapping for AP' });
        ...
        try {
            enqueueWebhooks('bill.posted', { billId: bill.id, companyId: bill.companyId });
        }
        catch (e) {
