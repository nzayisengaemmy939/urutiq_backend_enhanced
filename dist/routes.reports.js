...
import { z } from 'zod';
import { financialReportCreate, financialReportUpdate, reportItemCreate, reportItemUpdate, reportScheduleCreate, reportScheduleUpdate, reportTemplateCreate, reportExecutionCreate, reportQuery, reportBuilderPreviewRequest, reportBuilderTemplateSave } from './validate.js';
...
async function audit(tenantId, userId, action, meta) {
    try {
...
async function calculateReportData(reportId, companyId, parameters) {
    ...
    for (const item of report.reportItems) {
        ...
        switch (item.type) {
                ...
                if (item.accountIds) {
                    ...
                    value = accounts.reduce((sum, account) => {
                        const balance = account.journalEntries.reduce((acc, entry) => {
                            // dimension filters
                            if (parameters?.department && entry.department !== parameters.department)
                                return acc;
                            if (parameters?.project && entry.project !== parameters.project)
                                return acc;
                            if (parameters?.location && entry.location !== parameters.location)
                                return acc;
                            // date filters on related journal
                            if (hasDateRange) {
                                const d = entry.journal?.date ? new Date(entry.journal.date) : undefined;
            ...
            case 'chart':
                ...
                break;
            case 'pivot': {
                const cfg = item.configuration ? JSON.parse(item.configuration) : { groupBy: 'type' };
                ...
                if (dimKeys.includes(groupKey) && Array.isArray(cfg.accountIds) && cfg.accountIds.length > 0) {
                    ...
                    for (const l of lines) {
                        const key = String((l[groupKey]) || 'Unspecified');
    ...
    if (report.type === 'balance_sheet') {
        result.summary.totalAssets = result.items
            .filter((item) => item.type === 'account' && item.details.some((d) => d.type === 'Asset'))
            .reduce((sum, item) => sum + Math.abs(item.value), 0);
        result.summary.totalLiabilities = result.items
            .filter((item) => item.type === 'account' && item.details.some((d) => d.type === 'Liability'))
            .reduce((sum, item) => sum + Math.abs(item.value), 0);
        result.summary.totalEquity = result.items
            .filter((item) => item.type === 'account' && item.details.some((d) => d.type === 'Equity'))
            .reduce((sum, item) => sum + Math.abs(item.value), 0);
    }
    else if (report.type === 'income_statement') {
        result.summary.totalRevenue = result.items
            .filter((item) => item.type === 'account' && item.details.some((d) => d.type === 'Revenue'))
            .reduce((sum, item) => sum + Math.abs(item.value), 0);
        result.summary.totalExpenses = result.items
            .filter((item) => item.type === 'account' && item.details.some((d) => d.type === 'Expense'))
            .reduce((sum, item) => sum + Math.abs(item.value), 0);
...
async function calculateReportDataFromSpec(spec, companyId) {
    const result = {
        report: {
            id: 'preview',
            name: spec.name,
    ...
    for (const item of spec.items) {
        ...
        switch (item.type) {
            ...
            case 'chart':
                ...
                break;
            case 'pivot': {
                const cfg = item.configuration || { groupBy: 'type' };
    ...
    if (spec.type === 'balance_sheet') {
        result.summary.totalAssets = result.items
            .filter((it) => it.type === 'account' && it.details.some((d) => d.type === 'Asset'))
            .reduce((sum, it) => sum + Math.abs(it.value), 0);
        result.summary.totalLiabilities = result.items
            .filter((it) => it.type === 'account' && it.details.some((d) => d.type === 'Liability'))
            .reduce((sum, it) => sum + Math.abs(it.value), 0);
        result.summary.totalEquity = result.items
            .filter((it) => it.type === 'account' && it.details.some((d) => d.type === 'Equity'))
            .reduce((sum, it) => sum + Math.abs(it.value), 0);
    }
    else if (spec.type === 'income_statement') {
        result.summary.totalRevenue = result.items
            .filter((it) => it.type === 'account' && it.details.some((d) => d.type === 'Revenue'))
            .reduce((sum, it) => sum + Math.abs(it.value), 0);
        result.summary.totalExpenses = result.items
            .filter((it) => it.type === 'account' && it.details.some((d) => d.type === 'Expense'))
            .reduce((sum, it) => sum + Math.abs(it.value), 0);
        result.summary.netIncome = result.summary.totalRevenue - result.summary.totalExpenses;
    }
    return result;
...
async function generateReportOutput(format, data) {
    switch (format) {
        case 'json': {
        ...
        case 'excel': {
            ...
            (data?.items || []).forEach((it) => ws.addRow({ id: it.id, name: it.name, type: it.type, order: it.order, value: it.value }));
...
router.post('/:id/deliver', async (req, res) => {
    try {
        ...
        if (channels.email && Array.isArray(channels.email.recipients) && channels.email.recipients.length) {
            ...
            deliveries.email = { ok: true, id: mail.messageId || true };
        }
        // Slack delivery
        if (channels.slack && channels.slack.webhookUrl) {
            try {
                ...
                deliveries.slack = { ok: false, error: e.message };
            }
        }
        ...
        if (channels.webhook && channels.webhook.url) {
            try {
                ...
                deliveries.webhook = { ok: false, error: e.message };
            }
        }
...
router.get('/:id/items/:itemId/lineage', async (req, res) => {
    try {
        const { companyId } = req;
        ...
        const lines = await prisma.journalLine.findMany({
            where: { accountId: { in: accountIds }, tenantId: req.tenantId },
...
router.get('/:id/items/:itemId/audit', async (req, res) => {
    try {
        const { id, itemId } = req.params;
...
router.get('/', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), async (req, res) => {
    try {
...
router.post('/', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), async (req, res) => {
    try {
        const { companyId } = req.query;
...
router.get('/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), async (req, res) => {
    try {
        const { companyId } = req;
        const { id } = req.params;
        if (!companyId) {
...
router.post('/:id/execute', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), async (req, res) => {
    try {
        const { companyId } = req.query;
        const { id } = req.params;
        const userId = req.user?.sub;
        ...
        try {
            ...
            const result = await calculateReportData(id, companyId, data.parameters);
...
router.get('/executions', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), async (req, res) => {
    try {
        const { companyId } = req.query;
...
router.get('/templates', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), async (req, res) => {
    try {
...
router.post('/templates', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), async (req, res) => {
    try {
        const userId = req.user?.sub;
        if (!userId) {
...
router.post('/builder/preview', async (req, res) => {
    try {
...
router.post('/builder/templates', async (req, res) => {
    try {
        const { userId } = req;
        if (!userId) {
            return res.status(400).json({ error: 'User ID is required' });
