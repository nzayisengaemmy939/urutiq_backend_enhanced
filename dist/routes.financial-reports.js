...
router.get('/reports/balance-sheet', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'viewer']), asyncHandler(async (req, res) => {
    ...
    const result = await FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateBalanceSheet, reportParams);
    res.json({
        success: true,
        data: result,
...
router.get('/reports/income-statement', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'viewer']), asyncHandler(async (req, res) => {
    const params = financialReportSchema.parse({
        ...req.query,
        companyId: req.query.companyId || req.user?.companyId
    });
    ...
    const result = await FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateIncomeStatement, reportParams);
    res.json({
        success: true,
        data: result,
        generatedAt: new Date().toISOString(),
...
router.get('/reports/cash-flow', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'viewer']), asyncHandler(async (req, res) => {
    const params = financialReportSchema.parse({
        ...req.query,
        companyId: req.query.companyId || req.user?.companyId
    });
    ...
    const result = await FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateCashFlowStatement, reportParams);
    res.json({
        success: true,
        data: result,
        generatedAt: new Date().toISOString(),
...
router.get('/reports/comprehensive', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const params = financialReportSchema.parse({
        ...req.query,
        companyId: req.query.companyId || req.user?.companyId
    });
    ...
    const [balanceSheet, incomeStatement, cashFlow] = await Promise.all([
        FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateBalanceSheet, reportParams),
        FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateIncomeStatement, reportParams),
        FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateCashFlowStatement, reportParams)
    ]);
    res.json({
        success: true,
        data: {
...
router.post('/reports/export', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    ...
    switch (reportType) {
        case 'balance_sheet':
            reportData = await FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateBalanceSheet, params);
            break;
        case 'income_statement':
            reportData = await FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateIncomeStatement, params);
            break;
        case 'cash_flow':
            reportData = await FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateCashFlowStatement, params);
            break;
        case 'comprehensive':
            const [balanceSheet, incomeStatement, cashFlow] = await Promise.all([
                FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateBalanceSheet, params),
                FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateIncomeStatement, params),
                FinancialReportingService.generateReportWithComparison(FinancialReportingService.generateCashFlowStatement, params)
            ]);
...
router.get('/reports/templates', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const templates = [
        {
...
router.post('/reports/schedule', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin']), asyncHandler(async (req, res) => {
