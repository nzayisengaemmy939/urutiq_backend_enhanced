...
// Create OAuth client
router.post('/oauth2/clients', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin']), asyncHandler(async (req, res) => {
...
// Get OAuth clients
router.get('/oauth2/clients', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin']), asyncHandler(async (req, res) => {
...
// Update OAuth client
router.put('/oauth2/clients/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin']), asyncHandler(async (req, res) => {
    const { id } = req.params;
...
// Delete OAuth client
router.delete('/oauth2/clients/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin']), asyncHandler(async (req, res) => {
    const { id } = req.params;
...
// Create API key
router.post('/api-keys', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
...
// Get API keys
router.get('/api-keys', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const { companyId, page = 1, limit = 50 } = req.query;
    if (!companyId) {
        throw new ApiError(400, 'COMPANY_ID_REQUIRED', 'Company ID is required');
    }
    const result = await ApiKeyService.getApiKeys(req.tenantId, companyId, parseInt(page), parseInt(limit));
    res.json(result);
}));
// Update API key
router.put('/api-keys/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const { id } = req.params;
...
// Delete API key
router.delete('/api-keys/:id', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), asyncHandler(async (req, res) => {
    const { id } = req.params;
    await ApiKeyService.deleteApiKey(id, req.tenantId);
    res.status(204).end();
}));
// Get API key permissions
router.get('/api-keys/permissions', asyncHandler(async (req, res) => {
...
router.get('/analytics/api-usage', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin']), asyncHandler(async (req, res) => {
    const { startDate = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(), // 7 days ago
    endDate = new Date().toISOString(), companyId } = req.query;
    const analytics = await PerformanceMonitoringService.getApiUsageAnalytics(req.tenantId, new Date(startDate), new Date(endDate), companyId);
    res.json(analytics);
}));
// Get performance metrics
router.get('/analytics/performance', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin']), asyncHandler(async (req, res) => {
    const { startDate = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(), endDate = new Date().toISOString(), companyId } = req.query;
    const metrics = await PerformanceMonitoringService.getPerformanceMetrics(req.tenantId, new Date(startDate), new Date(endDate), companyId);
    res.json(metrics);
}));
// Get custom metrics
router.get('/analytics/metrics/:metricName', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin']), asyncHandler(async (req, res) => {
    const { metricName } = req.params;
    const { startDate = new Date(Date.now() - 7 * 24 * 60 * 60 * 1000).toISOString(), endDate = new Date().toISOString() } = req.query;
    const metrics = await PerformanceMonitoringService.getCustomMetrics(req.tenantId, metricName, new Date(startDate), new Date(endDate));
    res.json(metrics);
}));
// Record custom metric
router.post('/analytics/metrics', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin']), asyncHandler(async (req, res) => {
    const { metricName, metricValue, metricUnit, tags } = req.body;
    await PerformanceMonitoringService.recordMetric(req.tenantId, metricName, metricValue, metricUnit, tags);
    res.status(201).json({ success: true });
}));
// OAuth 2.1 Scopes endpoint
router.get('/oauth2/scopes', asyncHandler(async (req, res) => {
    res.json({
        scopes: OAUTH_SCOPES,
        description: {
