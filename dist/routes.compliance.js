...
export function mountComplianceRoutes(router) {
    // Tax Rates
    router.get('/tax-rates', requireAuth, async (req, res) => {
    ...
    router.post('/tax-rates', requireAuth, validateBody(complianceSchemas.taxRateCreate), async (req, res) => {
        const { companyId, taxName, rate, appliesTo, isActive } = req.body;
        const created = await prisma.taxRate.create({
            data: { tenantId: req.tenantId, companyId, taxName, rate, appliesTo, isActive }
        });
        res.status(201).json(created);
    });
    router.put('/tax-rates/:id', requireAuth, validateBody(complianceSchemas.taxRateUpdate), async (req, res) => {
        const { id } = req.params;
    ...
    router.delete('/tax-rates/:id', requireAuth, async (req, res) => {
        const { id } = req.params;
    ...
    router.post('/reports', requireAuth, validateBody(complianceSchemas.reportGenerate), async (req, res) => {
        const { companyId, reportType, parameters, createdBy } = req.body;
    ...
    router.put('/reports/:id', requireAuth, validateBody(complianceSchemas.reportGenerate), async (req, res) => {
        try {
            const { id } = req.params;
            const { companyId, reportType, parameters, createdBy } = req.body;
    ...
    router.get('/reports/:filename', async (req, res) => {
        const tenantId = req.headers['x-tenant-id'] || req.query.tenantId || 'tenant_demo';
...
%%EOF`;
    ...
    router.post('/audit-logs', requireAuth, validateBody(complianceSchemas.auditLogCreate), async (req, res) => {
        const { userId, action, entityType, entityId, ipAddress, userAgent } = req.body;
    ...
    router.post('/audit-logs/action', requireAuth, async (req, res) => {
