...
export function mountWorkspaceRoutes(router) {
    // Workspaces
    ...
    router.post('/workspaces', requireAuth, validateBody(workspaceSchemas.workspaceCreate), async (req, res) => {
    ...
    router.put('/workspaces/:id', requireAuth, validateBody(workspaceSchemas.workspaceUpdate), async (req, res) => {
        const { id } = req.params;
        const updated = await prisma.workspace.update({ where: { id, tenantId: req.tenantId }, data: req.body });
        res.json(updated);
    });
    ...
    // Workspace Members
    router.get('/workspaces/:id/members', requireAuth, async (req, res) => {
        const { id } = req.params;
    ...
    router.post('/workspaces/:id/members', requireAuth, validateBody(workspaceSchemas.memberAdd), async (req, res) => {
        const { id } = req.params;
        const { userId, role } = req.body;
        const created = await prisma.workspaceMember.create({ data: { tenantId: req.tenantId, workspaceId: id, userId, role } });
        res.status(201).json(created);
    });
    router.delete('/workspaces/:id/members/:memberId', requireAuth, async (req, res) => {
    ...
    router.post('/files', requireAuth, validateBody(fileSchemas.fileCreate), async (req, res) => {
        const { companyId, workspaceId, name, mimeType, sizeBytes, storageKey, sha256 } = req.body;
    ...
    router.post('/files/upload', requireAuth, async (req, res, next) => {
        try {
    ...
    router.post('/notifications', requireAuth, validateBody(notificationSchemas.notifyCreate), async (req, res) => {
        const { companyId, userId, type, title, body } = req.body;
    ...
    router.put('/notifications/:id', requireAuth, validateBody(notificationSchemas.notifyUpdate), async (req, res) => {
        const { id } = req.params;
        const updated = await prisma.notification.update({ where: { id, tenantId: req.tenantId }, data: req.body });
        res.json(updated);
    });
