...
export function mountPurchaseOrderRoutes(router) {
    // List match exceptions (out-of-tolerance POs with linked bills)
    router.get('/purchase-orders/match-exceptions', async (req, res) => {
        const limit = Math.max(1, Math.min(100, parseInt(String(req.query.limit || '50'), 10)));
        const tolerancePctDefault = Number(req.query.tolerancePct || process.env.THREE_WAY_TOLERANCE_PCT || 2);
        const toleranceAbsDefault = Number(req.query.toleranceAbs || process.env.THREE_WAY_TOLERANCE_ABS || 5);
        ...
        try {
            ...
            const exceptions = pos.map(po => {
                ...
                const isImport = String(po.purchaseType || '').toLowerCase() === 'import';
    ...
    router.post('/purchase-orders/:id/resolve-exception', async (req, res) => {
        const { id } = req.params;
        try {
    ...
    router.post('/purchase-orders', validateBody(purchaseOrderSchemas.create), async (req, res) => {
        const data = req.body;
        try {
            const result = await prisma.$transaction(async (tx) => {
                // Calculate total amount including import costs
                const baseTotal = data.lines.reduce((sum, line) => {
                ...
                const lines = await Promise.all(data.lines.map((line) => tx.purchaseOrderLine.create({
                    data: {
                        tenantId: req.tenantId,
    ...
    router.put('/purchase-orders/:id', validateBody(purchaseOrderSchemas.update), async (req, res) => {
        const { id } = req.params;
        const data = req.body;
        try {
            const result = await prisma.$transaction(async (tx) => {
                ...
                if (data.lines) {
                    baseTotal = data.lines.reduce((sum, line) => {
                        const lineTotal = line.quantity * line.unitPrice * (1 + line.taxRate / 100);
                        return sum + lineTotal;
                    }, 0);
                }
    ...
    router.post('/purchase-orders/:id/receipts', validateBody(purchaseOrderSchemas.receipt), async (req, res) => {
        const { id } = req.params;
        const data = req.body;
        try {
            const result = await prisma.$transaction(async (tx) => {
                ...
                const items = await Promise.all(data.items.map((item) => tx.receiptItem.create({
                    data: {
                        tenantId: req.tenantId,
    ...
    router.get('/purchase-orders/:id/receipts', async (req, res) => {
        const { id } = req.params;
        try {
    ...
    router.post('/purchase-orders/:id/deliver', validateBody(purchaseOrderSchemas.delivery), async (req, res) => {
        const { id } = req.params;
        const data = req.body;
        try {
    ...
    router.get('/purchase-orders/:id/delivery-status', async (req, res) => {
        const { id } = req.params;
        try {
    ...
    router.post('/purchase-orders/:id/match-bill', async (req, res) => {
        const { id } = req.params;
            ...
            const result = await prisma.$transaction(async (tx) => {
                ...
                try {
                    if (purchaseOrder.companyId) {
                        ...
                        const getVal = (k) => settings.find(s => s.key === k)?.value;
                        const isImport = String(purchaseOrder.purchaseType || '').toLowerCase() === 'import';
                        ...
                        if (resolvedPct !== undefined)
                            tolerancePct = Number(resolvedPct);
                        if (resolvedAbs !== undefined)
                            toleranceAbs = Number(resolvedAbs);
                    }
                }
                catch { }
                const poTotal = Number(purchaseOrder.totalAmount);
                const billTotal = Number(bill.totalAmount);
                const receiptQtyTotal = purchaseOrder.receipts.reduce((sum, receipt) => sum + receipt.items.reduce((itemSum, item) => itemSum + Number(item.quantityAccepted), 0), 0);
                const diff = Math.abs(poTotal - billTotal);
                const pct = poTotal > 0 ? (diff / poTotal) * 100 : 0;
    ...
    router.post('/purchase-orders/:id/send-to-vendor', async (req, res) => {
        const { id } = req.params;
        ...
        try {
            ...
            await emailService.sendPurchaseOrderToVendor(purchaseOrder.vendor.email, purchaseOrder.vendor.name, purchaseOrder.poNumber, pdfBuffer, purchaseOrder.company.name, notes);
