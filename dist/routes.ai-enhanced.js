...
export function mountEnhancedAIRoutes(router) {
    ...
    router.post('/ai/categorize/transaction', asyncHandler(async (req, res) => {
        ...
        const categorization = await enhancedAIService.categorizeTransaction(req.tenantId, companyId, description, parseFloat(amount), transactionType || 'expense');
        res.json({
            success: true,
            categorization,
    ...
    router.post('/ai/categorize/batch', asyncHandler(async (req, res) => {
        ...
        for (const transaction of transactions) {
            const categorization = await enhancedAIService.categorizeTransaction(req.tenantId, companyId, transaction.description, parseFloat(transaction.amount), transaction.transactionType || 'expense');
    ...
    router.post('/ai/anomalies/enhanced', asyncHandler(async (req, res) => {
        const { companyId, transactions } = req.body;
        if (!companyId || !transactions || !Array.isArray(transactions)) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        const anomalies = await enhancedAIService.detectAnomaliesEnhanced(req.tenantId, companyId, transactions);
        res.json({
            success: true,
            anomalies,
    ...
    router.post('/ai/reports/natural-language', asyncHandler(async (req, res) => {
    ...
    router.get('/ai/reports/templates', asyncHandler(async (req, res) => {
        const templates = [
            {
                id: 'cash-flow',
    ...
    router.post('/ai/insights/smart', asyncHandler(async (req, res) => {
        const { companyId, historicalData } = req.body;
        if (!companyId || !historicalData || !Array.isArray(historicalData)) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        const insights = await enhancedAIService.generateSmartInsights(req.tenantId, companyId, historicalData);
        res.json({
            success: true,
            insights,
    ...
    router.post('/ai/forecast/cash-flow', asyncHandler(async (req, res) => {
        ...
        const insights = await enhancedAIService.generateSmartInsights(req.tenantId, companyId, cashFlowData);
    ...
    router.post('/ai/assistant/query', asyncHandler(async (req, res) => {
        const { companyId, query, context } = req.body;
        if (!companyId || !query) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        const result = await enhancedAIService.processAIAssistantQuery(req.tenantId, companyId, query, context);
        res.json({
            success: true,
            result,
    ...
    router.get('/ai/assistant/history', asyncHandler(async (req, res) => {
    ...
    // 6. Batch Processing
    router.post('/ai/process/batch', asyncHandler(async (req, res) => {
        const { companyId, transactions } = req.body;
        if (!companyId || !transactions || !Array.isArray(transactions)) {
            return res.status(400).json({ error: 'Missing required fields' });
        }
        const result = await enhancedAIService.processBatchTransactions(req.tenantId, companyId, transactions);
        res.json({
            success: true,
            result,
            processed: {
    ...
    // 7. AI Health Check
    router.get('/ai/health/enhanced', asyncHandler(async (req, res) => {
        const companyId = String(req.query.companyId || '');
        try {
    ...
    router.post('/ai/feedback/learning', asyncHandler(async (req, res) => {
    ...
    router.post('/ai/models/switch', asyncHandler(async (req, res) => {
