...
router.get('/:id/pdf', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'manager']), validateRequest({
    ...
    const { id } = req.params;
    const { includeReceived, generatedBy } = req.query;
    try {
...
router.post('/:id/pdf/email', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'manager']), validateRequest({
    body: emailSchema
}), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { to, cc, bcc, subject, message, includeReceived } = req.body;
    try {
        // Generate PDF
        const pdfBuffer = await purchaseOrderPDFService.generatePurchaseOrderPDF(id, {
            includeReceived,
...
router.post('/:id/pdf/save', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant']), validateRequest({
    body: z.object({
    ...
    const { id } = req.params;
    const { filePath, includeReceived, generatedBy } = req.body;
    try {
        // Generate default file path if not provided
        const defaultPath = path.join(process.cwd(), 'uploads', 'purchase-orders', `PO-${id}-${Date.now()}.pdf`);
...
router.get('/:id/pdf/base64', authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['admin', 'accountant', 'manager']), validateRequest({
    query: pdfGenerationSchema
}), asyncHandler(async (req, res) => {
    const { id } = req.params;
    const { includeReceived, generatedBy } = req.query;
