...
router.get('/trial-balance', cacheStrategies.medium('trial-balance'), authMiddleware(process.env.JWT_SECRET || 'dev-secret'), requireRoles(['accountant', 'admin']), asyncHandler(async (req, res) => {
    try {
...
router.get('/ar-aging', async (req, res) => {
    try {
        ...
        const agingReport = customers.map(customer => {
            ...
            const calculateAmount = (invoiceList) => {
...
router.get('/ap-aging', async (req, res) => {
    try {
        ...
        const agingReport = vendors.map(vendor => {
            ...
            const calculateAmount = (billList) => {
