...
import { aiConfigurationService, DEFAULT_INDUSTRY_CONFIGS, DEFAULT_AI_CONFIG, DEFAULT_BEHAVIOR_CONFIG } from './ai-config';
export function mountAIConfigRoutes(router) {
    // Get all available industry configurations
    router.get('/ai/config/industries', asyncHandler(async (req, res) => {
    ...
    router.get('/ai/config/:companyId', asyncHandler(async (req, res) => {
    ...
    router.put('/ai/config/:companyId/prompts', asyncHandler(async (req, res) => {
        const { companyId } = req.params;
        const { tenantId } = req;
    ...
    router.put('/ai/config/:companyId/behavior', asyncHandler(async (req, res) => {
        const { companyId } = req.params;
        const { tenantId } = req;
    ...
    router.put('/ai/config/:companyId/categories', asyncHandler(async (req, res) => {
        const { companyId } = req.params;
        const { tenantId } = req;
    ...
    router.post('/ai/config/:companyId/industry/:industryCode', asyncHandler(async (req, res) => {
    ...
    router.post('/ai/config/:companyId/reset', asyncHandler(async (req, res) => {
        const { companyId } = req.params;
        const { tenantId } = req;
    ...
    router.get('/ai/config/defaults', asyncHandler(async (req, res) => {
        res.json({
            success: true,
            data: {
    ...
    router.post('/ai/config/:companyId/test', asyncHandler(async (req, res) => {
        const { companyId } = req.params;
        const { tenantId } = req;
    ...
    router.get('/ai/config/:companyId/history', asyncHandler(async (req, res) => {
        const { companyId } = req.params;
        const { tenantId } = req;
    ...
    router.get('/ai/config/:companyId/export', asyncHandler(async (req, res) => {
        const { companyId } = req.params;
        const { tenantId } = req;
        const [prompts, behavior, categories] = await Promise.all([
            aiConfigurationService.getCustomizedPrompts(tenantId, companyId),
    ...
    router.post('/ai/config/:companyId/import', asyncHandler(async (req, res) => {
        const { companyId } = req.params;
        const { tenantId } = req;
    ...
    router.get('/ai/config/:companyId/stats', asyncHandler(async (req, res) => {
        const { companyId } = req.params;
        const { tenantId } = req;
        const categories = await aiConfigurationService.getCustomizedCategories(tenantId, companyId);
        const behavior = await aiConfigurationService.getCustomizedBehavior(tenantId, companyId);
        const stats = {
            totalCategories: categories.expenseCategories.length +
            ...
            activeCategories: categories.expenseCategories.filter(c => c.isActive).length +
