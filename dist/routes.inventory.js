...
export function mountInventoryRoutes(router) {
    ...
    router.post('/products', validateBody(inventorySchemas.productCreate), async (req, res) => {
        const data = req.body;
    ...
    router.put('/products/:id', validateBody(inventorySchemas.productUpdate), async (req, res) => {
        const { id } = req.params;
        const data = req.body;
        try {
    ...
    router.get('/locations', async (req, res) => {
        ...
        const locationsWithStock = locations.map(location => {
            const totalStock = location.products.reduce((sum, productLocation) => sum + Number(productLocation.quantity || 0), 0);
            const totalValue = location.products.reduce((sum, productLocation) => sum + (Number(productLocation.quantity || 0) * Number(productLocation.product.costPrice || 0)), 0);
            const totalRetailValue = location.products.reduce((sum, productLocation) => sum + (Number(productLocation.quantity || 0) * Number(productLocation.product.unitPrice || 0)), 0);
            return {
                ...location,
                stockMetrics: {
                    totalStock,
    ...
    router.post('/locations', validateBody(inventorySchemas.locationCreate), async (req, res) => {
        const data = req.body;
    ...
    router.put('/locations/:id', validateBody(inventorySchemas.locationUpdate), async (req, res) => {
        const { id } = req.params;
        const data = req.body;
        try {
    ...
    router.post('/alerts/settings', async (req, res) => {
        try {
            const { lowStockThreshold, overstockThreshold, criticalStockThreshold, emailNotifications, smsNotifications, dashboardAlerts, autoAcknowledgeDays, dailyDigestTime, weeklySummaryDay, weeklySummaryTime, immediateAlerts, immediateAlertsCriticalOnly } = req.body;
    ...
    router.post('/transfers', validateBody(inventorySchemas.transferCreate), async (req, res) => {
        try {
            const { productId, fromLocationId, toLocationId, quantity, transferDate, reference, notes, requestedBy } = req.body;
    ...
    router.get('/forecast-insights', async (req, res) => {
        const companyId = String(req.query.companyId || '');
        const period = String(req.query.period || '30d');
        try {
            // Calculate date range based on period
    ...
    router.post('/inventory-movements', validateBody(inventorySchemas.movementCreate), async (req, res) => {
        const { productId, movementType, quantity, movementDate, reference, locationId, reason, unitCost } = req.body;
    ...
    router.post('/product-locations', async (req, res) => {
        const { productId, locationId, quantity, reorderPoint, maxQuantity } = req.body;
    ...
    router.delete('/product-locations/:id', async (req, res) => {
        const { id } = req.params;
